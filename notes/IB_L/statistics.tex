...
\author{Lectured by D. Spiegelhalter\\\small Notes taken by Dexter Chua}
\date{Lent 2015}
...
For example, we can have $X\sim \Poisson(\mu)$ and $\theta = \mu\in\Theta = (0, \infty)$.
...
\[
  \var_\theta(T_U) = \var_\theta(X)/n^2 = \theta(1 - \theta)/n.
\]
...
Also, suppose $X\sim \Poisson(\lambda)$, and for some reason, you want to estimate $\theta = [P(X = 0)]^2 = e^{-2\lambda}$. Then any estimator $T(X)$ must satisfy $\E_\theta(T(X)) = \theta$, or equivalently,
\[
