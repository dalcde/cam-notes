...
\begin{eg}
  Suppose that $X_1, \cdots, X_n$ Are iid $\Poisson(\lambda)$ random variables, and $\lambda$ has na exponential distribution with mean $1$.So $\pi(\lambda) = e^{-\lambda}.$
...
\begin{eg}
  ...
  \begin{align*}
    \Lambda_\mathbf{x}(H_0; H_1) &= \frac{(2\pi\sigma_0^2)^{-n/2}\exp\left(-\frac{1}{2\sigma^2_0}\sum(x_i - \mu_1)^2\right)}{(2\pi\sigma_0^2)^{-n/2}\exp\left(-\frac{1}{2\sigma^2_0}\sum(x_i - \mu_1)^2\right)}\\
