...
\begin{eg}
  ...
  \]
  So the primal and dual have the same optimal value.
\end{eg}
...
\begin{center}
  \emph{max}imize $c^T x$ subject to
  \begin{align*}
    Ax &= b\\
    x &\geq 0
  \end{align*}
...
\begin{proof}
  ...
  If $y$ or $z$ has strictly fewer than $r$ non-zero entries, then we are done. Otherwise, let 
  \[
...
\[
  \lambda\in Y = \{\lambda: c^T - \lambda^TA \geq 0, \lambda^T \geq 0\}.
\]
For $\lambda\in Y$, the minimum of $L(x, z, \lambda)$ is attained when $(c^T - \lambda^T A)x$ and $\lambda^T z = 0$ by complementary slackness. So
\[
...
\begin{proof}
  ...
  \]
  And by weak duality, $x$ and $\lambda$ are optimal.
\end{proof}
...
In this case, for any feasible solution $x\in \R^n$, we must have $x_N \geq 0$. So $(c_N^T - c_B^TA_B^{-1}A_N)x_N \leq 0$ and
\[
  f(x) \leq c_B^T A_B^{-1}b.
\]
So if we choose $x_B = A_B^{-1}b$, $x_N = 0$, then we have an optimal solution.

What if $(c_N^T - c_B^T A_B^{-1}A_N) \not\leq 0$? Say $(c_N^T - c_B^T A_B^{-1}A_N)_i > 0$.

We can increase the value of the objective function by increasing $(x_N)_i$. As we increase $(x_N)_i$, we have to satisfy the functional constraints. We can keep increasing $(x_N)_i$ until another variable hits $0$, say $(x_B)_j$. Then we will have to stop.
...
We mostly focus on games with two players.

\subsection{Games and Solutions}
\begin{defi}[Bimatrix game]
  A two-player game, or \emph{bimatrix game}, is given by $P, Q\in \R^{m\times n}$. Player 1, or the \emph{row player}, chooses a row $i\in \{1, \cdots, m\}$, while player 2, the \emph{column player}, chooses a column $j\in \{1, \cdots, n\}$. These happen simultaneously. Players then get payoffs $P_{ij}$ and $Q_{ij}$ respectively.
\end{defi}
...
\begin{defi}[Strategy]
  ...
  \[
    Y = \{y\in \R^m, y \geq 0, \sum y_i = 1\}
  \]
  Each vector corresponds to the probabilities of selecting each row or column.

  The strategy profile $(x, y)\in X\times Y$ induces a lottery, and we write $p(x, y) = x^T Py$ for the expected payoff of the row player.

  If $x_i = 1$ for some $i$, we call $x$ a \emph{pure strategy}.
\end{defi}

\begin{eg}[Prisoner's dilemma]
  Each player can either choose $S$ or $T$, with payoffs
  \begin{center}
    \begin{tabular}{ccc}
      \toprule
      & $S$ & $T$\\
  ...
  This comes from the following hypothetical scenario: 

  Alice and Bob commit a crime together, and are caught by the police. They can choose to remain silent ($S$) or testify ($T$). If both keep silent, the police has little evidence and they go to jail for a short period of time, hence the $(2, 2)$ payoff. If one testifies and one remains silent, the one who testifies gets awarded and is freed (payoff $3$), while the other gets stuck in jail for a long time (payoff $0$). If both testify, then they both go to jail and the payoffs are $(1, 1)$.

  The problem is that regardless of what the other person does, it is always strictly better to testify than not. We say $T$ is a \emph{dominant strategy}, and $(1, 1)$ is \emph{Pareto dominated} by $(2, 2)$.
\end{eg}

\begin{eg}[Chicken]\leavevmode
  \begin{center}
    \begin{tabular}{ccc}
      \toprule
      & $C$ & $D$ \\
    ...
    \end{tabular}
  \end{center}
  The story is this: Two people drive a car towards each other. They can choose to continue driving, or chicken out and swerve away. If both don't chicken, they die, and the payoffs are both $(0, 0)$. If one chickens and the other doesn't, the person who chickens has a low payoff $1$ (but better than dying) while the person who didn't has a high payoff $3$. If both chicken out, their payoffs are in the middle, $(2, 2)$, since they don't die, but both are chickens.
  ...
  \begin{center}
    ...
    \end{align*}
  \end{center}
  It should be clear that the maximin strategy is to chicken. However, this isn't really what we are looking for, since if both players employ this maximin strategy, it would be better for you to chicken out.
\end{eg}
...
\begin{thm}[Nash, 1961]
  Every bimatrix game has an equilibrium game.
\end{thm}
...
\begin{thm}[von Neumann, 1928]
  ...
  \]
  The left hand side is the worst payoff the row player can get if he employs the maximin strategy. The right hand side is the worst payoff the column player can get if he uses his maximin strategy.
  
  The theorem states that the two both sides will end up with the same maximin strategy, and this will form an equilibrium.
\end{thm}
...
\begin{proof}
  ...
  \[
    \left(1 - \sum_{j = 1}^n y_j\right)v + \sum_{i = 1}^m \left(\sum_{j = 1}^n p_{ij}y_j - w\right)x_i- \sum_{j = 1}^n w_j z_j + w.
  \]
  The dual is therefore
  \begin{center}
    minimize $w$ subject to
    \begin{align*}
      \sum_{j = 1}^m p_{ij}y_j &\leq w\quad\text{ for all }i\\
      \sum_{y_j} &= 1
    \end{align*}
  \end{center}
...
\begin{defi}[Directed graph/network]
  A \emph{directed graph} or \emph{network} is a pair $G = (V, E)$, where $V$ is the set of vertices and $E\subseteq V\times V$ is the set of edges.
\end{defi}
...
$c_{ij}$ is the cost of transferring one unit of stuff from vertex $i$ to vertex $j$ (fill entries with $0$ if there is no edge between the vertices), and $\underline{m}_{ij}$ and $\overline{m}_{ij}$ denote the lower and upper bounds on the amount of flow along $(i, j)\in E$.

$x\in \R^{n\times n}$ is a minimum-cost flow if it minimizes the cost of transferring stuff, ie. it is an optimal solution to the problem
\begin{center}
...
Note that for the system to be closed, we must have
\[
  \sum_{i\in V}b_i = 0.
\]
...
\begin{proof}
  ...
  We now claim that there is a one-to-one correspondence between flows in the original problem and the new problem, and they have equal costs.

  For any flow $X$, the flow on $(ij, j)$ is $x_{ij}$ and the flow on $(ij, i)$ is $\overline{m}_{ij} - x_{ij}$.  The total flow into $i$ is then
  \[
  ...
  \]
  which is exactly the flow conservation constraint for the node $i$ in the original minimal-cost flow problem.
\end{proof}
...
\begin{eg}
  ...
  \begin{center}
    \begin{tabular}{c|cc|cc|cc|cc|c}
      ...
      & 6 & \bb{5} & 2 & \bb{3} &   & \bb{4} &   & \bb{6} & 8\\\hline
      &   &        &   &        &   &        &   &        & \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      &   & \bb{2} & 3 & \bb{7} & 7 & \bb{4} &   & \bb{1} & 10\\\hline
      &   &        &   &        &   &        &   &        & \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      &   & \bb{5} &   & \bb{6} & 1 & \bb{2} & 8 & \bb{4} & 9\\\hline
      & \bbb{6}   & \bbb{5} &\bbb{8} & \bbb{8} &
    \end{tabular}
  \end{center}
  \begin{center}
    \begin{tikzpicture}
      ...
      \node at (1, 0) [circ] {};
      \node at (1, 0) [right] {$d_1 = 6$};
      \node at (1, -1) [circ] {};
      \node at (1, -1) [right] {$d_2 = 5$};
      \node at (1, -2) [circ] {};
      \node at (1, -2) [right] {$d_3 = 8$};
      \node at (1, -3) [circ] {};
      \node at (1, -3) [right] {$d_4 = 8$};

      \draw [->] (0, 0) -- (1, 0) node [pos=0.5, above] {\tiny 6};
      \draw [->] (0, 0) -- (1, -1) node [pos=0.5, above] {\tiny 2};
      \draw [->] (0, -1) -- (1, -1) node [pos=0.5, above] {\tiny 3};
      \draw [->] (0, -1) -- (1, -2) node [pos=0.5, above] {\tiny 7};
      \draw [->] (0, -2) -- (1, -2) node [pos=0.5, above] {\tiny 1};
      \draw [->] (0, -2) -- (1, -3) node [pos=0.5, above] {\tiny 8};
    \end{tikzpicture}
  \end{center}
  % insert diagram
  ...
  \]
  for edges in the spanning tree, $x_{ij} \not= 0$. So $c_{ij} - \lambda_i + \mu_i = 0$. Hence we must have $\mu_1 = - - \lambda_i + \mu_i = 0$. Hence we must have $\mu_1 = -5$. We can fill in the values of the other Lagrange multipliers as follows, and obtain

  \begin{center}
    \begin{tabular}{c|cc|cc|cc|cc|c}
      & \bbb{-5} & \bbb{-3} & \bbb{0} & \bbb{-2}\\\hline
      &   &        &   &        &   &        &   &        & \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      0 & 6 & \bb{5} & 2 & \bb{3} &   & \bb{4} &   & \bb{6} & 8\\\hline
      &   &        &   &        &   &        &   &        & \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      4 &   & \bb{2} & 3 & \bb{7} & 7 & \bb{4} &   & \bb{1} & 10\\\hline
      &   &        &   &        &   &        &   &        & \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      2 &   & \bb{5} &   & \bb{6} & 1 & \bb{2} & 8 & \bb{4} & 9\\\hline
      & \bbb{6}   & \bbb{5} &\bbb{8} & \bbb{8} &
    \end{tabular}
  \end{center}
  ...
  What we do is add an edge, say from 10 to the second 8. Then we have created a cycle. We keep increasing the flow on the new edge. This causes the values on other edges to change by flow conservation. So we keep doing this until some other edge reaches zero.
  ...
  \begin{center}
    \begin{tabular}{c|cc|cc|cc|cc|c}
      &  &  &  & &&&&\\\hline
      &   &        &   &     &&&   &  &  \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      & $6 - \delta$ & \bb{5} & $2 + \delta$ & \bb{3} &   & \bb{4} &   & \bb{6} & 8\\\hline
      &        &   &        &   & &&       &      & \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      & $\delta$ & \bb{2} & $3 - \delta$ & \bb{7} & 7 & \bb{4} &   & \bb{1} & 10\\\hline
      &  &   &   &        &   &    &    & \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      &   & \bb{5} &   & \bb{6} & 1 & \bb{2} & 8 & \bb{4} & 9\\\hline
      & \bbb{6}   & \bbb{5} &\bbb{8} & \bbb{8} &
    \end{tabular}
  ...
  \begin{center}
    \begin{tabular}{c|cc|cc|cc|cc|c}
      & \bbb{-5} & \bbb{-3} & \bbb{-7} & \bbb{-9} &\\\hline
      &      &   &     &    & \bbb7   &  \bbb9  \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      0 & 3 & \bb{5} & 5 & \bb{3} &   & \bb{4} &   & \bb{6} & 8\\\hline
      &   &        & \bbb0      &   &        &  \bbb9 \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      -3 & 3 & \bb{2} &  & \bb{7} & 7 & \bb{4} &   & \bb{1} & 10\\\hline
      & \bbb0       & \bbb{-2}        &    &    & \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      -5 &   & \bb{5} &   & \bb{6} & 1 & \bb{2} & 8 & \bb{4} & 9\\\hline
      & \bbb{6}   & \bbb{5} &\bbb{8} & \bbb{8} &
    \end{tabular}
  \end{center}

  Note that we have to re-compute the Lagrange multipliers. We see a violation at the bottom right. So we do it again:
  \begin{center}
    \begin{tabular}{c|cc|cc|cc|cc|c}
      & & & & & & & & &\\\hline
      &      &   &     &    & &   &  &  \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      & 3 & \bb{5} & 5 & \bb{3} &   & \bb{4} &   & \bb{6} & 8\\\hline
      &   &        & &      &   &        &  & \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      & 3 & \bb{2} &  & \bb{7} & $7 - \delta$ & \bb{4} & $\delta$ & \bb{1} & 10\\\hline
      &        & &&        &    &    & \\\cline{3-3}\cline{5-5}\cline{7-7}\cline{9-9}
      &   & \bb{5} &   & \bb{6} & $1 + \delta$ & \bb{2} & $8 - \delta$ & \bb{4} & 9\\\hline
      & \bbb{6}   & \bbb{5} &\bbb{8} & \bbb{8} &
    \end{tabular}
  \end{center}
...
We can turn this into a minimum-cost flow problem. We add an edge from $n$ to $1$ with $-1$ cost and infinite capacity. Then the minimal cost flow will cause as much stuff to flow from $n$ to $1$ as possible. So the same amount of stuff will have to flow from $1$ to $n$ through the network.
...
\begin{eg}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  We can keep adding flow until we reach (red is flow, black is capacity)
  \begin{center}
    \begin{tikzpicture}[xscale=2]
      \node at (0, 0) [circ] {-};
      \node at (3, 0) [circ] {-};
    ...
    \end{tikzpicture}
  \end{center}
  We know this is an optimum, since our total flow is $6$, and we can draw a cut with capacity 6:
  \begin{center}
    \begin{tikzpicture}[xscale=2]
      \node at (0, 0) [circ] {-};
      \node at (3, 0) [circ] {-};
