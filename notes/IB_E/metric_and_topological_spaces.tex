...
\begin{eg}
  Let $X = C[0, 1]$ with the topology induced by $\|\cdot \|_1$ and $Y = C[0, 1]$ with the topology induced by $\|\cdot\|_\infty$. Then $F: Y\to X$ by $f\mapsto f$ is continuous buy $F^{-1}$ is not.
\end{eg}
...
\begin{defi}[Hausdorff space]
  A topological space $X$ is \emph{Hausdorff} if for all $x_1, x_2\in X$ with $x_1 \not= x_2$, there exists open neighbourhoods $U_i$ of $x_i$ such that $U_1 \cap U_2 = \emptyset$.
\end{defi}
...
\begin{defi}[Subspace topology]
  If $X$ is a topological space and $Y\subseteq X$, the \emph{subspace topology} on $Y$ is given by: $V$ is an open subset of $Y$ if there is some $U$ such that $V = Y\cap U$.
\end{defi}
...
\begin{defi}[Basis]
  Let $\cU$ be a topology on $X$. A subset $\mathcal{B} \subseteq \cU$ is a \emph{basis} for every $U\in \cU$ is a union of sets in $\mathcal{B}$.
\end{defi}
...
\begin{eg}\leavevmode
  \begin{itemize}
    ...
    \item If $A\subseteq \{(r, z): r > 0\} \subseteq \R^2$, $R(A)$ is the set obtained rotating $A$ around the $z$ axis, then $R(A) \simeq S\times A$ by 
      \[
...
\end{cor}
\note if $X$ is disconnected, then $\exists$ a continuous surjective $f: X\mapsto \{0, 1\}$. Then $g(x) = f(x) - \frac{1}{2}$. Then $g$ does not satisfy the intermediate value theorem. So the intermediate value theorem holds for $X$ if and only if $X$ is connected.
...
\begin{eg}\leavevmode
$[0, 1] \not\simeq \R$. So $[0, 1] \simeq (0, 1)\simeq \R$.
...
\begin{eg}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw [blue, domain=0.01:3, samples=300] plot (\x, {2.5 * exp(-\x) * sin(1 / \x r)});
    \end{tikzpicture}
  \end{center}
...
\begin{proof}
  ...
  So $A, U\setminus A$ are disjoint open subsetes of $U$. Since $U$ is connected, we must have $U\setminus A$ empty (since $A$ is not). So $U = A$ is path-connected.
\end{proof}
...
\begin{eg}\leavevmode
  \begin{enumerate}
    ...
    \item Let $X = \R$ and $\mathcal{V} = \{(-R, R) : R\in \R, R > 0\}$. Then this is an open cover with no finite subcover. So $\R$ is not compact. Hence all open intervals are not compact.
...
\end{prop}
Note that this is a prototype compactness proof. A lot of compactness proofs are like this.

\begin{proof}
  Let $U = X\setminus C$. We want to show that $U$ is open.

  Fix $x\in U$. Since $X$ is Hausdorff, so for each $y\in C$, $\exists U_{xy}$ and $W_{xy}$ that are open neighbourhoods of $x$ and $y$ respectively with $U_{xy}\cap W_{xy} = \emptyset$.

  Then $\mathcal{W} = \{W_{xy}\cap C: y\in C\}$ is an open cover of $C$ (this is the critical step). Since $C$ is compact, there exists a finite subcover $\mathcal{W}' = \{W_{xy_1}\cap C, \cdots, W_{xy_n}\cap C\}$.

  Let $W_x = \bigcup_{i = 1}^nW_{xy_{i}} \supseteq C$ and $U_x = \bigcap_{i = 1}^n U_{xy_i}$. Then $U_x\cap W_x = \emptyset$, and $U_x$ is open, since it is a finite intersection of open set. Hence $U_x \subseteq X \setminus C = U$. So $U = \bigcup_{x\in U}U_x$ is a union of open sets, hence open.
\end{proof}
...
\begin{thm}[Heine-Borel]
  ...
  Conversely, if $C$ is closed and bounded, then $C\subseteq [-N, N]$ for some $N\in \R$. Since $[-N, N] \simeq [0, 1]$ is compact, $C = C\cap [-N, N]$ is closed in $[-N, N]$. So it is compact.
\end{thm}
Note that this is something about $\R$, not compactness. We should not think of compact spaces as ``closed and bounded'' sets.
...
\begin{proof}
  First consider the special type of open cover $\mathcal{V}$ of $X\times Y$ such that every $U\subseteq \mathcal{V}$ has the form $U = V\times W$, where $V\subseteq X$ and $W\subseteq Y$ are open.
