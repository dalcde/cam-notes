...
\author{Lectured by R. Weber\\\small Notes taken by Dexter Chua}
\date{Lent 2015}
...
\begin{defi}[Probability space]
  A \emph{probability space} is a triple $(\Omega, \F, P)$. $\Omega$ is the \emph{sample space}, $\F$ is a collection of subsets of $\Omega$, and $P: \F\to [0, 1]$ is the \emph{probability measure}.
  
  $\F$ has to satisfy the following axioms:
  \begin{enumerate}
    \item $\emptyset, \Omega\in \F$.
    \item $A\in \F \Rightarrow  A^C\in \F$.
    \item $A_1, A_2, \cdots, \in \F \Rightarrow \bigcup_{i = 1}^\infty A_i \in \F$.
  \end{enumerate}
...
\begin{thm}
  Suppose $\E[X_1] = \sum k p_k = \mu$ and $\var (X_1) = \E[(X - \mu)^2] = \sum (k - \mu)^2 p_k < \infty$.
