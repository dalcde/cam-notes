...
\begin{align*}
  P\left(\bigcup_{1}^n A_i\right) &\geq/\leq - \sum_{i_1 < i_2} P(A_{i_1}A_{i_2}) + \sum_{i_1 < i_2 < i_3} P(A_{i_1}A_{i_2}A_{i_3}) + \cdots\\
...
Branching processes are used to model population growth by reproduction. Consider $X_0, X_1, \cdots$, where $X_n$ is the number of individuals in a generation. We assume the following:
\begin{enumerate}
  \item $X_0 = 1$
...
\end{enumerate}
\subsection{Generating function of a branching process}.
Let $F(z)$ be the pgf of $Y_i^n$. Then
\[
...
\begin{thm}[mean and variance of population size]
...
\end{align*}
Of course, we can also obtain this using the probability generating functions.
\end{proof}
...
\[
  F(q) = F\left(\lim_{n\to \infty} F_n(0)\right) = \lim_{n \to \infty} \lim_{n\to \infty} F(F_n(0)) = \lim_{n\to \infty}F_{n + 1}(0) = q.
\]
So
\[
  F(q) = q.
...
\begin{proof}
  ...
  To show that $q = 1$ when $\mu \leq 1$, we show that $q = 1$ is the only root. We know that $F'(z), F''(z) \geq 0$ for $z\in (0, 1)$ (proof: write it out again!). So $F$ is increasing and convex. Since $F'(1) = \mu \leq 1$, it must look like this:
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw [dashed] (0, 0) -- (4, 4);
      \draw (0, 2.3) .. controls (2, 2.5) .. (4, 4);
    \end{tikzpicture}
  \end{center}
  So $z = 1$ is the only root.
\end{proof}

\section{Random walk and gambler's ruin}
\subsection{Random walks}
\begin{defi}[Random walk]
  Let $X_1, \cdots, X_n$ be iid random variables such that $X_n = +1$ with probability $p$, and $-1$ with probability $1 - p$. Let $S_n = S_0 + X_1 + \cdots + X_n$. wlog, $S_0 = 0$. Then $(S_0, S_1, \cdots, S_n)$ is a \emph{1-dimensional random walk}.
...
\[
  C = \frac{1}{q - p},
\]
for $p \not = q$. The we find the complementary solution:
\[
  pt^2 - t + q = 0,
\]
...
\[
  D_2 = z(a - z).
\]
\subsection{Use of generating functions in random variables}
...
\]
We see that $U_z(1) = q_z$. We can apply the same method to find the generating function for absorption at $a$, say $V_z(s)$. Then the generating function for the duration is $U + Z$. Hence the expected duration $D_z = U'_z(1) + V'_z(1).$
...
\[
  \P(a \leq x \leq b) = F(b) - F(a) = \int_a^b f(x)\;\d x.
\]
...
Then
\[
  \P(a \leq x \leq b) = \int_a^b f(z) \;\d z = e^{-\lambda } - e^{-\lambda b}.
\]
\begin{prop}
...
Then
\[
  \P(x \leq X \leq x + \delta x | X \geq x) = \frac{\P(x \leq X \leq x + \delta x}{\P(X \geq x)} = \frac{\d x f(x)}{1 - F(x)} = \delta x\cdot h(x).
\]
In the case of exponential distribution, $h(x)$ is constant.
...
\begin{eg}
  ...
  \begin{align*}
    ...
    &= (1 - e^{-y}).
  \end{align*}
  But this is the cumulative distribution function of $\mathbf{E}(1)$. So $Y$ is exponentially distributed with $\lambda = 1$.
\end{eg}
...
\]
where the first equality holds because in both cases, we have $x$ copies of $\P(X = x)$ in the sum.

\begin{proof}
  \begin{align*}
    \int_0^\infty \P(X \geq x)\;\d x &= \int_0^\infty \int_x^\infty f(y)\;\d y\;\d x\\
    &= \int_0^\infty \int_0^\infty I[y\geq x]f(y)\;\d y\;\d x\\
    &= \int_0^\infty \int_0^\infty I[y \geq x]\;\d x f(y)\;\d y\\
    &= \int_0^\infty \int_0^y \;\d xf(y)\;\d y\\
...
\begin{eg}
  Suppose $X\sim \mathbf{E}(\lambda)$. Then 
  \[
...
\begin{eg}
  ...
  \end{align*}
\end{eg}
\subsection{Stochastic ordering of random variables}
...
\]
We can also order things by hazard rate. Recall that
\[
...
\begin{defi}[Jointly distributed random variables]
  We say $X_1, \cdots, X_n$ are \emph{jointly distributed continuous random variables} and have \emph{joint pdf} $f$ i for any set $A\subseteq \R^n$ if
  \[
    \P((X_1, \cdots, X_n)\in A) = \int_{(x_1, \cdots x_n)\in A} f(x_1, \cdots, x_n)\;\d x_1\cdots \d x_n.
  \]
  where
  \[
    f(x_11, \cdots, x_n) \geq 0
  \]
  and
  \[
...
\begin{defi}[Independent continuous random vairables]
  ...
  If we let $F_{X_i}$ and $f_{X_i}$ are the cdf, pdf of $X$, then
  \[
...
\begin{eg}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  where $A$ is the permitted region. The total area of the white part is simple the area of a square with length $L - \ell$. So the area of $A$ is $L^2 - (L - \ell)^2 = 2L\ell - \ell^2$. So the desired probability is
  \[
...
\begin{defi}[Normal distribution]
  ...
  \[
    f(x) = \frac{1}{\sqrt{2 pi}\sigma}e^{-\frac{(x - \mu)^2}{2\sigma2}},
  \]
  for $-\infty < x < \infty$.
...
\begin{prop}
  \[
    \int_{-\infty}^\infty \frac{1}{\sqrt{2\pi \sigma^2}} e^{-\frac{1}{2\sigma^2}(x - \mu)^2}\;\d x.
  \]
\end{prop}
\begin{proof}
...
\begin{eg}
  ...
  We have $X\sim N(70, 3^2)$ and $Y\sim N(71, 3^2)$. Then (cf. later lectures) $Y - X \sim (1, 18)$.
  \[
...
\[
  nf(y)F(y)^{n - 1} = \frac{\d }{\d y}F(y)^ = nf(y)F(y)^{n - 1}.
\]
Also,
\[
...
It turns out that
\[
  f(y_1, \cdots, y_n) = n! f(y_1)f(y_2)\cdots f(y_n).
\]
...
\begin{eg}
  ...
  If we define $U = X/Y$, $V = XY$. We will later show that they are not independent.
...
\begin{eg}
  ...
  So
  \[
    g_Y(y) = \int_{-\infty}^\infty f(y - z), z)\;\d z = \int_{-\infty} f(z, y - z)\;\d z.
  \]
...
\begin{eg}
  Suppose $X_1, \cdots, X_n$ are iid random variables. Then what is the joint pdf, say $g$, of the order statistics $Y_1,\cdots, Y_1$?
...
\begin{eg}
  Let $X_1, \cdots, X_n$ be iid $\mathcal{E}(\lambda)$, and $Y_1, \cdots, Y_n$ is the order statistic. Let
  \begin{align*}
    Z_1 &= Y_1\\
    Z_2 &= Y_2 - Y_1\\
    &\vdots\\
    Z_n &= Y_n - Y_{n - 1}.
  \end{align*}
  We can write this as a $\mathbf{Z} = A\mathbf{Y}$, with
  \[
    A =
    \begin{pmatrix}
      1 & 0 & 0 & \cdots&  0\\
...
\[
  \int_{-\infty}^\infty \frac{1}{\pm(1 + x^2)}\;\d x = \int_{-\pi/2}^{\pi/2}\frac{1}{\pi}\;\d \theta = 1
\]
...
\begin{proof}
  \[
    \E[X] = \int_0^\infty \frac{x}{\pi(1 + x^2)}\;\d x + \int_{-\infty}^\infty \frac{x}{\pi(1 + x^2}\;\d x = \infty - \infty
  \]
...
\begin{align*}
  f(z) &= \int_{-\infty}^\infty f(_X(x)f_Y(z - x)\;\d x\\
...
\begin{eg}
  Suppose $X_1, \cdots, Xn$ are iid $\mathcal{E}(\lambda)$. Let $S_n = X_1 + \cdots + X_n$. Then the mgf of $S_n$ is
  \[
    \E[e^{\theta(X_1 + \ldots + X_n)} = \E[e^{\theta X_1}]\cdots \E[e^{\theta X_n}] = (\E[e^{\theta X}])^n = \left(\frac{\lambda}{\lambda - \theta}\right)^n.
  \]
\end{eg}
...
\begin{align*}
  ...
  \intertext{The integral is just integrating over $\Gamma(n, \lambda - \theta)$, which gives one. So we have}
...
Substitute $z = \frac{x - \mu}{\sigma}$. Then
\begin{align*}
  \E[e^{\theta X}] &= \int_{-\infty}^\infty \frac{1}{2\pi}e^{\theta(\mu + \sigma z)} e^{-\frac{1}{2}z^2}\;\d z\\
  &= e^{\theta\mu + \frac{1}{2}\theta^2\sigma^2}\int_{-\infty}^\infty \underbrace{\frac{1}{2\pi}e^{-\frac{1}{2}(z - \theta \sigma)^2}}_{\text{pdf of }N(\sigma\theta, 1)}\;\d z\\
...
\begin{thm}[Central limit theorem]
  Let $X_1, \cdots, X_n$ be iid random variables with $\E[X_i] = \mu$, $\var(X_i) = \sigma^2 < \infty$. Define
  \[
    S_n = X_1 + \cdots + X_n.
  \]
...
\begin{thm}[Continuity thm]
  If the random variables $X_1, X_2, \cdots$ have mgfs $m_1(\theta), m_2(\theta), \cdots$ and $m_i(\theta) \to m(\theta)$ as $i\to\infty$ for all $\theta$, then $X_i \to_D $ the random variable with mgf $m(\theta)$.
\end{thm}

We now provide a sketch-proof of the central limit theorem:
\begin{proof}
  Let $\mu = 0, \sigma^2 = 1$ (replace $X_i$ with $\frac{X_i - \mu}{\sigma}$).

  Then
  \begin{align*}
    m_{X_i}(\theta) &= \E[e^{\theta X_i}] = 1 + \theta \E[X_i] + \frac{\theta^2}{2!} \E[X_i^2] + \cdots\\
    &=1 + \frac{\theta^2}{2} + \frac{1}{3!}\theta^3 \E[X_i^3] + \cdots
  \end{align*}
  ...
  \begin{align*}
    ...
    &= \left(1 + \frac{1}{2}\theta^2 \frac{1}{n} + \frac{1}{3!}\E[X^3]\frac{1}{n^{3/2}}\right)^{n}\\
    &\to e^{\frac{1}{2}\theta^2}
  \end{align*}
...
\begin{eg}[Estimating $\pi$ with Buffonl's needle]
  ...
  \begin{align*}
    ...
    &= \frac{\pi 2\ell/(\pi L)}{\E[N/n] + (N/n - \E[N]/n}\\
    &= \frac{\pi p}{p + (p' - p)}\\
  ...
  \]
  We want a small variance, and that occurs when $p$ is the largest. Since $p = 2\ell/\pi L$, this is maximized with $\ell = L$. So
  \[
    p = \frac{2}{\pi},
  \]
  and
