...
\begin{center}
  \begin{tikzpicture}
    ...
    \draw (0, 0) -- (2, 1.5) node [fill, draw, circle, inner sep = 0, minimum size=3]{} node [pos = 0.5, anchor = south east] {$\rho$};
...
\begin{eg}
  Take $\mathbf{F}(\mathbf{r}) = (xe^y, z^2, xy)$ and we want find the line integral from $\mathbf{a}=(0, 0, 0)$ to $\mathbf{b}=(1, 1, 1)$.
  \begin{center}
    \begin{tikzpicture}
      \node [draw, circle, fill = black, inner sep = 0, minimum size = 2.5] {};
      \node [left] {$a$};
      \node at (2, 2) [draw, circle, fill = black, inner sep = 0, minimum size = 2.5] {};
      \node at (2, 2) [right] {$b$};
...
\begin{eg}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw (0, 1) circle [x radius = 1.7, y radius = 0.36];
      \draw (1.7, 1) -- (0, 0) -- (2, 0);
...
We first prove Green's theorem from Stokes' theorem.
...
\begin{proof}
  ...
  \]
  and the normal to $C$ is
  \[
    \mathbf{n} = \mathbf{t}\times \mathbf{k} \left(\frac{\d y}{\d s}, - \frac{\d x}{\d s}, 0\right),
  \]
  which points out of the curve.

  Given any $P(x, y)$, $Q(x, y)$, we can consider the vector field
  \[
    \mathbf{F} = (P, Q, 0),
  \]
  So
  ...
  \[
    \int_C \mathbf{F} \cdot \mathbf{r} = \int \mathbf{F}\cdot \mathbf{t}\;\d s = \int_C P\;\d x + Q\;\d y,
  \]
...
\begin{prop}
  Greens theorem $\Leftrightarrow$ 2D Stokes' theorem.
\end{prop}

\begin{proof}
  Now consider $G = (-Q, -P, 0)$. Then
  \[
...
\begin{prop}
  Green's theorem $\Leftrightarrow$ 2D divergence theorem.
\end{prop}

\begin{proof}
  We seek to prove 2d divergence theorem:
  \[
    \int_A\nabla \cdot \mathbf{G}\;\d A = \int_{C = \partial A} \mathbf{G}\cdot \mathbf{n}\;\d s.
  \]
  We consider $\mathbf{G} = G(x, y)\hat{\mathbf{j}}$. This is sufficient because a field $\mathbf{H}$ in the direction $\hat{\mathbf{i}}$ can be treated similarly and then we obtain a 2dd general field by a linear combination.
\end{proof}
\end{document}
