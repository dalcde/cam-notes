...
\begin{prop}
  If (iii) $\nabla\times \mathbf{F}= 0 $, then (ii) $\in_C \mathbf{F}\cdot \d \mathbf{r}$ is independent of $C$.
\end{prop}
...
\]
by divergence. So this states that the rate of change of the quantity $Q$ in $V$ is the flux of the stuff flowing out of the surface. ie $Q$ cannot just disappear but must smoothly flow out.

In particular, if $V$ is the whole universe (ie $\R^3$), then the surface is nil and the equation states that
\[
  \frac{\d Q}{\d t} = 0,
\]
\end{document}
