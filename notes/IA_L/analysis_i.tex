...
\begin{cor}[Mean value theorem]
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node [fill=black, circle, inner sep = 0, minimum size = 2.5] {};
      \node [left] {$f(a)$};
      \node at (3, 2) [fill=black, circle, inner sep = 0, minimum size = 2.5] {};
...
\begin{proof}
  ...
  \]
  Since $\sum a_nz^n$ converges, the terms $a_nz^n$ are counded. So pick $C$ such that
  \[
    |a_nz^n| \leq C
  \]
  for every $n$. Then
...
\begin{defi}[Radius of convergence]
  The \emph{radius of convergece} of a power series $\sum a_nz^n$ is
  \[
    R = \sup\{|z|: \sum a_nz^n\}.
  \]
...
\begin{lemma}
  The radius of convergence of a the power series $\sum a_nz^n$ is
  \[
