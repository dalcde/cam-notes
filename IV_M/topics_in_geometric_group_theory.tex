...
So he made a second conjecture, that if $M$ is a compact manifold with $\pi_1M \cong 1$, then $M \cong S^3$. Note that the condition already implies $H_*(M) \cong H_*(S^3)$ by Hurewicz theorem and Poincar\'e duality. This is known as the Poincar\'e conjecture, which was proved in 2002 by Perelman. % check
...
\begin{eg}
  In the free group $F(S) = \bra S \mid \emptyset\ket$, the elements of $S^*$ correspond to (simplicial) paths in $\Cay_S(F(S))$ starting at $*$. Given a word $w \in S^*$, we can perform a sequence of \term{elementary reductions}, removing every occurrence of $ss^{-1}$ or $s^{-1}s$, until no such blocks are present. The result is a \term{reduced word}. If we are left with the empty word, then $w$ represents the identity. Otherwise, we are not.
\end{eg}
...
\begin{proof}
%  We first show that $\delta_{\mathcal{Q}} \leq \delta_{\mathcal{P}}$. We let
%  \[
%    C = \max_{t \in T} |w_t|_S
%  \]
%  
%  For the other direction, we consider the retraction
%  \[
%    \rho: F(S \coprod T) \to F(S)
%  \]
%  that sends $s \in S \to s$ and $t \mapsto w_t$. if
%  \[
%    w = \prod_{i = 1}^d g_i r_i^{\pm } g_i^{-1},
%  \]
%  with $r_i \in R \cup R'$ and $g_i \in F(S \coprod T)$, then
%  \[
%    \rho(w) = \prod_{i = 1}^d \rho(g_i) \rho(r_i)^{\pm 1} \rho(g_i)^{-1}.
%  \]
%  Given $w \in F(S)$ of word of length $\leq n$, we need to bound $\Area_\mathcal{P}(w)$ by a linear function of $\Area_{\mathcal{Q}}(w)$. Suppose
%  \[
%    w = \prod_{i = 1}^d g_i r_i^{\pm} g_i^{-1}
%  \]
%  realizes the $\Area_{\mathcal{P}}(w)$.
\end{proof}
