...
\begin{thm}[Dedekind's criterion]
...
\begin{proof}(of Dedekind's theorem)
  ...
  \[
    ...
    \end{tikzcd}
  \]
  It is an exercise to show that the kernel of the map from $\Z[\alpha]$ to $\F_p[x]/\varphi_i$ is the ideal in $\Z[\alpha]$ generated by $p$ and $\tilde{\varphi}_i(\alpha)$. % fill in
  ...
  \]
  So they must be the same! (exercise) % fill in
...
\begin{defi}[Covolume and fundamental domain]
