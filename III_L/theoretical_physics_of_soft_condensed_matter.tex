...
The image of $D$ is then a closed loop in $M$. Two mappings $f_1, f_2$, are \term{homotopic} if they can be continuously deformed into each other. Defects lie in the same \term{homotopy class} if maps for all $D$'s enclosing them are homotopic.
...
The \term{fundamental group} $\pi_1(\mathcal{M})$ is the group of all possible such defects when $D$ is a closed loop. Also, $\pi_n(\mathcal{M})$ describes $n$-dimensional enclosing surfaces.
...
We want to think about \term{motile} particles, which are particles that are self-propelled. For example, micro-organisms such as bacteria and algae can move by themselves, and so can synthetic microswimmers. For example, we can make a sphere and coat it with gold and platinum on two sides
\begin{center}
  \begin{tikzpicture}
  ...
  \end{tikzpicture}
\end{center}
We put this in H$_2$O$_2$. These are asymmetric catalysts of the breakdown 2H$_2$O$_2$ $\to$ 2H$_2$O + O$_2$, and the gradient in reaction rates causes motion. In both cases, the motion is propelled by some fuel. The conversion of fuel into motion implies we have entropy production. So we have ``microscopic dynamics''. There is no Newton's laws or Lagrangian mechanics and no time reversal symmetry.

There are consequences here for steady state behaviour:
\begin{enumerate}
...
\end{enumerate}
Hence steady-state fluxes are allowed.
\begin{eg}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \foreach \y in {0.2, 0.4, 0.6, 0.8} {
        \draw (0.9, \y - 0.1) -- (1.1, \y) -- (0.9, \y + 0.1);
      }
      \foreach \y in {1.2, 1.4, 1.6, 1.8} {
        \draw (1.1, \y - 0.1) -- (0.9, \y) -- (1.1, \y + 0.1);
...
Consider first a polar system. Then the order parameter is $\mathbf{p}$. In the simplest case, we have the field is relaxational with $\mathbf{v} = \mathbf{0}$. The hydrodynamic level equation is
\[
...
\]
We an think of this as an effective body force in the Navier--Stokes equation. In situations like
\begin{center}
  \begin{tikzpicture}
    \draw [->] (0, 0) -- (1, 0);
    \draw [->] (0, 0.2) -- (1, 0.4);
  ...
  \end{tikzpicture}
\end{center}
we have a force acting to the right for $\zeta > 0$.
...
\]
This is unchanged by activity or self propulsion. Thus, at lowest order, all the self-propelled motion can do is the active stress term. The leading-order stress is
\[
  \Sigma^A = \zeta Q.
\]
This breaks the free energy structure, because if we take a uniform nematic, then the passive stress vanishes, since it comes from elastic distortions only, but the active stress does not. The reason we expect this is that if the rods like to move on their own, we get local flow fields around each particle. % insert uniform nematic picture.

The effect body force density is
\[
  \mathbf{f} = \nabla \cdot \Sigma^A = \zeta \nabla \cdot Q \sim \zeta \lambda (\nabla \cdot \mathbf{n}) \mathbf{n}.
\]
This is effectively the same as the case of the polar case.

Now suppose we start with something locally uniform.
\begin{center}
  \begin{tikzpicture}
  ...
  \end{tikzpicture}
\end{center}
If this is deformed as follows:
% insert picture

then this is destablizing for $\zeta > 0$. Conversely, bends are destabilized for $\zeta < 0$. Either case, there is a tendency for these things to be destabilized, and a consequence is that active nematics are never stably uniform for large systems. Typically, we get spontaneous flow.

We can say a bit more about the activity parameter $\zeta$. The sign depends on local flow patterns. One possibility is a contractile, with axial in and equatorial out. This has $\zeta > 0$.

 % insert picture
Reversing this gives an equatorial in, axial out system, and is called extensile. This has $\zeta < 0$.
...
If the liquid crystal is active, then we expect the local flows to interact with the shear flow.

Suppose the shear rate is $v_x = y g$. Then the viscous stress is
\[
  \Sigma^\eta = \eta g 
  \begin{pmatrix}
    0 & 1\\
...
Observe that $\Sigma^A$ does not depend on the shear rate. So in the contractile case, the total stress looks like
\begin{center}
  \begin{tikzpicture}
    \draw [->] (-2, 0) -- (2, 0) node [right] {$g$};
    \draw [->] (0, -2) -- (0, 2) node [above] {$\Sigma_{TOT}$};

    \draw [mblue] (-2, -2) -- (0, -1) -- (0, 1) -- (2, 2);
  \end{tikzpicture}
\end{center}
...
\begin{center}
  \begin{tikzpicture}
    ...
    \draw [mblue] (-2, -2) -- (0, 1) -- (0, -1) -- (2, 2); % insert intercepts $\pm g^*$
  \end{tikzpicture}
\end{center}
...
For simplicity, work in two dimensions. We have two simple defects
...
So the defects themselves are like quasi-particles that are themselves active. We see that cntractile rods move in the direction  of the opening, and the extensile goes in the other direction. The outcome of this is self-sustaining ``turbulent'' motion, and defect pairs $\pm \frac{1}{2}$ are formed \emph{locally}. The $-\frac{1}{2}$ stay put and the $+\frac{1}{2}$ ones self-propel. When defect pairs are spontaneously produced, half of them will swim apart. 

% maybe insert some pictures.
