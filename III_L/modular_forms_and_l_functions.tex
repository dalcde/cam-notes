...
This $M_k$ has some more structure --- it is a module for the \emph{Hecke algebra}. This underlies the connection with arithmetic, ie. Galois representations etc.

How might we try to get some extra structure on $M_k$? We might try to see what happens if we let something else in $\GL_2(\R)^+$ act on $f$. Unfortunately, in general, if $f$ is a modular form and $\gamma \in \GL_2(\R)^+$, then $g = f \underset{k}{|} \gamma$ is not a modular form. If given a $\delta\in \Gamma(1)$, then it acts on $g$ by
\[
...
It turns out we need to act in a smarter way, based on the following lemma:
\begin{lemma}
  Suppose $\gamma_1, \cdots, \gamma_n \in \GL_2(\R)^+$ and $\Gamma, \Gamma' \subseteq \GL_2(\R)^+$ are subgroups such that $\Gamma'$ permutes the cosets of $\Gamma \gamma_i$ ($i = 1, \cdots, n$). Let $f: \H \to \C$ be such that
  \[
    f \underset{k}{|} \gamma = f
  \]
  for all $\gamma \in \Gamma$. Then
  \[
    g = \sum_{i = 1}^n f \underset{k}{|}\gamma_i
  \]
  satisfies
  \[
    g \underset{k}{|} \delta = g
  \]
  for all $\delta \in \Gamma'$.
\end{lemma}

\begin{proof}
  Let $\delta \in \Gamma'$. By hypothesis, there exists $\pi \in S_n$ and $\varepsilon_i \in \Gamma$ for $i = 1,\cdots, n$ such that
  \[
    \Gamma \gamma_1 \delta = \Gamma \gamma_{\pi(i)},\quad \gamma_{\pi(i)} = \varepsilon_i \gamma_i \delta.
  \]
  Then
  \[
    g\underset{k}{|} \delta = \sum f\underset{k}{|} \gamma_i \delta = \sum f\underset{k}{|} \varepsilon_i ^{-1} \gamma_{\pi(i)} = \sum f\underset{k}{|} \gamma_{\pi(i)} = g,
  \]
  since $\varepsilon_i^{-1} \in \Gamma$.
\end{proof}
That was, of course, just group theory, and didn't have much to do with modular forms. We do yet a bit more group theory.

Let $G$ be a group, and $\Gamma \subseteq G$ a subgroup. Consider the right coset space $\Gamma \setminus G = \{\Gamma g \mid g \in G\}$, on which $G$ (hence $\Gamma$) acts by right multiplication. Then the stabilizer in $\Gamma$ of $\Gamma g$ is $\Gamma \cap g^{-1} \Gamma g$. Consider the following hypothesis
\begin{center}
  \textbf{Hypothesis (H)}: For all $g \in G$, $(\Gamma: \Gamma \cap g^{-1} \Gamma g) < \infty$
\end{center}
Then $(G, \Gamma)$ satisfies (H) iff $\Gamma \setminus G$ is a union of \emph{finite} $\Gamma$-orbits, by the orbit-stabilizer theorem.
...
From now on, we assume our pair $(G, \Gamma)$ satisfies (H). What do the orbits of $\Gamma g_i$ look like? If $\{\Gamma g_i\}_{1 \leq i \leq n}$ is an orbit, then the union of those things is just
\[
  \coprod_{1 \leq i \leq n} \Gamma g_i = \Gamma g_j \Gamma
\]
for any $j$. This is a \term{double coset}.

In particular, given any $g \in G$, the orbit of $\Gamma g$ has union $\Gamma g \Gamma$.

We can characterize double cosets as orbits of $\Gamma \times \Gamma$ acting on $G$ by
\[
  (\gamma, \delta) \cdot g = \gamma g \delta^{-1}.
\]
So $G$ is the disjoint union of the double cosets of $\Gamma$.

Note that single cosets all have the same size, but this is not necessarily true for double cosets, since we can have $xgy = x' gy'$ for $x, y, x', y' \in \Gamma$ with $x \not= x'$ and $y' \not= y$.

Our aim is to define a ring $\mathcal{H}(G, \Gamma)$ generated by double cosets called the \emph{Hecke algebra}. As an abelian group, it is the free abelian group on symbols $[\Gamma g\Gamma]$ for each double coset $[\Gamma g \Gamma]$.

Multiplication has to be done with some more care. We define first an action of double cosets on $G$-modules. Let $M$ be a $G$-module, ie. an abelian group with a right action of $G$ which is $\Z$-linear. Alternatively, it is a right $\Z G$-module. We let
\[
...
\end{prop}
To prove this, we just have to count the number of times a coset appears on both sides.
...
\begin{proof}
  ...
  \]
\end{proof}
We could have taken this as the definition of the product, but we have to prove that this is independent of the choice of representatives $g_i$ and $h_j$, and of $S$, and that it is associative.

For the rest of the section, we take $G = \GL_2(\Q)^+$ and $\Gamma(1) = \SL_2(\Z)$. We are going to compute the Hecke algebra in this case.
...
\[
  \gamma' = \gamma \delta.
\]
So we have $\deg \gamma' = \pm n$, and if $\det \gamma' = +n$, then $\delta \in \SL_2(\Z) = \Gamma$. This gives a bijection between cosets $\Gamma \gamma$ such that $\gamma \in \Mat_2(\Z)$, $\det \gamma = n$ and subgroups $\Lambda \subseteq \Z^2$ of index $n$.

Now take a subgroup $\Lambda \subseteq \Z^2 = \Z e_1 \oplus \Z e_2$. We let
\[
...
Then
\[
  \{\gamma \in \Mat_2(\Z), \det \gamma = n)\} = \coprod_{\gamma \in \Pi n}\Gamma \gamma.
\]
...
\begin{proof}\leavevmode
  \begin{enumerate}
    \item This is the Smith normal form theorem, or, alternatively, the fact that we can row and column reduce.
  \end{enumerate}
\end{proof}
\printindex
\end{document}
