...
\]
We proved a functional equation for this, which was rather useful. Along with this, it is convenient to consider $2$ other series we obtain from this:\index{$\vartheta_2$}
\[
  \vartheta_2(z) = \sum_{n \in \Z} e^{\pi i (n + 1/2)^2 z} = q^{1/8} \sum_{n \in \Z} q^{n(n + 1)/2} = 2 q^{1/8} \sum_{n \geq 0} q^{n(n + 1)/2}.
\]
We can also consider\index{$\vartheta_4$}
\[
