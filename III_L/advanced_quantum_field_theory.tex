...
\emph{Warning: Do not read this section}
...
Now if we start from a critical point $g_i^*$, then the couplings are independent of scale, so we can take the limit $\Lambda_0 \to \infty$ \emph{after} computing all the correlation functions, and we will obtain finite results, as nothing depended on $\Lambda_0$.
...
\]
The number on the right is non-negative with equality iff $V_i = 0$ for all $i \geq 3$. Hence, for $1$-loop diagrams, we only need to consider vertices with precisely two $\chi$-lines attached. For example, we have a contribution that looks like
\begin{center}
  \makecenter{\begin{tikzpicture}
...
\]
If we make the temporary (drastic) simplifying assumption that $\phi$ is \emph{constant}, then in momentum space, this action becomes
\begin{align*}
...
\end{align*}
The $\chi$ path integral is finite if we work on a compact space, say $T^d$ with side length $L$, as there are only finitely many Fourier modes. Then the momenta are $p_\mu = \frac{2\pi}{L} n_\mu$, and the Gaussian path integral over the $\tilde{\chi}$ modes gives
\[
  e^{-\delta_\Lambda S} = \int \D \chi\; e^{-(S[\phi + \chi] - S[\phi])} = C \left(\frac{\pi}{\Lambda^2 + V''(\phi)}\right)^{N/2},
\]
where $N$ is the number of $\chi$ modes in our shell of radius $\Lambda$ and thickness $\delta \Lambda$. From our previous formula, we see that it is just
\[
...
\]
This diverges as $L \to \infty$! This is infrared divergence, and it can be traced to our simplifying assumption that $\phi$ is everywhere constant. More generally, it is not unreasonable to believe that we have
\[
...
\]
As before, the first term does not relate to quantum corrections. It is just due to us rescaling our normalization factors, and the $2k$th derivative is there to extract the factor of $\phi^{2k}$ in term.
...
\begin{eg}
  ...
  \end{align*}
\end{eg}
Note that the first on the right hand side is just the classical behaviour of the dimensionless couplings. It has nothing to do with the $\chi$ field. The remaining terms are quantum corrections ($\sim \hbar$), and each comes from specific Feynman diagrams. For example,
\[
  \frac{a g_4}{1 + g_2}
\]
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
There is $1$ $\chi$ propagator, and this gives rise to the one $1 + g_2$ factor in the denominator.
...
We see that there is a trivial critical point with $g_{2k}^* = 0$ for all $k \geq 2$, since there are no vertices. This is just the free theory.
...
\]
Thus, the matrix $B_{ij}$ is upper triangular, and hence its eigenvalues are just the diagonal entries, which are
\[
  k(d - 2) - d = 2k - 4,
\]
in four dimensions.

So vertices $\phi^{2k}$ with $k \geq 3$ are \emph{irrelevant}. If we turn them on at some scale, they become negligible as we fall towards the infrared. The mass term $g_2$ is relevant, as we said before, so even a small mass becomes increasingly significant in the infrared.

We might worry that in this regime, our previous treatment using perturbation theory is no longer valid, but we can just check our exact formulation and see that it doesn't break.
...
\]
where we neglected $g_6$ as it is irrelevant. Thus, to this order, we find
\[
  \frac{1}{g_4(\Lambda)} = C - \frac{3}{16 \pi^2} \log \Lambda,
\]
where we used the value of $a$ in four dimensions. Equivalently, we have
\[
  g_4(\Lambda) = \frac{16 \pi^2}{3}\left(\log \left(\frac{\mu}{\Lambda}\right)\right)^{-1}
\]
for some scale $\mu > \Lambda$. We need $\mu > \Lambda$ because we want $g_4 > 0$ if there are no higher order terms. Now notice this was a marginal coupling to leading order, and consequently it doesn't run as some \emph{power} of $\Lambda$. This runs only \emph{logarithmicly} in $\Lambda$, and is marginally irrelevant. As we reduce $\Lambda \to 0$, then we actually get back to free theory. So boring things happen.

On the other hand, if we take $\Lambda \to \infty$, then $\frac{\mu}{\Lambda}$ hits $1$, and we divide by zero. So our perturbation theory breaks!

In the last term, we did $\phi^4$ theory, and because we were doing tree diagrams only, we didn't have problems. But now we see that the $\phi^4$ theory doesn't actually exist!
