...
How about interactions? So far, we have been taking rather unexciting 1-dimensional manifolds as our universe, and there are only two possible choices. If we allow singularities in our manifolds, then we would allow \emph{graphs} instead of just a line and a circle.

Quantum gravity then says we should not only integrate over all possible lengths, but also all possible graphs. It turns out if we allow ourselves to introduce additional factors for each singularity (vertex) of the graph, then we will obtain interactions in our theory!

Feynman's original approach to QFT was from the worldline perspective. To compute correlation functions such as $\bra \Phi(x_1) \cdots \Phi(x_n)\ket$ in $\Phi^4$ theory on $\R^n$, say, we consider all $4$-valent with $n$ external legs with one endpoint at each of the $x_i$. And then we proceed just as in quantum gravity.
...
\[
  \int_{z \in \R^n}\d^n z \int_{[0, \infty)^3} \d T_1\; \d T_2\; \d T_3\; \int_{\mathcal{C}_{T_1}[z, x]} \D x\; e^{-S_{T_1}[x]} \int_{C_{T_2}[z, z]} \D x\; e^{-S_{T_2}[x]} \int_{C_{T_3}[y, z]} \D x\; e^{-S_{T_3}[x]},
\]
where
\[
  S_T[x] = \frac{1}{2} \int_0^T x^2 \;\d t + \frac{m^2}{2}\int_0^T \d t,
\]
thinking of the second term as the ``cosmological constant''. The 1D integrals over $T_i$'s are the ``1d quantum gravity'' part of the path integral (also known as the Schwinger parameters for the graph).
...
In classical field theory, N\"other's theorem relates symmetries to conservation laws. Suppose we had a variation $\delta \phi = \varepsilon f(\phi, \partial \phi)$ of the field. The most common case is when $f(\phi, \partial \phi)$ depends on $\phi$ only locally, in which case we can think of the transformation as being generated by the vector
\[
  V_f = \int_M \d^n x \sqrt{g}\; f(\phi, \partial \phi) \frac{\delta}{\delta \phi(x)}
\]
on the space of fields. If the function $S[\phi]$ is invariant under $V_f$ when $\varepsilon$ is constant, then for general $\varepsilon(x)$, we must have
\[
  \delta S = \int \d^n x\; j^\mu(x) \partial_\mu \varepsilon.
...
\[
  \delta S = - \int \d^n x\; (\partial^\mu j_\mu) \varepsilon(x).
\]
...
\[
  \partial^\mu j_\mu = 0.
\]
Also, we define the \emph{charge} $Q[N]$ associated to an (oriented) co-dimension $1$ hypersurface $N$ as
\[
  Q[N] = \int_N n^\mu j_\mu \d^{n - 1}x,
\]
...
The derivation of N\"other's theorem used the classical equation of motion. But in a quantum theory, the equation of motion no longer holds. We must re-examine what happens in the quantum theory.

Suppose a transformation $\phi \mapsto \phi'$ of the fields has the property that the
\[
  \D \phi e^{-S[\phi]} = \D \phi' e^{-S[\phi']}.
\]
This is the expression that is important in the quantum theory. Of course, if the transformation leaves both $S[\phi]$ and $\D \phi$ invariant, then this requirement is satisfied. In practice, we usually just look for symmetries of the theory that preserves $S[\phi]$, and then try to find a definition of $\D \phi$ that preserves the symmetry.

There are cases where the classical action is invariant, but the path integral measure is not. For example, we might have a $\phi$ that is rotationally invariant. However, when we regularize the path integral measure, we need to pick a lattice to sample the points. When we do this, we typically reduce the symmetry group to a much small er group. There are two possibilities:
\begin{enumerate}
  \item The symmetry could be restored in the classical limit. This typically means there exists a regularized path integral measure manifestly invariant under this symmetry, but just didn't use it. For example, rotational invariance in a lattice theory.
...
\]
since we just renamed our variables. But using the fact that the measure is invariant when $\varepsilon$ is constant, we can again argue that this is
\[
...
\[
  \int \D \phi e^{-S[\phi]} \int_M j^\mu \partial_\mu \varepsilon \;\d^n x = 0.
\]
Integrating by parts, we can write this as
\[
  -\int_M \varepsilon \partial^\mu \bra j_\mu (x)\ket\; \d^n x,
\]
for $\varepsilon$ with compact support. Note that we dropped a normalization factor of $\mathcal{Z}$, because $\mathcal{Z}$ times zero is still zero. So we know that $\bra j_\mu (x)\ket$ is a conserved current, just as we had classically.

One of the main uses of symmetries is to constrain correlation functions. Suppose an operator changes under transformation $\phi \mapsto \phi'$ as
\[
  \mathcal{O}(\phi) \mapsto \mathcal{O}(\phi'),
\]
ie. the only change in the operator comes from its $\phi$-dependence. For example, if $\mathcal{O}$ depends on the derivative of $\phi$ in some particular fixed direction, then $\mathcal{O}$ would transform in a more complicated way. On the other hand, if $\mathcal{O}$ is just a scalar function of the value of the field, then we are safe.

Now suppose the transformation preserves
\[
  \D \phi e^{-S[\phi]},
\]
then we have
...
\]
Using the invariance, this is equal to
\[
  \frac{1}{\mathcal{Z}} \D \phi e^{-S[\phi]} \mathcal{O}_1(\phi'(x_1) \cdots \mathcal{O}_n (\phi'(x_n)).
\]
...
\begin{eg}
  ...
  In terms of Feynman graphs, we note that the propagators join up a $\phi$ and $\bar\phi$ together. So if we don't have equal number of $\phi$ and $\bar\phi$, then we can't draw any Feynman diagrams at all! So the correlator must vanish.
\end{eg}
...
\begin{eg}
  ...
  \]
  The scalar operators transform as $\mathcal{O}(\phi(x)) \mapsto \mathcal{O}(\phi(x - a))$. In shorthand, $\mathcal{O}(x) \mapsto \mathcal{O}(x - a)$.

  In this case, we have
  \[
    \bra \mathcal{O}_1(x_1) \cdots \mathcal{O}_n(x_n)\ket = \bra \mathcal{O}_1(x_1 - a) \cdots \mathcal{O}_n(x_n - a)\bket.
  \]
...
\begin{align*}
  ...
  &\hphantom{= \int \D \phi e^{-S[\phi]} \left(1 - \int j^\mu (x) \partial_\mu \varepsilon(x) \;\d x\right)}\left.+ \sum_{i = 1}^n \varepsilon(x_i) \delta \mathcal{O}(x_i) \prod_{j \not= i} \mathcal{O}_j(x_j)\right),
\end{align*}
where
\[
  \delta \mathcal{O}(x_i) = \frac{\partial \mathcal{O}}{\partial \phi} \delta \phi.
\]
Again, the zeroth order piece of the correlation function cancels, and to lowest non-trivial order, we find
\[
  0 = \int \partial^\mu \varepsilon(x) \left\bra j_\mu(x) \prod_{i = 1}^n \mathcal{O}_i(x_i)\right\ket\;\d^d x - \sum_{i = 1}^m \varepsilon(x_i) \left\bra \delta \mathcal{O}_i(x_i) \prod_{j \not= i} \mathcal{O}_j(x_j)\right\ket.
\]
...
What we need is a little trick. We rearrange, and then integrate by parts to obtain
\begin{multline*}
...
\[
  \partial^\mu \left\bra j_\mu (x) \prod \mathcal{O}_i(x_i)\right\ket = -\sum_{i = 1}^n \left\bra \delta \mathcal{O}_i(x_i) \prod_{j \not= i} \mathcal{O}_j(x_j)\right\ket.
\]
...
\]
is divergence free except at the other insertions.
...
\]
In other words, the correlation function function must be invariant under the transformation. This is just the infinitesimal version of what we've seen already.
...
\begin{align*}
  ...
  \intertext{And by the Ward identity, this is equal to}
  &= - \sum_{i \in I} \left\bra \delta \mathcal{O}_i(x_i) \prod_{j \not= i}^m \mathcal{O}_j(x_j)\right\ket.
\end{align*}
In particular, if $M'$ contains only one point, and we choose the region to be infinitesimally thin, then in the canonical picture, we have
\[
...
\]
which is invariant under the global transformations
\[
  \psi (x) \mapsto e^{i\alpha} \psi(x),\quad A_\mu(x) \mapsto A_\mu(x)
\]
for $\alpha \in \R/2\pi \Z$. The path integral measure is also invariant under this transformation provided we integrate over equal numbers of $\psi$ and $\bar\psi$ modes in the regularization. To obtain the Ward identity, we do the normal things --- we let $\alpha$ depend on position. So the transformations become
\[
...
Let's look at this in momentum space. We consider the Fourier transform
\[
...
\[
  ...
  + \cdots
\]
where IPI is the sum of all \term{one-particle irreducible graphs}, ie. graphs connected graphs that cannot be disconnected by any single internal line. % insert examples
...
\[
  ...
  \Sigma(\slashed{k})
\]
where $k$ is the momentum following in and out of the end points. This is called the \term{electron self-energy}. For now, we will imagine that we have someone managed to compute this,  and see what we can do. In terms of $\Sigma(\slashed{k})$, the exact propagator is
\begin{align*}
...
We also define the \term{exact electromagnetic vertex} $\Gamma_\mu(k_1, k_2)$ by
\begin{align*}
  ...
  &\hphantom{\int \d^4 x\; \d^4 x_1\; \d^4 x_2\; } e^{ip\cdot (x - x_2)} e^{ik_1 \cdot (x_1 - x_2)} e^{i(p + k_1 - k_2)\cdot x_2}\\
  &= \delta^4(p + k_1 - k_2) S(k_1) \Gamma_\mu(k_1, k_2) S(k_2),
\end{align*}
where $S$ is what we had before, the exact electron propagator. To understand this, note that since
\[
  j_\mu(x) = \bar\psi(x) \gamma_\mu \psi(x),
\]
we have
\[
  \bar \psi(x_1) j_\mu(x) \bar\psi(x_2)\ket
\]
contains diagrams coming from
\[
  \bra \psi(x_1) \bar\psi(x)\ket \gamma_\mu \bra \psi(x) \bar\psi(x_2)\ket
\]
Once we Fourier transform this, we just get $S(k_1) \gamma_\mu S(k_2)$. So we are not really learning anything new about the interaction. These diagrams don't tell us about quantum corrections to the actual photon \emph{vertex}. What other diagrams are there? The remaining diagrams join the two exact electron propagators together, eg.
\begin{center}
  \begin{tikzpicture}
    \draw (0, 0) node [circ] {} node [above] {$\psi(x_1)$} -- (2, 0) node [circ] {} node [above] {$\bar\psi(x_2)$};
    % draw a (x) in the center with j(x) above, and then a photon propagator across two sides.
  \end{tikzpicture} 
\end{center}
...
This is of course important. This identity is an important check of gauge invariance --- the covariant kinetic term $i \bar\psi \slashed D \psi$ should be treated as a whole. We don't get separate quantum corrections for the kinetic part and the interaction part, or else we would have broken gauge invariance.
...
\[
  S_{\Lambda_0} [\phi] = \int \d^d x\; \frac{1}{2}(\partial \phi)^2 + \frac{m^2}{2} \phi^2 + \sum_i g_i \Lambda^{d - d_i}_0 \mathcal{O}_i(\phi_i , \partial\phi),
\]
where $\mathcal{O}(\phi, \partial \phi)$ are monomials in fields and derivatives, and the mass dimension $[\mathcal{O}_i] = d_i$.
...
\[
  [\phi] = \frac{d - 2}{2}.
\]
Thus if $\mathcal{O} = \phi^4$, say, then
\[
  d_{\phi^4} =  [\phi^4] = 2 (d - 2).
\]
On the other hand, if $\mathcal{O} = \phi^2 (\partial\phi)^2$, then
\[
  d_{\phi^2 (\partial \phi)^2} [\phi^2 (\partial \phi)^2] = 2 (d - 2) + 2.
\]
The powers of the cut-off $\Lambda_0$ are a convention chosen so that the couplings $g_i$ are dimensionless.
