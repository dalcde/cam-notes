...
Given a connection $\nabla$, we can extend its action to elements $s_{\mu_1, \ldots, \mu_p}(x) \in \Omega^p_M(E) \to \Omega^{p + 1}_M(E)$ satisfying
\[ % include construction
  \nabla (\alpha_1 s_1 + \alpha_2 s_2) = \alpha_1( \nabla s_1) + \alpha_2 (\nabla s_2),
\]
and the Leibnitz rule for \emph{forms}
\[
  \nabla (\omega \wedge s) + (\d \omega) \wedge s + (-1)^{\deg \omega} \omega \wedge \nabla s.
\]
whenever $\omega \in \Omega^q(M)$ and $s \in \Omega_M^{p - q}(E)$.

$\nabla$ is thus a ``covariant generalization'' of the de Rham operator $\d$. However, where the ordinary $\d$ is nilpotent, ie. $\d^2 = 0$, here this is not necessarily the case.
...
\begin{align*}
  ...
  &= \frac{1}{2} (\partial_\mu A_\nu - \partial_\nu A_\mu + [A_\mu, A_\nu]) \d x^\mu \wedge \d x^\nu
\end{align*}
We have a matrix commutator, which doesn't appear in in the abelian case, because things are abelian.

This is important. This is no longer linear in $A$. When we do Yang--Mills later, we will consider $F^2$, and then we will have $A^3$ and $A^4$ terms, and this causes interaction of the gauge field with itself.
...
\[
  \nabla^3 s = \nabla^2 (\nabla s) = F \wedge \nabla s.
\]
These two ways of thinking about $\nabla^3$ are consistent iff $\nabla (F_\nabla) \equiv 0$. This is known as the \term{Bianchi identity}.
...
Let's consider a simple finite-dimensional example. Suppose we have fields $(x, y): \{\mathrm{pt}\} \to \R$, but suppose the action $S[x, y]$ is rotationally invariant. Then the path integral
\[
  \int_{\R^2} \d x\; \d y\; e^{-S[x, y]} = \int_0^{2\pi} \d \theta \int_0^\infty r \;\d r e^{-S[r]}.
\]
The factor $2\pi$ is here the volume of the gauge group $\U(1) = \SO(2)$, while the radial integral is the integral over the quotient space $(\R^2 \setminus \{0\})/\SO(2) \cong \R_{> 0}$. Note that the measure on this quotient space is $r\;\d r$, and not the trivial measure $\d r$. We have to do something similar in Yang--Mills.
