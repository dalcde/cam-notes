...
Let's consider the theory of two real-valued fields $x, y: S^1 \to \R$ on a circle of circumference $T$, so that we don't have to worry about boundary conditions. Similar to the case of $0$ dimensions, we choose the action
\[
  S[x,  y] = \int_{S^1} \left(\frac{1}{2} \dot{x}^2 + \frac{1}{2} \dot{y}^2 + \frac{1}{2} m^2 x^2 + \frac{1}{2}M^2 y^2 + \frac{\lambda}{4} x^2y^2 \right)\;\d t.
\]
Going to momentum space gives Feynman rules
\begin{center}
  \makecenter{
  \begin{tikzpicture}
  ...
  \end{tikzpicture}} % check momentum
\end{center}
with a vertex
\begin{center}
  \begin{tikzpicture}[scale=0.707]
  ...
  \end{tikzpicture}
\end{center}
If we are only interested in the correlations involving $x(t)$, then we can integrate out the field $y(t)$ first. We have
\[
  \int \D x \exp\left(-\frac{1}{2} \int y\left(-\frac{\d^2}{\d t^2} + M^2 + \frac{\lambda x^2}{2}\right) y  \;\d t\right),
\]
where we integrated by parts to turn $\dot{y}^2$ to $y \ddot{y}$. This is just, formally, a Gaussian integral, and formally, we can write this as
\[
  \det \left(-\frac{\d^2}{\d t^2} + M^2 + \frac{\lambda x^2}{2}\right)^{-1/2}.
\]
$S_{\mathrm{eff}}[x]$ thus contains a term
\begin{align*}
  \S_{\mathrm{eff}}[x] &= \int_{S^1} \frac{1}{2}(\dot{x}^2 + m^2 x^2) \;\d t + \frac{1}{2} \log \det \left(-\frac{\d^2}{\d t^2} + M^2 + \frac{\lambda x^2}{2}\right)\\
  &= \int_{S^1} \frac{1}{2}(\dot{x}^2 + m^2 x^2) \;\d t + \frac{1}{2} \tr \log \left(-\frac{\d^2}{\d t^2} + M^2\right) + \frac{1}{2} \tr \log \left(1 - \lambda \left(\frac{\d^2}{\d t^2} - M^2\right)^{-1} \frac{x^2}{2}\right)
\end{align*}
The second term is field independent, and is going to cancel if we compute normalized correlation functions. The last bit, we need to understand. We can expand the final term using the fact that the Green's function for the operator
\[
  \frac{\d^2}{\d t^2} - M^2
\]
on $S^1$, satisfying
...
is given by
\[
  G(t, t') = \frac{1}{2M} \sum_{n \in \Z} \exp\left(-M |t - t' + \beta k|\right), % check
\]
with
\[
  \beta = \frac{1}{T}.
\]
This is what we mean by the inverse of the operator, and the last term in the effective potential becomes
\[
  \tr \log \left(1 - \lambda G(t, t') \frac{x^2}{2}\right) = - \frac{\lambda}{2} \int_{S_1}G(t, t) x^2(t) \;\d t - \frac{\lambda^2}{8} \int_{S^1 \times S^1} \d t\; \d t' G(t', t) x^2(t) G(t, t') x^2(t') \cdots
\] % explain. Trace means we have $G(t, t)$ term, and integrate over everything.
These terms are \emph{non-local}! This term involves integrating over many different points in $S^1$.

We can also see this non-locality from the corresponding Feynman diagrams. The first term corresponds to
\begin{center}
  \begin{tikzpicture}
    % single loop, dashed loop, label top, bottom x(t)
  \end{tikzpicture}
\end{center}
Here $G(t, t)$ corresponds to the $y$ field.

The second diagram looks like
% again two loop diagram, label left x(t), right x(t')
We see that here we have two vertices, and so we have two factors of $-\lambda$.
...
To make progress, we note that if $M$ is very large, then we would expect $G(t, t')$ could be highly suppressed for $t \not= t'$. So we can try to expand around $t = t'$. We have
\begin{align*}
&\int \d t\; \d t' G(t, t')^2 x^2(t) x'(t^2)\\
&= \int \d t\; \d t' G(t, t')^2 x^2(t) \left(x^2 (t) + 2x(t) \dot{x}(t) (t' - t) + \left(\dot{x}^2(t) + \frac{1}{2} x(t) \dot{x}(t)\right) (t - t')^2 + \cdots\right).
\end{align*}
...
The point is that by dimensional analysis, every extra derivative is accompanied by a further power of $\frac{1}{M}$. Thus, provided $x(t)$ is slowly varying on scales of order $\frac{1}{M}$, we may hope to truncate the series. So at energies $E \ll M$, our theory looks approximately local. So as long as we only use our low-energy approximation to answer low-energy questions, we are fine. However, if we try to take our low-energy theory and try to extrapolate it to higher and higher energies, up to $E \sim M$, it is going to be nonsense. In particular, it becomes non-unitary, and probability is not preserved. % insert waffle about not taking into account higher interactions involving y
...
At high energies, this \emph{4-Fermi theory} becomes non-unitary, and $G_{\mathrm{weak}}$ is revealed as an approximation to a $W$-boson propagator. Instead of
% 4-vertex interaction
what we really have is
% insert $W$-boson interacting
There is many other theories we can write down that has negative mass dimension, the most famous one being general relativity.
...
In $d = 1$, a metric $g$ only has one component $g(t) = e(t)$. There is no curvature. The only diffeomorphism invariant of this metric is the total length
\[
  T = \oint e(t) \;\d t.
\]
...
\begin{align*}
  \int_0^\infty \brak{y} e^{-HT} \bket{x} &= \int_0^\infty \d T \frac{\d^n p\; \d^n q}{ (2\pi)^n} \braket{y}{q} \brak{q} e^{-HT}\bket{p} \braket{p}{x}\\
  &= \int_0^\infty \d T\; \frac{\d^n p\; \d ^n q}{(2\pi^n} e^{ip\cdot x - i q \cdot y} e^{-Tp^2/2} \delta^n(p - q)\\
  &= \int_0^\infty \d T \frac{1}{(2\pi)^n} e^{ip\cdot (x - y)} e^{-Tp^2/2}\;\d^n p\\
  &= 2 \int \frac{\d^n p}{(2\pi)^n}\\
  &= 2 \int \frac{\d^n p}{(2\pi)^n} e^{ip\cdot (x - y)} \frac{1}{p^2}.
\end{align*}
If we included a cosmological constant term
\[
  \frac{m^2}{2} \int_0^T \d t,
\]
then we would have to replace $p^2$ with $p^2 + m^2$, and this is equal to $2D(x, y)$, the propagator for a scalar field on the target space $\R^n$ with action
\[
  S[\Phi] = \int \d^n x \; \frac{1}{2} (\partial \Phi)^2 + \frac{m^2}{2} \Phi^2.
\]
If we integrate over general universes, so that we have graphs instead of just a circle and a line, and this gives us interactions.
