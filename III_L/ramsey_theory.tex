...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.7]
      ...
      \draw [mred] (2, 0) edge [bend left] (5, 1);
      \draw [mred] (5, 1) edge [bend left] (8, 2);
...
\begin{proof}
  ...
  \]
  where the $A_i$'s are the colour-classes of $c|_{[N]}$. By the previous theorem, we are done if $|A_i| \geq \delta N$ for some $1 \leq i \leq k$. So suppose not.
...
\begin{proof}
  ...
  We say $X, Y \subseteq \N$ form an $(r, k)$-structure if they are disjoint; $X$ is red; $Y$ is an arithmetic progression; all $X-Y$ edges are red; and $|X| = r$ and $|Y| = k$. We show by induction that there is an $(r, k)$-structure for each $r$ and $k$.
