...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
We now try to find a (complete) \emph{subgraph} that is monochromatic, ie. a subgraph all of whose edges are the same colour. With a bit of effort, we can find a red monochromatic subgraph of size $4$:
\begin{center}
  \begin{tikzpicture}
...
\begin{proof}{Proof of topological Hindman's theorem (continued)}
  ...
  It now remains to prove the final claim. We first note that $\mathcal{F}$ is non-empty. Indeed, pick any $y \in Y$. Since $X = \bar{x}$, there exists some sequence $T^{n_i}(x) \to y$. Now the maps $(T^{n_i})_{i \geq 0}$ cluster at some $f \in E_T$, ie. every open neighbourhood of $F$ contains infinitely many $T^{n_i}$. Then $f(x) = y$. So $f \in \mathcal{F}$. % check this
