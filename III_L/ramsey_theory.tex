...
\begin{proof}
  ...
  \]
  By dividing out common factors, we may wlog $\mathcal{L} = 0$. Then for all $i \in I$, we have % do not divide out
  \[
    x_i \equiv d \pmod p.
  \]
...
\begin{prop}
  If $A$ is a rational matrix wiith the columns property, then there is some $m, p, c \in \N$ such that $Ax = 0$ has a solution inside any $(m, p, c)$ set, ie. all entries of the solution lie in the $(m, p, c)$ set.
\end{prop}
In the case of a single equation, we reduced the general problem to the case of 3 variables only. Here we are going to do something similar --- we will use the columns property to reduce the solution to sommething much smaller.
...
\begin{proof}
  ...
  Now we try to write down a general solution with $r$ many free varaibles. Given $x_1, \cdots, x_r \in \N^r$, we look at
  \[
...
Where did this whole idea of the $(m, p, c)$-sets come from? The original proof by Rado didn't use $(m, p, c)$-sets, but it tunrs out in many cases, these $(m, p, c)$-sets are the right things to talk about.
...
Let's look at some specific examples. Consider $c$:
\begin{center}
  \begin{tikzpicture}
    \draw (-2.5, 0.5) -- (2.5, 0.5);
    \node [above] at (0, 0.5) {$0$};
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
Then $\bar{c}$ contains all translates of these, and also
\begin{center}
  \begin{tikzpicture}
    \draw (-3.5, 0.5) -- (3.5, 0.5);
    \node [above] at (0, 0.5) {$0$};
...
\begin{proof}
  ...
  \]
  for all $s \in \Z$. This is a contraadiction, since $\bar{x} = X$ by minimality.
\end{proof}
...
\begin{proof}
  ...
  \]
  wne then take $x = T^{s_1} y$ and $n = s_2 - s_1$.
  ...
  Inductinvely, we find $z_s \in X$, $n_s \in \N$ and some $0 < \varepsilon_s \leq \varepsilon_{s - 1}$ such that
  \[
    T^{in_s}(B(z_s, \varepsilon_s)) \subseteq B(z_{s - 1}, \varepsilon_{s - 1})
  \]
  for all $1 \leq i \leq r$.

  By compactness, $(z_s)$ has a convergent subsequence, and in particular, there exists $i< j \in \N$ such taht $\rho(z_i, z_j) < \frac{\varepsilon}{2}$.
...
We now want to prove Hindman's theorem. We will again first prove a ``topological'' version, but unlike the topological van der Waerden theorem, it will look nothing like the actualy Hindman's theorem. So we still need to do some work to deduce the actual version from the topological version.
...
\begin{thm}[Topological Hindman]\index{topological Hindman theorem}
...
\]
We say for intervals $I_1, I_2 \subseteq \Z$, we have $c(I_1) \preccurlyeq c(I_2)$, if $c(I_1)$ is a contiguous subsequence of $c(I_2)$. % contiguous is useless?
...
\end{defi}
For example, every peeriodic colouring has the bounded gaps property.
...
\begin{proof}
  ...
  We apply topoloical Hindman to $(\bar{x}, \mathcal{L})$ to find a minimal colouring $y$ such that $Ex$ and $y$ are proximal. Then either
  \[
  ...
  Sinece $x$ and $y$ are positiely proximal, there exists $I \subseteq \Z$ such that $|I| \geq M$ and $\min(I) > 0$ such that $x(I) = y(I)$. Then we pick $a_1 \in I$ such that $x(a_1) = y(a_1) = y(0)$.
  ...
  \]
  Let $a_{n + 1}$ denote by the position at which $y([0, \cdots, a_1 + \cdots + a_n])$ occurs in $y(I)$. It is then immediate that $x(z) = y(z) = \mathrm{red}$ for all $z \in FS(a_1, \cdots, a_{n + 1})$, as any elemtn in $FS(a_1, \cdots, a_{n + 1})$ is either
  \begin{itemize}
