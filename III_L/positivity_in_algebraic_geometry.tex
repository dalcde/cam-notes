...
\begin{proof}\leavevmode
  \begin{enumerate}
    ...
    \item[$(\Rightarrow)$] If $\Phi$ is not finite, then there exists $C \subseteq X$ such that $\Phi(C)$ is a point. Then $D \cdot C = \Phi^* \mathcal{O}(1) \cdot C = 0$, by the push pull formula. But this is just $\deg_C (\Phi^* \mathcal{O}(1))$. So by Nakai's criterion, $D$ is not ample.
  \end{enumerate}
\end{proof}
  ...
  \begin{itemize}
    ...
    \item[$(\Leftarrow)$] We assume first that $X$ is integral. We proceed by induction on $\dim X$, noting that $\dim X = 1$ is trivial.
      ...
      \begin{claim}
        ...
        \end{enumerate}
      \end{claim}
      If we assume the claim, then we know that $\mathcal{O}_D(D)$ is ample. It is then enoguh to prove that $|mD|$ is globally generated for $m \gg 0$. Then we have a map $X \to \P(|mD|)$, which cannot contract any curve $C$, and so $mD \cdot C = 0$. So this map is finite, and we know that pulling an ample divisor along a finite map is ample.
  \end{itemize}
\end{proof}
\printindex
\end{document}
