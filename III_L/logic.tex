...
Recall (or prove quickly in your head) that if we have a universal theory, ie. a theory all of whose axioms are of the form $(\forall \cdots)(\text{stuff not involving quantifiers})$, then any substructure of a model is a model. It turns out there is a converse --- if a theory is such that every substructure of a model is a model, then it is a universal theory. This is a nice result, because it gives us a correspondence between syntax and semantics.
...
\begin{lemma}
  For any consistent theory $T$ and any model $\mathcal{M}$ of $T_{\forall}$, the set of universal consequences of $T$, the theory $T \cup \mathrm{diagram}(\mathcal{M})$ is consistent.
\end{lemma}

\begin{proof}
  Let $\mathcal{M}$ be a model of $T_{\forall}$. Add to $\mathcal{L}(T)$ names for every element of $\mathcal{M}$. Add to $T$ all the quantifier-free assertions about these constants to be true. We want this theory to be consistent.

  Suppose not. Then there is an inconsistency that can be derived from finitely many of the new axioms. Call this finite conjunction $\psi$. Then we have a proof of $\not \psi$ from $T$. But $T$ knows nothing about these new constants. So we have a universal consequence of $\forall \mathbf{x} \neg \psi$. But this contradicts the fact that $\mathcal{M}$ satisfies all universal consequences of $T$.
\end{proof}
...
\begin{proof}
  ...
  Let $\mathcal{M}$ be an arbitrary model of $T_\forall$. Then $T \cup \mathrm{diagram}(\mathcal{M})$ is consistent. So it must have a model, say $\mathcal{M}^*$, and this is a model of $T$. Moreover, $\mathcal{M}$ is a submodel of $\mathcal{M}^*$. So $\mathcal{M}$ is a model of $T$.
...
\begin{defi}[Product of structures]\index{product of structures}
  Suppose $\{A_i\}_{i \in I}$ is a family of structures for the same signature. Then
  \[
    \prod_{i \in I} A_i
  \]
  has carrier set the set of all functions
  \[
    I \to \bigcup_{i \in I} A_i
  \]
  such that $f(i) \in A_i$. % how to deal with formulae and relations?
\end{defi}
We say a product \emph{preserves} a formulae in $\Gamma$ if
\[
  \prod_{i \in I} A_i \vDash \varphi \Longleftrightarrow \forall i \in I, A_i \vDash \varphi
\]
for all $\varphi \in \Gamma$.

What sort of things do products preserve? As we know from undergraduate mathematics, equational theories such as groups and rings are preserved (equational theories are axiomatized for formulas of the form
\[
  (\forall \mathbf{x}) (w_1(\mathbf{\mathbf{x}}) = w_2(\mathbf{x})),
\]
where $w_1$ and $w_2$ are some terms in $\mathbf{x}$)

It turns out there are other things that are preserved by products, namely \emph{Horn clauses}.
...
\begin{defi}[Horn clause]\index{Horn clause}
  ...
  \[
    \left(\bigwedge_{i < j} \varphi_i\right) \to \varphi_j,
  \]
  where $\varphi_i, \varphi_j$ are atomic. Note that $\bot$ is atomic.
\end{defi}

\begin{eg}
  Transitivity, symmetry, antisymmetry, reflexivity are all universal Horn clauses (ie. a universal quantifier followed by a Horn clause).
\end{eg}
...
\begin{proof}
  ...
  \[
    A_i \vDash (\forall \mathbf{x}) \bigwedge_{i < j} \varphi(\mathbf{x}) \to \varphi_j(\mathbf{x}).
  \]
  ...
  \]
  for each $i < j$, But then
  \[
    A_n \vDash \varphi_j(f_1(n), \cdots, f_k(n))
  \]
  for all $n$. But then the product also believes in $\varphi_j(f_1, \cdots, f_n)$ by definition. So we are done.
\end{proof}

For example the definition of a poset can be written as universal Horn clauses, and so the product of posets is a posets. However, the product of total orders is in general not a total order, as the definition of a total order is not universal Horn.

\subsubsection*{Reduced products}
Reduced products arise from filters on the index set.
\begin{defi}[Filter]\index{filter}
  Let $I$ be a set. A \emph{filter} on $I$ is a (non-empty) subset $F \subseteq P(I)$ if $F$ is closed under intersection and superset.
\end{defi}
...
Filters on $I$ form a complete poset under inclusion, with top element given by $F = P(I)$.

\begin{defi}[Proper filter]\index{proper filter}\index{filter!proper}
  A \emph{proper filter} is a filter $F \not= P(I)$.
\end{defi}

The \emph{proper} filters are a chain complete poset (this has got to do with the fact that the definition of filter is Horn).

By Zorn's lemma, there are \emph{maximal} proper filters.
...
\end{eg}
The is silly, as it is specified by a finite amount of information, namely $17$.
...
\begin{defi}[Principal filter]\index{principal filter}
  ...
  for some $x \in I$.
\end{defi}
We don't care about principal filter. By Zorn's lemma, we can indeed find non-principal maximal filters.
\begin{eg}
  By Zorn's lemma, there is a maximal filter extending the cofinite filter on $\N$, and this is non-principal.
\end{eg}
It turns out we cannot explicitly construct an ultrafilter, but Zorn's lemma says they exist.
...
It is obvious once you start trying to write down a proof, as long as we note the following fact;
\begin{lemma}
...
\begin{defi}[Ultrapower]\index{ultrapower}
  If all the factors in an ultraproduct are the same, then we call it an \emph{ultrapower}.
\end{defi}

Now if the ultrafilter is principal $\{J \subseteq I: i \in J\}$, then the ultraproduct is isomorphic to $A_i$. So we are only interested in non-principal ultrafilters.

Note also that there is an elementary embedding $\iota: \mathcal{M} \to \mathcal{M}^I/\mathcal{U}$ given by sending $m \in \mathcal{M}$ to $[\lambda i. m] \in \mathcal{M}^I/\mathcal{U}$. Here $\lambda i. m$ is the function that always returns $m$, as usual.
...
Recall that by the compactness theorem, if we have a theory $T$ such that every finite subset of $T$ has a model, then the whole theory has a model. Indeed, if every finite subset of $T$ has a model, then every finite subset of $T$ is consistent. Since proofs are finite, we know $T$ is consistent. So by completeness, $T$ has a model.

But wouldn't it be nice if we have a way to go directly from ``every finite subset of $T$ has a model'' to ``$T$ has a model''? Indeed, we can prove this.
...
\begin{proof}
  Let $\Delta$ be such a theory. Let $S = \mathcal{P}_{\aleph_0}(\Delta)$ be the set of all finite subsets of $\Delta$. For $s \in S$, let
  \[
    X_s = \{t \in S: s \subseteq t\}.
  \]
  Pick $\mathcal{M}_s \vDash s$ for all $s \in S$, which exists by assumption. We notice that $\{X_s: s \in S\}$ generate a proper filter on $S$. We extend this to ultrafilter $\mathcal{U}$ by Zorn's lemma. Then we claim that
  \[
...
\begin{defi}[Realization of type]\index{type!realize}\index{realizes type}
...
\begin{thm}[Ehrenfeucht--Mostowski theorem (1956)]\index{Ehrenfeucht--Mostowski theorem}
  Let $\bra I, \leq\ket$ be a total order, and let $T$ be a theory with infinite models. Let $P(\ph) \in \mathcal{L}(T)$ such that
  \[
    T \vdash \text{the extension of $P$ is a total order}.
  \]
  Then $T$ has a model $\mathcal{M}$ in which $I$ is embedded in the extension of $P$, and the copy of $I$ is a set of indiscernibles.

  Indeed, every order-automorphism of $\bra I, \leq \ket$ extends to an automorphism of $\mathcal{M}$.
\end{thm}
...
\begin{proof}
  Let $T$ and $\bra I, \leq \ket$ be as in the statement of the theorem. We add to $\mathcal{L}(T)$ names for every element of $I$, say $\{c_i: i \in I\}$. We add axioms that $c_i \not= c_j$ whenever $i \not= j$. We call this theory $T^*$. By compactness, we know $T^*$ is consistent. Now we add to $T^*$ axioms to say that the $c_i$ form a set of indiscernibles. So we are adding axioms like
  \[
    \varphi(c_i, c_j) \Leftrightarrow \varphi(c_{i'}, c_{j'})
  \]
  for all $i < j$ and $i' < j'$.

  We do this simultaneously for all $\varphi \in \mathcal{L}(T)$ and all tuples of the appropriate length. We call this theory $T^I$, and it will say that $\bra I, \leq\ket$ forms a set of indiscernibles. The next stage is, of course, to prove that $T^I$ is consistent.
\end{proof}
