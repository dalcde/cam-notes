...
\subsection{Computability by \texorpdfstring{$\lambda$}{lambda}-calculus} % this section needs a better rewrite
...
\end{defi}
This captures ``function extensionality'', ie. the idea that functions are the same iff they give the same results for all inputs. % insert something about normal forms.
...
To do so, we need a \term{reduction strategy}. It is possible that a $\lambda$ expression can be $\beta$-reduced in many ways, eg. if we have
\[
...
\begin{enumerate}
  \item Produce an ``infintie list'' (\term{stream}) of natural numbers.
...
We start by talking about well-foundedness. The idea is that we want to allow for the following \term{$R$-induction} principle:
\begin{prooftree}
  \AxiomC{$\forall_x ((\forall_y R(y, x) \to \varphi(y)) \to \varphi(x))$}
  \UnaryInfC{$\forall_x \varphi(x)$}
\end{prooftree}
...
The notion of well-foundedness is important in computer science, because it is what guarantees termination. Suppose we have a program with a loop of the form ``repeat \ldots until \ldots''. To prove that the ``until'' part will always happen, we look for a loop parameter taking values in $\dom(R)$ such that in every iteration of the loop, the value decreases. % fix this, talk about ackermann function
...
This leads us to think about quasiorders that are reflexive transitive closures of well-founded relations. We call these \term{well-founded quasi-orders}. Note that these are not actually well-founded, as any reflexive relation can certainly not be well-founded.

Under what operations are this class closed? We can think about \emph{lifting} a quasi-order on $X$ to a quasi-order on $\mathcal{P}(X), \mathcal{P}_{\aleph_0}(X), X^{ < \omega}$, multisets on $X$ etc. % < \omega means lists
...
Now consider the class of those quasi-orders $\bra X, \leq\ket$ such that $\bra \mathcal{P}(X), \leq^+\ket$ is well-founded. This class of quasi-orders is the class of \term{well quasi-orders}, otherwise known as \term{WQO}'s.
...
So $ \bra X, \leq\ket$ is a WQO as long as there are no infinite descending chains, and no infinite anti-chains. % it is sufficient?
...
\end{proof}
% The second property is what is usually taken as the definition of a WQO ?
