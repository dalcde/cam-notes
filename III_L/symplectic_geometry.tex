...
\begin{defi}[Non-degenerate/symplectic bilinear map]\index{non-degenerate bilinear map}\index{symplectic bilinear map}
  We say the bilinear map $\Omega$ is non-degenerate if $\tilde{\Omega}: V \to V^*$ is bijective, i.e.\ $U = \{0\}$.

  In this case, $(V, \Omega)$ is called a \term{symplectic vector space}.
\end{defi}
...
There are several consequences of being a symplectic vector space. First of all, $\tilde{\Omega}$ gives an canonical isomorphism $V \cong V^*$, and $\dim V$ is even. In particular, we can find a \term{symplectic basis} $\{e_1, \ldots, e_n, f_1, \ldots, f_n\}$ of $V$ such that
\[
...
\]
the \term{tautological $1$-form}. Then we set
\[
  \omega = - \d \alpha = \sum_{i = 1}^n \;\d x_i \wedge \;\d \xi_i,
\]
the \term{canonical symplectic form} on $T^* X$.
...
We look at the $4n$-dimensional manifold $M_1 \times M_2$, and we define the \term{product form} $\omega = \pr_1^* \omega_1 + \pr_2^* \omega_2$. We have
\[
...
\]
which is called the \term{twisted product form}\index{$\tilde{\omega}$}.
