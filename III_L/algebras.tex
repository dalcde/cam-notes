...
If we want to think of things in cohomology instead, then we are looking at things modulo coboundaries. For the graded Lie algebra $\bigwedge^{\Cdot + 1}(\Der A)$, the Maurer--Cartan elements, ie. solutions of the Maurer--Cartan equation, are the \term{formal Poisson structures}. They are formal power series of the form
\[
  \Pi = \sum t_i \pi_i
\]
...
\begin{defi}[Drinfeld double]\index{Drinfeld double}\index{quantum double}
  ...
  \]
  $D(G)$ is \term{quasitriangular}, ie. there is an invertible element $R$ of $D(G) \otimes D(G)$ such that
  \[
