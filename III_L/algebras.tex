...
\begin{eg}
  ...
  \]
  It is an exercise to verify this works.
\end{eg}
...
We will soon show that the algebras we know and love are domains. To do so, we need to make use of the filtration we know about them.
\begin{lemma}
...
\begin{proof}
  ...
  \]
  is a direct sum of non-zero submodules. But this is an infinite strictly ascending chain as $n \to \infty$, which is a contradiction. % read this
\end{proof}

Recall that when we proved Artin--Wedderburn, we needed to use Krull--Schmidt, which told us the decomposition is unique up to re-ordering. That relied on the endomorphism algebra belng local. We need something similar here.

\begin{lemma}
  Let $E$ be an indecomposable injective riht module. Then $\End_A(E)$ is a local algebra, and the unique maximal ideal $I$ satisfies $\End_A(E)/I \cong D$ for some division algebra $D$. In faact,
  \[
    I = \{f \in \End(E): \ker f\text{ is essential}\}.
  \]
  Since $E$ is indecomposable innjecitve, this is the same as saying $\ker F$ is non-zero.
\end{lemma}

\begin{proof}
  Let $f: E \to E$ and $\ker f = \{0\}$. Then $f(E)$ is an injective module, and so is a direct summand of $E$. But $E$ is indecomposable. So $f$ is surjective. So it is an isomorphism, and hence invertible.

  It is an easy exercise to show that the endomorphism with essential kernel form an ideal in $\End_A(E)$. % fill in as well
\end{proof}
...
\begin{lemma}
  Let $E_1, \cdots, E_r$ be indecomposable injectives. Putting $E = E_1 \oplus \cdots \oplus E_r$. Let $I = \{f \in \End_A(E): \ker f\text{ is essential}\}$. This is an ideal, and then
  \[
...
\begin{proof}
  Reorder the $E_i$ so as to get groups of isomorphism ones, just as we did with simples when proving Artin-Wedderburn. Note that if $E_i \not \cong E_j$, then the kernel of a map $E_i \to E_j$ is essential in $E_i$. $I$ consists of the matrices where all the centers in $\hom(E_i, E_j)$ have kernel essential in $E_i$. The remaining isa straightforward check, as in Artin--Wedderburn.
