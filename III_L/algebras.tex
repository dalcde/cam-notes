...
\begin{defi}[Hochschild chain complex]\index{Hochschild chain complex}\index{chain complex!Hochschild}
  ...
  \[
    \begin{tikzcd}
      \cdots \ar[r, "d"] & A \otimes_k A \otimes_k A \otimes A \ar[r, "d"] & A \otimes_k A \ar[r, "\mu"] & A \ar[r] & 0.
    \end{tikzcd},
  \]
...
as a bimodule by
\[
  x(a_0 \otimes a_1 \otimes \cdots \otimes a_{n + 1})y = (x a_0) \otimes a_0 \otimes \cdots \otimes (a_{n + 1} y).
\]
...
Note also that the sequence is exact, whose proof is a computation. This complex is a free resolution of our bimodule $_AA_A$.

Hochschild homology and cohomology is obtained by applying functors $\ph\otimes_{A\mdash A} M$ and $\Hom_{A\mdash A}(\ph, M)$, where $M$ is a bimodule. Since these are not exact functors, we get interesting homology and cohomology.

\subsection{Cohomology}
We want to apply $\Hom_{A\mdash A}(\ph, M)$ to the Hochschild chain complex. However, it is easier to describe $\Hom_{A \mdash A}(A^{\otimes (n + 2)}, M)$ by identifying it with $\Hom_k(A^{\otimes n}, M)$, using the fact that $A^{\otimes (n + 2)}$ is a free $A\mdash A$-bimodule generated by a basis of $A^{\otimes n}$.
...
Note that $\im \delta_{n - 1} \subseteq \ker \delta_n$.
...
\end{defi}
We are particularly interested in the case when $M = _AA_A$, and in this case we will later see that the Hochschild chain complex has additional structure. We will consider the graded vector space
\[
  HH^\Cdot (A, A) = \bigoplus_{A = 0}^\infty HH^n(A, A),
\]
which has the structure of a Gerstenhaber algebra.

Recall that we obtained the cochain complex by applying $\Hom_{A\mdash A}(\ph, M)$. If $M$ is an injective bimodule, then this functor is exact. So we deduce $HH^n(A, M) = 0$ for $n \geq 1$.

If $_AA_A$ is a projective bimodule and embeds as a bimodule direct summand of $A \otimes A$. Then we find that the Hochschild chain complex, we find that each $A^{\otimes (n + 2)}$ splits as the direct sum of the image of $A^{\otimes (n + 3)} \to A^{\otimes(n + 2)}$ and the kernel of $A^{\otimes (n + 2)} \to A^{\otimes (n + 1)}$, and further these components are all projective. So applying $\Hom_{A\mdash A}(\ph, M)$ yields an exact complex. So we get that for $n \geq 1$, we have $HH^n(A, M) = 0$ for all bimodules $M$.
...
\begin{defi}[$k$-separable]\index{separable algebra}\index{$k$-separable algebra}\index{separable algebra}\index{algebra!$k$-separable}
  An algebra $A$ is \emph{$k$-separable} if $_AA_A$ embeds as a direct summand of $A \otimes A$, in which case if we view our bimodules as $A \otimes a^\op$-modules, then we have a \term{separating idempotent} $e \in A \otimes A$ so that $_AA_A$ viewed as $A \otimes A^{\op}$-bimodule is $(A \otimes A^\op)e$.
\end{defi}

\begin{lemma}
  If $\Dim A = 0$, then $A$ is separable.
\end{lemma}
This is the converse of our previous observation.

\begin{proof}
  If $\Dim A = 0$, then $HH^1(A, M) = 0$ for any $M$. This means that when we apply $\Hom_{A\mdash A}(\ph, M)$ to the chain complex, we get exactness in the first slot. In other words, any map $\phi: \ker \mu \to M$ (where $\mu$ is the multiplication map) can be extended to $\phi': A \otimes A \to M$.

  We now pick $M = \ker \mu$, and $\phi$ the identity map. So we know that there is a map $A \otimes A \to \ker \mu$ that restricts to the identity on $\ker \mu$. In other words, the sequence
  \[
    \begin{tikzcd}
  ...
  So this splits as
  \[
    A \otimes A = \ker \mu \oplus _AA_A.
  \]
  Thus we can embed $_AA_A$ in $A \otimes A$ as a direct summand.
\end{proof}
...
The obvious question is --- is this notion actually a generalization of separable field extensions? This is indeed the case. However $k$-separable algebras must be finite-dimensional $k$-vector spaces. So it doesn't pass on to the infinite case. % insert a proof

Let's now start thinking about less trivial cohomology groups. We now try to look at what happens in low dimensions.
\begin{prop}
  We have
  \[
...
\[
  D_1 D_2 - D_2 D-1 \in \End_k(A)
\]
is in fact a derivation if $D_1$ and $D_2$ are.

The second cohomology group can be interpreted in terms of extensions of algebras.

\begin{defi}[Semi-direct product]\index{semi-direct product}\index{$A \ltimes M$} % check that bar is on left
...
This is sort-of an extension of $A$ by $M$.

Last time, we saw that the first cohomology was about derivations modulo inner derivations. We can look at derivations in this context.
\begin{lemma}
  We have
  \[
...
\begin{proof}
  ...
  \[
    ...
    \end{tikzcd}
  \]
  The function $A \to M$ given by $a \mapsto D_a$ is a dierivation, since under the embedding, we have
  \[
...
We may interpret this in terms of the automorphisms of the semi-direct product.

\begin{lemma}
  We have
  \[
    \Der(A, M) \cong \{\text{automorphisms of $A \ltimes M$ of the form $a \mapsto a + f(a) \varepsilon$, $m\varepsilon \mapsto m\varepsilon$}\},
  \]
  where we view $A \ltimes M \cong A + M \varepsilon$.

  Moreover, the inner derivations correspond to automorphisms achived by conjugation by $1 + m\varepsilon$, which is a unit with inverse $1 - m \varepsilon$.
\end{lemma}
The proof is a direct check.

This applies in particular when we pick $M = _AA_A$, and the Lie algebra of derivations of $A$ may be viewed as the set of infinitesimal automorphisms. This is a vague remark.

\separator

Let's now consider $HH^2(A, M)$.

\begin{defi}[Extension]\index{extension}
  Let $A$ be an algebra and $M$ and $A\mdash A$-bimodule. An extension of $A$ by $M$. An \emph{extension} of $A$ by $M$ is a $k$-algebra $B$ containing a 2-sided ideal $I$ with $I^2 = 0$, $B/I \cong A$ and $M \cong I$ as an $A\mdash A$-bimodule. Note that since $I^2 = 0$, the left and right multiplication in $B$ induces an $A\mdash A$-bimodule structure on $I$, rather than just a $B\mdash B$-bimodule.
\end{defi}
...
\begin{proof}
  Let $B$ be an extension with, as usual, $\pi: B \to A$, $I ^2 = 0$, $I = M = \ker \pi$. We now try to find a cocycle.
  ...
  \]
  We can check that $f_\rho$ is a $2$-cocycle, ie. it lies in $\ker \delta_2$.
  ...
  \end{align*}
  where $\ph$ denotesthe $A\mdash A$-bimodule action in $I$. Thus, we find
  \[
...
We do a bit of deformation theory, about star products. This is something mathematical physicists like to do.

Let $A$ be a $k$-algebra, and denote its underlying vector space by $V$. We want to consider defining associative products on $V \otimes_k k[[t]]$ that reduces to multiplication on $A$ when we set $t = 0$.

Such products are often denoted by $*$, and are called \term{star products}\index{$*$}.

We want to find $k[[t]]$-bilinear maps
\[
  V[[t]] \times V[[t]] \to V[[t]].
\]
But since they are bilinear, all we need to do is to consider what we send $a, b \in V = a$ to:
\[
  f: (a, b) \mapsto ab + t F_1(a, b) + t^2 F_2(a, b) + \cdots.
\]
...
It turns out star products are related to cohomology. The associativity of the star product implies
\[
  f(f(a, b), c) = f(a, f(b, c)).\tag{$*$}
\]
By looking at coefficients of $t$, this $(*)$ implies that for all $\lambda = 0, 1, 2, \cdots$, we have
\[
...
\[
  a F_1(b, c) - F-1(ab, c) + F_1(a, bc) - F_1(a, b)c = 0.
\]
Thus the corresponding function $A \otimes A \to A$ is a $2$-cocycle. This is not surprising. We've just seen (a while ago) that working mod $t^2$, the extensions of $A$ by $_AA_A$ are governed by $HH^2$.

We call such a $2$-cocycle an \term{infinitesimal deformation}\index{deformation!infinitesimal}. Note that given an arbitrary $2$ co-cycle $A \otimes A \to A$, it may not be possible to produce a $*$-product with the given $2$-cocycle as $F_1$. If it can be, then it is called \emph{integrable}\index{integrable cocycle}.

Let's rewrite $(**)$ as $(\dagger_\lambda)$:
\[
...
\begin{defi}[Equivalence of star proeducts]\index{equivalence of star products}\index{star product!equivalence}
  Two star products $f$ and $g$ are \emph{equivalent} on $V \otimes k[[t]]$ if there is a $k[[t]]$-linear automorphism $\Phi$ of $V \otimes k[[t]]$ of the form
  \[
  ...
  \]
  If we look at the corresponding map
  \[
    \begin{tikzcd}
      (V \otimes k[[t]]) \otimes (V \otimes k[[t]]) \ar[r, "f"] \ar[d, "\Phi"] & V \otimes k[[t]] \ar[d, "\Phi"]\\
      (V \otimes k[[t]]) \otimes (V \otimes k[[t]]) \ar[r, "g"] & V \otimes k[[t]]
    \end{tikzcd}
  \]
...
We want to understand the equations $(\dagger)$ better. To do this, we introduce algebraic structures on the Hochschild cochain complex, and on $HH^{\Cdot}(A, A)$.

Cup products are a standard tool in cohomology theories. We will write
\[
...
\]
So we say $\delta$ is a (left) \term{graded derivation}\index{derivation!graded} of the graded algebra $S^\Cdot(A, A)$. In homological (grade) algebra, we often use the same terminology but with suitable (or convenient) sign changes which depends on the degree.
...
\begin{defi}[Graded Lie algebra]\index{graded Lie algebra}\index{Lie algebra!graded}
  ...
  \begin{itemize}
    ...
    \item $(-1)^{mp} [[f, g], h] + (-1)^{mn} [[g, h], f] + (-1)^{np} [[h, f] ,g] = 0$, the \term{graded Jacobi identity}\index{Jacobi identity!graded},
  \end{itemize}
...
The prototype Gerstenhaber algebra is the exterior algebra $\Lambda_A \Der A$ for a commutative algebra $A$ (with $A$ in degree $0$).
...
\[
  [\lambda_1 \wedge \cdots \wedge \lambda_m, \lambda_1' \wedge \cdots \wedge \lambda_n'] = (-1)^{(m - 1)(n - 1)} \sum_{i, j} (-1)^{i + j} [\lambda_i, \lambda_j] \underbrace{\lambda_1 \wedge \cdots \lambda_m}_{\text{$i$th missing}} \wedge \underbrace{\lambda_1' \wedge \cdots \wedge \lambda_n'}_{\text{$j$th missing}}.
\]
...
\begin{thm}[Hochschild--Kostant--Ronsenberg (HKR) theorem]\index{Hochschild--Kostant--Ronsenberg theorem}\index{HKR theorem}
  ...
  \[
    \wedge_A (\Der A) \to HH^*(A, A)
  \]
  is an isomorphism of Gerstenhaber algebras.
\end{thm}
We will not say what ``smooth'' means, but this applies if $A = k[X_1, \cdots, X_n]$, or if $k = \C$ or $R$ and $A$ is appropriate functions on smooth manifolds or ralgebraic varieties.
...
If we want to think of things in cohomology instead, then we are looking at things modulo coboundaries. For the graded Lie algebra $\Lambda^{\Cdot + 1}(\Der A)$, the Maurer--Cartan elements, ie. solutions of the Maurer--Cartan equation, are the \term{formal Poisson structures}. They are formal power series of the form
\[
  \Pi = \sum t_i \pi_i
\]
...
\[
  [\Pi, \Pi] = 0.
\]
There is a deep theorem of Kontsench from the early 2000's which implies
...
\[
  \begin{tikzcd}
    \cdots \ar[r, "b_1"] & (A \otimes_k A \otimes_k A \otimes A) \otimes_{A\mdash A} M \ar[r, "b_0"] & (A \otimes_k A) \otimes_{A \mdash A}M \cong M
  \end{tikzcd},
\]
...
\begin{lemma}
  \[
    HH_0(A, M) = \frac{M}{\bra xm - mx: m \im M, x \in A\ket}.
  \]
  In particular,
  \[
    HH_0(A, A) = \frac{A}{[A, A]}.
...
\begin{defi}[Coalgebra]\index{coalgebra}
  ...
  \[
    \begin{tikzcd}
      ...
      C \otimes C \ar[r, "\Delta \otimes \id"] & C \ar[l, "\Delta"] \ar[u, "\Delta"]
    \end{tikzcd}
    \quad
    \begin{tikzcd}
      k \otimes C & C \otimes C \ar[l, "\varepsilon \otimes \id"]\ar[r, "\id \otimes \varepsilon"] & C \otimes k\\
      & C \ar[u, "\mu"] \ar[ru, "\cong"] \ar[rl, "\cong"]
    \end{tikzcd}
  \]
