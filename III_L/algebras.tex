...
\[
  K_i(A) \to HH_i(A),
\]
where $HH_*$ is the Hochschild homology, and we have just seen the $i = 0$ case, and the $i = 1$ case is called the \term{Dennis trace map}.
...
\begin{eg}
  ...
  \]
\end{eg}
To prove that $A_n(k)$ and $\mathcal{U}(\mathfrak{g})$ are Noetherian, we use some deformation theory.
...
\end{defi}
From the definition, we know $\bigcup A_i$ is a subalgebra.
...
We usually assume our filtration is exhaustive and separated.
...
\]
which allows us to form an \term{associated graded module}
\[
...
We can actually define it more generally.
\begin{defi}[Gelfand-Kirillov dimension]\index{Gelfand-Kirillov dimension}\index{$\GKdim(M)$}\index{$d(M)$}
  ...
  \]
\end{defi}
In the case of $A = A_n(k)$ or $\mathcal{U}(\mathfrak{g})$, then this matches the previous definition.

Note that this is independent of the choice of generating sets, which is an exercise on the example sheet. % prove this!

Recall we showed that no non-zero $A_n(k)$-module $M$ can have finite dimension as a $k$-vector space. So we know $d(M) > 0$. Also, we know that $d(M)$ is an integer for cases $A = A_n$ or $\mathcal{U}(\mathfrak{g})$, since it is the degree of a polynomial. However, for $M = A$, we can get non-integral values. In fact, the values we can get are $0, 1, 2$, and then any real number $\geq 2$. We can also have $\infty$ if the $\limsup$ doesn't exist.
