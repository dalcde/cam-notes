...
\]
The associativity forces $\Psi$ to satisfy the condition that it must be a $2$-cycle, which we will go into more detail soon. We also want
\[
  1 = 1e. 
\]
So we need
\[
  \Psi(e, g) = \Psi(g, e) = 1 \in B.
\]
This \term{crossed product} $A$ is of the form
\[
  \bigoplus Bg,
\]
where each summand is a $B\mdash B$-bimodule, and is labelled by an element $g \in G$. Also, we have $Bg_1 \cdot Bg_2 \subseteq B g_1 g_2$. Such an algebra is called a \term{$G$-graded algebra}.

We now restrict things a bit. We are going to take $B$ to be a finite field extension $K$. Let's assume that this extension of $k$ is Galois, and the group of automorphisms of $K$ that fix $k$ is called the \term{Galois group} \term{$\Gal(K/k)$}, and we pick this to be our $G$.

We also consider $\Psi: G \times G \to K^\times$. We observe that $K^\times$ is an abelian multiplicative group. Therefore we can regard $K^\times$ as a $\Z G$-module, with $G$ acting via the Galois action.
...
\begin{eg}
  ...
  \]
  So we want $\Psi(G, G) = -1$. % complete?
\end{eg}
Note that in general, crossed products need not be division algebras.

Let us use the notation $(K, G, \Psi)$, where $\Psi: G \times G \to K^\times$. Associativity of these crossed products is equivalent to a $2$-cocycle condition $\Psi$, which you will be asked to make precise on the first example sheet.
...
\[
  ...
  \end{tikzcd}
\]
satisfies a $2$-coboundary conditions, which are again left for the first example sheet. Therefore the second (group) cohomology
\[
...
\end{defi}
Note that given any simple algebra, the simplicity forces the center to be a field. So this is not really so much of a restriction. % why?
...
\section{Projectives and blocks}
...
\begin{proof}
  ...
  \]
  Since $\End_A(M_1)$ is local, we know some $\alpha_i \circ \beta_i$ must be invertible, ie. a unit, as they cannot all lie in the Jacobson radical. We may wlog $\alpha_ \circ \beta_1$ is a unit. If this is the case, then both $\alpha_1$ and $\beta_1$ have to be invertible. So $M_1 \cong M_1'$. % why
...
'
\begin{lemma}
  Suppose $M$ is an indecomposable module satisfying ACC and DCC on submodules.
\end{lemma}
...
\begin{cor}
  ...
  \[
    1 = e_1 + \cdots  + e_r,
  \]
  with $\{e_i\}$ orthogonal idempotents in $A$, and $\bar{e}_i = f_i$.
\end{cor}
\end{document}
