...
\[
  ...
  \end{tikzcd}
\]
where $p: S(E) \to E$ is the projection, and $p_!$ is whatever makes the diagram commutes (since $j^*$ and $\Phi$ are isomorphisms). The bottom sequence is the \term{Gysin sequence}, and it is exact because the top row is exact. This is in fact a long exact sequence of $H^*(X, R)$-modules, ie. the maps commute with cup products.
...
\[
  ...
  \end{tikzcd}
\]
If we look at this composition via the bottom map, then $[N]$ gets sent to $D_M^{-1}([N])$. So we know that the Poincar\'e dual of a submanifold is the (extension by zero of the) normal Thom class. % return to this
...
So far everything works for an arbitrary manifold. Now we suppose further that $M$ is smooth. Then $\delta$ is the Thom class of the normal bundle $\mathcal{V}_{M \subseteq M \times M}$ of % huh
\[
