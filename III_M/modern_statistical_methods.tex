...
\subsection{Kernels}
We'll aim to characterize the similarity measures $k: X \times X \to \R$ for which there exists a feature map $\phi: X \to \mathcal{H}$, where $\mathcal{H}$ is a real inner product space, with% use $\mathcal{X}$
\[
...
\begin{defi}[Positive-definite kernel]\index{positive-definite kernel}\index{kernel}
  A \emph{positive-definite kernel} (or simply \emph{kernel}) is a symmetric map $k: X \times X \to \R$ such that for all $n \in \N$ and $x_1, \ldots, x_n \in X$, the matrix $K \in \R^n \times \R^n$ with entries
  \[
    K_{ij} = k(x_i, x_j)
  \]
...
\begin{proof}
  Let $x_1, \ldots, x_n \in X$, and $\alpha \in \R^n$. Consider
  \begin{align*}
...
\begin{eg}
  The \term{Jaccard similarity} is defined as follows: Take $X$ to be the set of all subsets of $\{1, \ldots, p\}$. For $x, x' \in X$, define
  \[
    k(x, x') = 
    \begin{cases}
