...
\begin{center}
  \begin{tikzpicture}
    ...
    \draw (2.5, 2) ellipse (0.5 and 0.1); % maybe rotate this.
  \end{tikzpicture}
\end{center}
...
We are actually more interested in convex functions. Let $\bar{\R} = \R \cup \{\infty\}$. We shall allow our functions to take value $\infty$. The point is that if we want our function to be defined in $[a, b]$, then it is convenient to extend it to be defined on all of $\R$ by setting the function to be $\infty$ outside of $[a, b]$.
...
\]
The Lagrangian method involves finding a $\theta^*$ such that the $x^*$ minimizing $L(x, \theta^*)$ satisfies $g(x^*) = 0$. This $x^*$ must then be optimal for the original problem. % fix this
...
\]
for some $\hat{\nu}$ with $\|\hat{\nu}\|_{\infty} \leq 1$ and $\hat{\nu}_{\hat{\rho}_\lambda^L} = \sgn (\hat{\beta}_{\lambda, \hat{\rho}^L_\lambda}^L)$. These are known as the \term{KKT conditions} for the Lasso.
...
\begin{thm}
  ...
  \[
    \P\left(\phi^2_{\hat{\Sigma}, s} \geq \frac{1}{2} \phi^2_{\Sigma^0, s}\right) \to 1 % \geq 1 - O(s \sqrt{\log p / n})
  \]
  as $n \to \infty$.
\end{thm}
...
We shall explore the method of \term{coordinate descent}. This is a general method to find the minimum of a function.
...
The \term{LARS-OLS hybrid} re-estimate $\beta^0_{\hat{S}_\lambda}$ by OLS. There are similarly the \term{relaxed Lasso} and \term{adaptive Lasso}.
...
\begin{defi}[Undirected edge]\index{undirected edge}
  An edge $(j, k)$ is \emph{undirected} if also $(k, j) \in E$. Otherwise, it is \emph{directed}\index{directed edge} and we write $j \to k$ to represent it.
\end{defi}
...
\begin{defi}[Parent and child]\index{parent}\index{$\mathrm{pa}$}\index{$\mathrm{ch}$}
  We say $j$ is a \emph{parent} of $k$ and $k$ is a \term{child} of $j$ if $j \to k$. The sets of parents and children of $k$ will be denoted $\pa(k)$ and $\child(k)$ respectively. % index these
\end{defi}
...
\begin{defi}[Descendent]\index{descendant}\index{$\mathrm{de}$}
...
\begin{defi}[Directed cycle]\index{directed cycle}
  A \emph{directed cycle} is (almost) a directed path but with the start and end points the same.
\end{defi}

\begin{defi}[Partially directed acyclic graph (PDAG)]\index{partially directed acyclic graph}
...
\begin{defi}[Moralised graph]\index{moralised graph}
...
\begin{proof}
  We assume $\mathcal{G}$ is topologically ordered. % define this
...
\end{prop}
The set of all DAGs that are Markov equivalent to a given DAG can be represented by a \term{CPDAG}, which contains an edge $(j, k)$ iff some member of the equivalence class does.
...
We can now describe the first part of the \term{PC algorithm}:
\begin{enumerate}
...
\begin{eg}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node [mred] at (-1.5, 3) {$H_{1}$};
      \node [mred] at (-0.5, 3) {$H_{2}$};
      \node at (0.5, 3) {$H_{3}$};
      \node at (1.5, 3) {$H_{4}$};
    \end{tikzpicture}
  \end{center}
...
But the FWER is still a rather strict requirement. Instead of controlling the probability that there is one false rejection, when $m$ is large, it might be more reasonable to control the proportion of false discoveries. Many modern multiple testing procedures aim to control the \term{false discovery rate}
\[
...
The recently introduced \term{debiased Lasso} overcomes these issues. See (van de Geer et al, 2014) for more details. Let $\hat{\beta}$ be the Lasso solution at $\lambda > 0$. Recall the KKT conditions
\[
