...
\pgfdeclaredecoration{penciline}{initial}{
    \state{initial}[width=+\pgfdecoratedinputsegmentremainingdistance,auto corner on length=1mm,]{
        ...
        {%TO
        \pgfpointadd{\pgfpointdecoratedinputsegmentlast}{\pgfpoint{3pt}{3pt}}
        }
    }
    \state{final}{}
}
...
%  Indeed, $\{p \in [0, 1]: \text{exists a finite set $S$ with $0 \in S$ and $\varphi_p(S) < 1$}\}$ is an open subset of $[0, 1]$, since for any fixed finite set $S$, $\varphi_p(S)$ is a continuous function in $p$. Thus, $\tilde{p}_c > 0$.
  ...
  \begin{enumerate}
    ...
    \item We want to use Russo's formula. We claim that it suffices to prove that
      ...
      \]
      Observe that $\{0 \overset{A}{\leftrightarrow} x\}$ and $\{\mathcal{S} = A\}$ are independent, since to determine if $\mathcal{S} = A$, we only look at the edges on the boundary of $A$. So the above is equal to % check
      \begin{align*}
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw [mblue, decorate, decoration=penciline] (0, 2.5) -- (6, 2.4);
      \draw [mred, decorate, decoration=penciline] (3, -0.5) -- (2.9, 5.5);
    \end{tikzpicture}
  \end{center}
...
\begin{proof}\leavevmode
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.6]
      ...
      \draw [mblue, decorate, decoration=penciline] (0, 3) -- (6, 2.9);
      \draw [mblue, decorate, decoration=penciline] (2, 1) -- (8, 1.1);
      \draw [mblue, decorate, decoration=penciline] (3, 0) -- (3.1, 4);
    \end{tikzpicture}
  \end{center} % insert paths
  We let
  \begin{align*}
...
\begin{proof}[Proof sketch]
  ...
  \begin{center}
    \begin{tikzpicture}
       ...
       \draw plot [smooth] coordinates {(-2, 1) (-0.5, 1.2) (0, 0.8) (-0.4, -1) (0, -1.3) (2, -1.1)};
       \node [circ] at (0, -1.3) {};
       \node [anchor = north east] {$(0, y_\pi)$};
    \end{tikzpicture}
  \end{center}
  Let % insert picture
  \begin{align*}
...
%  If there exists a left-right crossing of $[0, n + 1] \times [0, n]$, then there also exists a left-right crossing of $[0, n + 1] \times [-1, n + 1]$. Then
%  \[
%    \P_{\frac{1}{2}}(\text{LR of }[0, n + 2] \times [-1, n + 1]) \P_p(\text{LR of }[0, n + 1] \times [-1, n + 1], \text{last edge open})
%  \]
\end{proof}
...
\index{slab percolation}In $d \geq 3$, define $S_k = \Z^2 \times \{0, 1, \ldots, k\}^{d - 2}$. Then $S_k \subseteq S_{k + 1} \subseteq \Z^d$.
...
If we take the dual of the triangular lattice, which is the hexagonal lattice. % insert picture
...
\]
We shall have $2$ distinguished vertices, $a$ and $z$, called the \term{source} and the \term{sink} respectively.
...
\begin{defi}[Flow from $a$ to $z$]\index{flow}
  ...
  \end{enumerate}
  The \term{strength} of the flow from $a$ to $z$ is $\|\theta\| = \div \theta(a)$.\index{$\|\theta\|$}

  We say this is a \term{unit flow} if $\|\theta\| = 1$.
\end{defi}
...
\begin{defi}[Current flow]\index{current flow}
  ...
  \]
\end{defi}
One checks that $I$ is indeed a flow. By definition, $I$ satisfies \term{Ohm's law}:
\[
  W(x) - W(y) = r(x, y) I(x, y).
\]
Also, $I$ satisfies the \term{cycle law}, i.e.\ for any cycle $\vec{e}_1, \vec{e}_2, \ldots, \vec{e}_n$, we have
\[
...
\begin{defi}[Effective resistance]\index{Effective resistance}
  ...
  \]
  for any voltage $W$ with associated current $I$. The \term{effective conductance} is $C_{eff}(a, z) = \frac{1}{R_{eff}(a, z)}$.
\end{defi}
...
From high school physics, we know that conductances in parallel add:
\begin{center}
  \begin{tikzpicture}
    ...
    \draw (6.5, 0) -- (7.5, 0);
    \draw [decorate, decoration={zigzag}] (7.5, 0) -- (8, 0) node [pos=0.5, above] {$c_2$};
    \draw (8, 0) -- (9, 0);
  \end{tikzpicture}
\end{center}

In general, let $e_1$ and $e_2$ be $2$ edges with the same endvertices. Then we we can replace them by a single edge of conductance the sum of the conductances. The voltages and currents remain unchanged outside of $e_1, e_2$. We can check Kirchhoff's and Ohm's law with
\[
  I(\vec{e}) = I(\vec{e}_1) + I(\vec{e}_2).
\]
Similarly, resistances in series add. Let $v$ be a node of degree $2$ and let $v_1$ and $v_2$ be its $2$ neighbours. Then we can replace these $2$ edges by a single edge of resistance $r_1 + r_2$. We can check Kirchoff's and Ohm's law with
\[
  I(\vec{v_1 v_2}) = I(\vec{v_1 v}) + I(\vec{vv_2}).
\]
\begin{center}
  \begin{tikzpicture}
    \draw (0, 0) -- (0.5, 0);
    ...
    \node at (4, 0) {$=$};
    \draw (5, 0) -- (6, 0);
    \draw (6, 0) -- (6.5, 0) node [pos=0.5, above] {$r_1 + r_2$};
    \draw (6.5, 0) -- (7.5, 0);
  \end{tikzpicture}
\end{center}
...
\begin{prop}
  ...
  \[
    G_\tau(a, x) = \pi(x) \E_a[\tau]. % invariant distribution. % G(a, x) is the number of times we hit x before tau happens
  \]
\end{prop}
...
Let $(G_k = (V_k, E_k))_k$ be an \term{exhaustion} of $G$ by finite graphs, i.e.
\begin{itemize}
...
Let $\mathcal{T}$ be the set of all spanning trees of $G$. Pick $T \in \mathcal{T}$ uniformly at random. We call it the \term{uniform spanning tree}.
\begin{thm}
  Let $e \not= f \in E$. Then
  \[
...
By Kolmogorov's theorem, we get a unique probability measure $\mu$ supported on the forests of $G$. $\mu$ is called the \term{wired uniform spanning forest}.
\printindex
\end{document}
