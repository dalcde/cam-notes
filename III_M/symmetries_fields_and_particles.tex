...
Now we can use what we know about the representations of $\sl(2)$ to get something interesting about $V$. Recall that we defined
\[
  h^\alpha = \frac{2}{(\alpha, \alpha)} H^\alpha = \frac{2}{(\alpha, \alpha)}(\alpha, \alpha) (\kappa^{-1})_{ij} \alpha^j H^i.
\]
...
\begin{defi}[Root lattice]\index{root lattice}
  ...
  \]
\end{defi}
We now define some funny normalization of the roots:
...
\begin{defi}[Weight lattice]\index{weight lattice}
  ...
  \[
    \mathcal{L}_W [\mathfrak{g}] = \check{\mathcal{L}}^*[\mathfrak{g}] = \{\lambda \in \mathfrak{h}^* \R: (\lambda, \mu) \in \Z \text{ for all }\mu \in \check{\mathcal{L}}[\mathfrak{g}]\}.
  \]
\end{defi}
So if $\lambda \in \mathcal{L}_W[\mathfrak{g}]$, then we have
\[
...
Every finite-dimensional representation $\rho$ of $\mathfrak{g}$ has a \term{highest weight}
\[
  \Lambda = \sum_{i = 1}^r \Lambda^i \omega_{(i)} \in S_\rho,
\]
with $\Lambda^i \in \Z, \Lambda^i \geq 0$.

This is an eigenvector satisfying
\[
  \rho(h^i) v_\Lambda = \Lambda^i v_\Lambda.
\]
Moreover, this is annihilated by
\[
  \rho(E^\alpha) v_\Lambda = 0
\]
for all $\alpha \in \Phi_+$.

These $\Lambda^i$ are the \term{Dynkin labels} of $\rho$.

The remaining weights $\lambda \in S_\rho$ of the representation are generated by acting with lowering operators.

Recall that if $v \in V_\lambda$ and $\rho(E^\alpha) v \in V_{\lambda + \alpha}$ if $\lambda + \alpha \in S_\rho$, and $0$ otherwise. So acting with the step operators translates us in the weight lattice by the vector corresponding to the root.

Here is a useful result about these things that we will state without proof:
\begin{prop}
  ...
  The process terminates when all the Dynkin labels are negative.
\end{prop}

\begin{eg}
  Take the example of $\mathfrak{g} = A_2$. It is a fact that the \emph{fundamental representation} $S_f$ has Dynkin labels $(1, 0)$. In other words,
  \[
    \Lambda = \omega_{(1)}.
  \]
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw [mgreen, thick, ->] (-1, 0.577) node [left] {$(-1, 1)$} -- (0, -1.155) node [below] {$(0, -1)$}; % highlight nodes
    \end{tikzpicture}
  \end{center}
\end{eg}

For a highest weight, we have $\Lambda^i \geq 0$ for all $i$. We say $\Lambda$ is a \term{dominant integral weight}. We call the set of dominant integral weights $\bar{\mathcal{L}}_@[\mathfrak{g}]$.

For a general finite-dimensional simplex complex Lie algebra $\mathfrak{g}$, the representations $\rho_\Lambda$ are all irreducible, and conversely all irreducible representations are of this form.

\subsection{Irreps of \texorpdfstring{$A_2$}{A_2}}
We know $A_2 \cong \mathcal{L}_\C(\su(3))$. According to the previous general statement, we have dominant weights of the form
\[
...
\]
This is symmetric in $\Lambda^1$ and $\Lambda^2$. So if $\Lambda^1\not= \Lambda^2$, then we get a pair of representations. We know that
\[
  \lambda \in S_{(\Lambda^1, \Lambda^2)} \Leftrightarrow -\lambda \in S_{(\Lambda^2, \Lambda^1)}
\]
So in fact they are related by
\[
  \rho_{(\Lambda_1, \Lambda_2)} = \bar{R}_{(\Lambda_1, \Lambda_2)}.
\]
On the other hand, if $\Lambda_1 = \Lambda_2$, then this representation is self-conjugate.

Consider the weight lattice, where we filled in the region of dominant weights:
\begin{center}
  \begin{tikzpicture}
    \foreach \x in {-2, 0, 2} {
      \foreach \y in {0, 1.155, -1.155} {
    ...
    % fill in dominant weights
  \end{tikzpicture}
\end{center}
Now we have a representation $\rho_{(1, 0)}$, which we found to be % insert diagram

Similarly, we have a conjugate representation. We can list these as
\begin{center}
  \begin{tabular}{ccc}
    \toprule
...
\[
  \Lambda - 2\alpha_{(1)} - 2 \alpha_{(2)} = (-1, -!) \in S_\rho.
\]
now notice that we only have seven roots, not eight. This is because our algorithm did not tell us the multiplicity of the roots. However, in this particular case, we know, because this is the adjoint representation, and the Cartan generators give us two things of weight $0$.

It is in fact a general fact that the weight lattice traces out a polygon like this, and the vertices at the perimeter are always non-degenerate.

\subsection{Decomposition of tensor products}
The next thing we need to know how to do is how we can decompose tensor products of irreps.
...
\[
  V_\Lambda = \bigoplus_{\lambda \in S_\Lambda} V_\lambda,\quad V_{\Lambda'} = \bigoplus_{\lambda' \in S_{\Lambda'}} V_{\lambda'}.
\]
If $\lambda \in S_\Lambda$ and $\lambda' \in S_{\Lambda'}$, then we know $\lambda + \lambda' \in \mathcal{L}_W[\mathfrak{g}]$, and this is a weight of the tensor product representation $\rho_\Lambda \otimes \rho_{\Lambda'}$. This is something we already know for representations of $\su(2)$, and the same proof shows that this is also true for a general simple Lie algebra.

Indeed, if $v_\lambda \in V_\Lambda$, then
\[
  \rho_\Lambda(H^i)v_\lambda = \lambda^i v_\lambda.
\]
Similarly, if $v_{\lambda'} \in V_{\lambda'}$, then
\[
  \rho_{\Lambda'}(H^i)v_{\lambda'} = \lambda'^i v_{\lambda'}.
\]
So we have
\begin{align*}
  (\rho_\Lambda \otimes \rho_{\Lambda'})(H^i)(v_\lambda \otimes v_{\lambda'}) &= \rho_\Lambda(H^i)v_\lambda \otimes v_{\lambda'} + v_\lambda \otimes \rho_{\Lambda'}(H^i) v_{\lambda'}\\
  &= \lambda^i v_\lambda \otimes v_{\lambda'} + \lambda'^i v_\lambda \otimes v_{\lambda'}\\
  &= (\lambda^i + \lambda'^i) v_\lambda \otimes v_{\lambda'}.
\end{align*}
...
\]
This allows us to decompose tensor products. So we have
\[
  \rho_{\Lambda} \otimes \rho_{\Lambda'} = \bigoplus_{\Lambda'' \in \mathcal{L}_W} \mathcal{N}_{\Lambda, \Lambda'}^{\Lambda''} \rho_{\Lambda''},
\]
where $\mathcal{N}_{\Lambda, \Lambda'}^{\Lambda''} \in \Z_{\geq 0}$ are the multiplicities.

The challenge is to figure out what the multiplicities are. To do this, we find out what the highest weight it, and then subtract off the factors corresponding to the highest weight, and then repeat.
\begin{eg}
