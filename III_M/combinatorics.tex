...
\subsection{The edge isoperimetric inequality in the cube}
...
\section{Sum sets}
Now let $G$ be an abelian group, and $A, B \subseteq G$. Define
\[
...
So let's look at a group with no subgroups. In other words, pick $G = \Z_p$. We claim that if $|A| + |B| \geq |G| + 1$, then $A + B = G$.
...
\begin{cor}
  ...
  \]
\end{cor}
What if we don't take sets, but sequences? Let $a_1, \ldots, a_m \in \Z_n$. What $m$ do we need to take to guarantee that some of the $a_i$ sum to $0$? By the pigeonhole principle, $m \geq n$ suffices. Indeed, consider the sequence
\[
...
\[
  a_{k + 1} + \cdots + a_{k'}.
\]
On the other hand, $m \geq n$ is also necessary, since we can take $a_i = 1$ for all $i$.
...
\begin{proof}
  ...
  \]
  If $a_i = a_{i + p - 1}$, then there are $p$ terms that are the same, and so we are done by adding them up. Otherwise, with $A_i = \{a_i, a_{i + p - 1}\}$ for $i = 1, \ldots, p - 1$, and $A_p = \{a_{2p - 1}\}$, then $|A_i| = 2$ for $i = 1, \ldots, p - 1$ and $|A_p| = 1$. Hence we know
  \[
...
\section{Projections}
Consider a bounded open set $K \subseteq \R^n$. Consider the projection of $K$ onto the different coordinate planes. Then we get $2^n - 2$ invariants given by the volumes of the projections.

More precisely, for $A \in [n]$, define
\[
  K_A = \{(x_i)_{i \in A} : \exists y \in K, y_i - x_i\text{ for all }i \in A\} \subseteq \R^A.
\]
We then take the Lebesgue measure of $K_A$ as a subset of $\R^A$, written $|K_A|$. This gives the set of invariants
\[
  \{|K_A| : A \subseteq [n]\}.
\]
We may or may not want to exclude the case $A = \emptyset$ and $A = [n]$. This gives a set of $2^n$ or $2^n - 1$ or $2^n - 2$ numbers that measure the size of the boundary of $K$.

Suppose we know a subset of those numbers. Can we use them to bound $|K|$? In some cases, it is completely trivial.
\begin{eg}
...
But, say, in $\R^3$, can we bound $|K|$ given $|K_{12}|$, $|K_{13}|$ and $|K_{23}|$?
...
Let's try to generalize this. We say a family $A_1, \ldots, A_r \subseteq [n]$ \emph{covers}\index{cover} $[n]$ if
\[
  \bigcup_{i = 1}^r A_r = [n],
\]
and is a \term{uniform $k$-cover} if each $i \in [n]$ is in exactly $k$ many of the sets.
...
\begin{thm}[Uniform cover inequality]\index{uniform cover inequality}
  If $A_1, \ldots, A_r$ is a uniform $k$-cover off $[n]$, then
  \[
    |K|^k = \prod_{i = 1}^r |K_A|.
  \]
\end{thm}
...
\begin{proof}
  ...
  \[
    \int f_1 \cdots f_k\;\d \leq \prod_{i = 1}^k \left(f_i^k\right)^{1/k}.
  \]
  ...
  \begin{align*}
    ...
    &\leq \prod_{A \in \mathcal{A}_-} |K_A|^{1/k} \int \prod_{A \in \mathcal{A}_+}\tag{by (1)}\\
...
\begin{proof}
  ...
  \[
    |K|^k = \prod_{A \in \mathcal{A}} x_A\tag{$1$}
  \]
  and moreover
  \[
    x_A \leq \prod_{i \in A} x_i\tag{$2$}
  \]
  for all $A \subseteq [n]$. We know this exists since there are only finitely many inequalities to be satisfied, and we can just decrease the $x_A$'s one by one. Now again by finiteness, for each $x_A$, there must be one inequality involving $x_A$ on the right-hand side that is in fact an equality.

  \begin{claim}
    For each $i \in [n]$, there exists a uniform cover $\mathcal{C}_i$ containing $\{i\}$ with equality
    \[
  ...
  and let
  \[
    \mathcal{C}' = \mathcal{C} \setminus \{\{1\}, \{2\}, \ldots, \{n\}\},
  \]
  and let
  \[
    k = \sum_{i = 1}^n k_i.
...
\begin{cor}
  Let $S$ be a set of sequences of length $n$ with terms from a finite set $X$. Then there exists a $k$-cover $\mathcal{A}$ of $[n]$ such that
  \[
    |S|^k \leq \prod_{A \in \mathcal{A}} |S_A|,
  \]
  where $S_A$ is the restriction of the sequences in $S$ to $A$.
\end{cor}

\section{Alon's combinatorial Nullstellensatz}
We begin by proving a sequence of lemmas analogous to things we know about polynomials in one variable. Recall the following result:
\begin{prop}[Division algorithm]\index{division algorithm}
  Let $R$ be a ring and $f, g \in R[X]$ with $g$ monic. Then we can write
  \[
    f = hg + r,
  \]
...
\begin{lemma}
  ...
  where
  \begin{align*}
    \deg h_i &\leq \deg f - \deg d_i\\
    \deg_{X_i} h_i &\leq \deg_{X_i} f - d_i\\
    \deg_{X_j} h_i &\leq \deg_{X_j} f\\
    \deg_{X_i} r &\leq d_i - 1\\
    \deg_{X_i} r \leq \deg_{X_i} f\\
    \deg r \leq \deg f
  \end{align*}
  for all $i, j$.
\end{lemma}
...
\begin{proof}
  ...
  Then we have
  \begin{align*}
    \deg_{X_1} h_1 &\leq \deg_{X_1} f - d_1\\
    \deg h_1 &\leq \deg f\\
    \deg_{X_j} &\leq h_1 \leq \deg_{X_j}f\\
    \deg_{X_1} r_1 &\leq d_1 - 1\\
    \deg_{X_j} r_1 &\leq \deg_{X_j} f\\
    \deg r \leq \deg f.
  \end{align*}
  Then repeat this with $f$ replaced by $r_1$, $g_1$ by $g_2$, and $X_1$ by $X_2$.
\end{proof}

We know that a polynomial of $1$ variable of degree $n \geq 1$ over a field has at most $n$ zeroes
...
\begin{proof}
  Let $d_i = |S_i| - 1$. We induct on $n$. If $n = 1$, then we are done. For $n \geq 2$, consider $h$ as a one-variable polynomial in $F[X_1, \ldots, X_{n - 1}]$ in $X_n$. Then
  \[
    h = \sum_{i = 0}^{d_n} g_i(X_1, \ldots, X_{n - 1}) X_m^i.
  \]
  Fix $(x_1, \ldots, x_{n - 1}) \in S_1 \times \cdots S_{n - 1}$, and set $c_i = g_i(x_1, \ldots, x_{n - 1}) \in \F$. Then
  \[
    \sum_{i = 0}^{d_n} c_i X_n^i
  \]
  vanishes on $S_n$. So $c_i = g_i(x_1, \ldots, x_{n - 1}) = 0$ for all $(x_1, \ldots, x_{n - 1}) \in S_1 \times \cdots \times S_{n - 1}$. So by induction, $g_i = 0$. So $h = 0$.
\end{proof}
...
\begin{lemma}
  For $i = 1, \ldots, n$, and let $S_i$ be a non-empty finite subset of $\F$, and let
  \[
