...
Another possible approach is via Galois cohomology. This is how it is done by Serre and Neukirch. This, however, relies on more advanced machinery, namely Galois cohomology.
...
Since we know that $N(LM/K) = N(L/K) \cap N(M/K)$, it suffices to consider the cases
\[
...
\]
separately. The first case is easy, because this corresponds to an unramified extension by the above corollary, and unramified extensions are completely characterized by the extension of the residues field.
...
\begin{eg}
  ...
  \]
  It is a fact that this is the Artin map of $\Q_p$ restricted to $\Z_p^\times$. Note that we have $W(\Q_p(\zeta_{p^\infty})/\Q_p) = \Gal(\Q_p(\zeta_{p^\infty})/\Q_p)$ because there is maximal unramified subextension is trivial.
...
We now want to do this in the general case. To do so, we need to introduce the notion of a formal group.
...
\begin{defi}[Formal module]\index{formal module}\index{formal group!module}\index{module!formal group}
  Let $\mathcal{O}$ be a ring. A \emph{formal $\mathcal{O}$-module} is a formal group $F$ over $\mathcal{O}$ with a ring homomorphism $\mathcal{O} \to \End_{\mathcal{O}}(F)$, written, $a \mapsto [a]_F$, such that
  \[
    [a]_F(X) = aX \mod X^2.
  \]
\end{defi}

Now let $K$ be a local field, and $q = |k_K|$. We let $\pi \in \mathcal{O}_K$ be a uniformizer.
...
\begin{eg}
  ...
  \]
  which is on the second example sheet.
\end{eg}

It turns out there aren't too many Lubin-Tate modules.
...
Inside $\mathcal{E}_\pi$, we have the polynomial of the form
\[
  uX^q + \pi(a_{q - 1} X^{q - 1} + \cdots + a_2 X^2) + \pi X
\]
with $u \in U_K^{(1)}$, and $a_{q-1}, \cdots, a_2 \in \mathcal{O}_K$. These are called \term{Lubin-Tate polynomials}.
...
We now take three Lubin-Tate series $e, e_1, e_2 \in \mathcal{E}_\pi$ and $a \in \mathcal{O}_K$. Then we know that there is a unique power series $F_e(X, Y) \in \mathcal{O}_K[[X, Y]]$ and $[a]_{e_1, e_2} \in \mathcal{O}_k[[X]]$ such that
\begin{align*}
...
We now state the theorem that classifies all Lubin-Tate moduels in terms of Lubin-Tate series
\begin{thm}
  ...
  If $a \in \mathcal{O}_K^\times$, then it is an isomorphism with $[a^{-1}]_{e_2, e_1}$.
\end{thm}
...
Recall that we fixed an algebraic closure $\bar{X}$ of $K$. We let $\bar{\mathfrak{m}} = \mathfrak{m}_{\bar{K}}$ be the maximal ideal in $\mathcal{O}_{\bar{K}}$.
...
\end{prop}
This isn't exactly immediate, because $\bar{K}$ need not be complete. However, it almost is.
...
\begin{defi}[$\pi^n$-division points]\index{$\pi^n$-division points}
  ...
  \[
    F(n) = \{x \in \bar{\mathfrak{m}}_f \mid \pi^n x= 0\} = \ker ([\pi^n]_F).
  \]
\end{defi}
...
\begin{proof}
  ...
  \]
  for some $g_n(x) \in \mathcal{O}_K[x]$. It follows that if $f_n(x) = 0$, then $|x| < 1$. So $|f_i(x)| < 1$ for all $i$.
\end{proof}
...
\[
  L_{n, \pi} = L_n = K(F(n)),
\]
which is the field of $\pi^n$ division points of $F$. We have inclusions $F(n) \subseteq F(n + 1)$ for all $n$, we obtain a corresponding inclusion of fields
\[
  L_n \subseteq L_{n + 1}.
\]
