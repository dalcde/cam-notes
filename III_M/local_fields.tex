...
We are going to start by some definitions, which you may or may not have seen before.
...
\begin{defi}[Normalized valuation]\index{normalized valuation}\index{valuation!normalized}
  Let $K$ be a DVF. The \emph{normalized valuation} $V_K$ on $K$ is the unique valuation on $K$ in the given equivalence class such that $v_K(\pi) = 1$ for any uniformizer $\pi$.
\end{defi}
If $x \in K$, then $v_K(x)$ measures how many times $\pi$ divides $x$.

\begin{eg}
  The usual valuation on $\Q_p$ is normalized, and so is the usual valuation on $k((t))$.
\end{eg}
...
\begin{defi}[Perfect ring]\index{perfect ring}
  Let $R$ be a ring of characteristic $p$. We say $R$ is \emph{perfect} if the Frobenius map $x \mapsto x^p$ is an automorphism of $R$.
\end{defi}
...
\begin{eg}
  ...
  \[
    \Z[x_i \mid i \in I] \subseteq \Z_p [x_i^{p^{-1}} \mid i \in I] \subseteq \cdots
  \]
  in the natural way.
...
\begin{prop}
  Let $A$ be a strict $p$-ring and $B$ be a ring with an element $x$ such that $B$ is $x$-adically complete and $B/xB$ is perfect of characteristic $p$. If $f: A/pA \to B/xB$ is a ring homomorphism. Then there exists a ring homomorphism $F:L A \to B$ with $f = F \mod x$.
\end{prop}
...
\begin{proof}[Proof sketch]
  If $W(R)$ and $W(R')$ are such strict $p$-rings, then the second part follows from the previous lemma.
  ...
  Finally, to show that this is unique. If $C$ is a strict $p$-ring with $C/pC \cong R \cong W(R)/pW(R)$, then the isomorphism $\bar{\alpha}: W(R)/pW(R) \to C/pC$ and its inverse $\bar{\alpha}^{-1}$ have unique lifts $\gamma: W(R) \to C$ and $\gamma^{-1}: C \to W(R)$, and these are inverses by uniqueness of lifts.
\end{proof}
...
\begin{proof}
  We need to first exhibit $W(k)$ as a subring of $R$. We know that $\id: k \to k$ lifts to a homomorphism $W(k) \to R$. The kernel is a prime ideal because $R$ is an integral domain because $R$ is an integral domain. So it is either $0$ or $p W(k)$. But $R$ has characteristic $0$. So it can't be $pW(k)$. So this must be an injection.
...
We now talk about binomial coefficients. Let $n \geq 1$. Then we know that
\[
...
\[
  \|(a_n)_n\| = \max_{n \in \N} |a_n|_p,
\]
and that $c_0$ is complete. So what Mahler's theorem gives us is an isometric isomorphism between $c_0$ and $C(\Z_p, \Q_p)$.
...
In the next section we will show that every extension of local fields can be written as an unramified extension followed by a totally ramified extension.
...
\begin{proof}
  ...
  \]
  for some $b \in \mathcal{O}_L$, by Taylor expansion around $\beta$. Since $v_L(f(\beta)) \geq 2$ and $v_L(f'(\beta)) = 0$ (since $\bar{f}$ is separable). So $v_L(f(\alpha)) = 1$. So $f(\alpha)$ is a uniformizer of $L$.
...
\begin{lemma}
  ...
  \[
    \Hom_{K-\mathrm{Alg}}(L, M) \longleftrightarrow \Hom_{k_K-\mathrm{Alg}} (k_L, k_M)
  \]
...
\begin{proof}
  ...
  So we obtain a map
  \[
    \Hom_{K-\mathrm{Alg}}(L, M) \to \Hom_{k_K-\mathrm{Alg}} (k_L, k_M)
  \]
  ...
  \[
    \begin{tikzcd}
      \varphi \ar[d, maps to] & \Hom_{K-\mathrm{Alg}} \ar[d, "\cong"] \ar[r, "\text{reduction}"] & \Hom_{k_K-\mathrm{Alg}} (k_L, k_M) \ar[d, "\cong"] & \bar{\varphi} \ar[d, maps to]\\
...
\subsection{Further ramification theory}
\subsubsection*{The unit group}
We now look at the unit group $\mathcal{O}_K^\times$ of a local field $K$. We want to define a filtration on this group. For each $s \in \Z_{\geq 1}$, we define\index{$U_k^{(s)}$}
\[
...
\begin{proof}
  ...
  \[
    (1 + \pi_K^s x)(1 + \pi_K^s y) = 1 + \pi^S(x + y + \pi^s xy) \mapsto x + y + \pi^s x + y \cong x + y \mod \pi_K,
  \]
  and the kernel is $U_K^{(s + 1)}$.
\end{proof}

\subsubsection*{The inertia group}
\begin{prop}
...
\begin{defi}[Inertia group]\index{inertia group}
  Let $L/K$ be a finite Galois extension of local fields. Then the kernel $I(L/K) = \Gal(L/T)$ of the homomorphism $\Gal(L/K) \to \Gal(k_L/k_K)$ is called the \emph{inertia group} of $L/K$.
\end{defi}

Then the inertia group is trivial iff $L/K$ is unramified. The field $T$ is sometimes called the \term{inertia field}.

\begin{lemma}
  Let $L/K$ be a finite Galois extension of local fields. Let $x \in k_L$ and $\sigma \in \Gal(L/K)$ with image $\bar{\sigma} \in \Gal(k_L/k_K)$. Then we have
  \[
    [\bar{\sigma}(x)] = \sigma([x]).
  \]
  In particular, if $\sigma \in I(L/K)$, then $\sigma([x]) = [x]$ for all $x$.
\end{lemma}
...
\subsubsection*{Higher ramification groups}

\begin{defi}[$s$th ramification group]\index{$s$th ramification group}\index{ramification group}
...
Note that we have
\[
  G_{-1}(L/K) = \Gal(L/K),\quad G_0(L/K) = I(L/K).
\]
\begin{prop}
...
\begin{proof}
  ...
  \]
  So it has the right image. To see this is independent of the choice of $\pi_L$, we let $u \in \mathcal{O}_L^\times$. Then $\sigma(u) = u + \pi_L^{s + 1}y$ for some $y \in \mathcal{O}_L$. Then any other uniformizer must be of the form $\pi_L u$, and
  \begin{align*}
  ...
  \]
  Take $\sigma \in \ker \phi \subseteq I(L/K)$. Then we have
  \[
...
Note that if $L/K$ is a finite extension of local fields, then $G_1(L/K)$ is a $p$-group, since we have an embedding
\[
...
\]
and that $k_L^\times$ has order prime to $p$, we know that $G_1(L/K)$ is the unique Sylow $p$-subgroup of $G_0(L/K)$. Here $G_1(L/K)$ is called the \term{wild inertia group}, and the quotient $G_0(L/K)/G_1(L/K)$ is the \term{tame quotient}.
...
\begin{proof}
  ...
  \]
\end{proof}
What's more difficult and interesting is quotients, namely going from $M/K$ to $L/K$.

Let $L/K$ be a finite Galois extension of local fields, and pick $\alpha \in \mathcal{O}_L$ such that $\mathcal{O}_L = \mathcal{O}_K[\alpha]$. We set
\[
  i_{L/K}(\sigma) = v_L(\sigma(\alpha) - \alpha)
\]
for $\sigma \in \Gal(L/K)$.

We want to show that this is independent of the choice of $\alpha$. We claim that
\[
  i_{L/K}(\sigma) = \min_{x \in \mathcal{O}_L} v_L(\sigma(x) - x).
\]
Indeed, if we write
\[
  g(x) = \sum_{i = 0}^m b_i x^i \in \mathcal{O}_K[x],
\]
then we have
\[
  v_L(\sigma g(\alpha) - g(\alpha)) = v_L\left( \sum_{i = 1}^n b_i (\sigma(\alpha)^i - \alpha^i)\right) \geq v_L(\sigma(\alpha) - \alpha),
\]
using the fact that $\sigma(\alpha) - \alpha \mid \sigma(\alpha)^i - \alpha^i$ for all $i$. Since every element of $\mathcal{O}_L$ can be written as a polynomial expression of $\alpha$ by assumption, our claim follows.

Then we can rewrite the definition of $G_s$ as
\[
  G_s(L/K) = \{\sigma \in \Gal(L/K): i_{L/K}(\sigma) \geq s + 1\}.
\]
Note that if $M/L/K$ are finite Galois extensions of local fields, then $\mathcal{O}_M = \mathcal{O}_K[\alpha]$ implies $\mathcal{O}_M = \mathcal{O}_L[\alpha]$. So for $\sigma \in \Gal(M/L)$, we have
\[
...
\begin{prop}
  ...
  \[
    i_{L/K}(\sigma) = e_{M/L}^{-1} \sum_{\tau \in \Gal(M/K), \tau|_L = \sigma} i_{M/K}(\tau).
  \]
\end{prop}
...
\begin{proof}
  ...
  In other direction, we pick $f \in \mathcal{O}_K[x]$ such that $f(\alpha) = \beta$. Then $f(\alpha) - \beta = 0$. This implies that the polynomial $f(\alpha) - \beta$ divides the minimal polynomial of $\alpha$ in $\mathcal{O}_L[x]$. So we have
  \[
    f(x) - \beta = F(x) h(x)
  \]
...
Let $L/K$ be a finite Galois extension of local fields. Define\index{$\eta_{L/K}$}
\[
...
\]
Then this $\eta$ is a piecewise linear function where all the break points are integers.
\begin{prop}
...
\begin{proof}
  We denote the RHS by $\theta(s)$. We look at the function
  \[
    s \mapsto \min (i_{L/K}(\sigma), s + 1).
  \]
  This looks like
  \begin{center}
    \begin{tikzpicture}
      ...
      \node [circ] at (2, 2) {};
      \node [below] at (2, 0) {$i_{L/K}(\sigma) - 1$};
    \end{tikzpicture}
  \end{center}
  So we see that $\theta(s)$ is piecewise linear and the break points are integers. SO to see they are the same, we see that they agree at a point, and that they have equal derivatives. We have
  \[
    \theta(0) = \frac{\text{number of }\sigma \in G: i_{L/K}(\sigma) \geq 1}{e_{L/K}} - 1 = 0 = \eta_{L/K}(0),
  \]
  since the numerator is the size of the inertia group.

  If $s \in [-1, \infty) \setminus \Z$, then
  \begin{align*}
    q'(s) &= e_{L/K}^{-1} (\text{number of }\sigma \in G: i_{L/K(\sigma) \geq s + 1}) \\
...
\begin{thm}[Herbrand's theorem]\index{Herbrand's theorem}
  ...
  Then we have
  \[
    \frac{G_s(M/K)H}{H} = G_t(L/K). % state this properly
  \]
...
\begin{proof}
  ...
  \begin{enumerate}
    \item If $i_{M/K}(g) \geq m$, then the above shows that $i_{M/K}(\tau g) \geq m$. But we also know that it is bounded above by $m$. So $i_{M/K}(\tau g) = m$. So our claim holds.
    \item If $i_{M/K}(g) < m$, then the above inequality is in fact an equality as the two terms have different valuations. So our claim also holds.
  \end{enumerate}
  So done.
\end{proof}
...
\begin{defi}[Upper numbering]\index{upper numbering of ramification group}\index{ramification group!upper numbering}\index{lower numbering of ramification group}\index{ramification group!lower numbering}
  Let $L/K$ be a Galois extension of local fields. Then the \emph{upper numbering} of the ramificaation groups of $L/K$ is defined by
  \[
...
To rephrase our previous theorem using the upper number, we need a little lemma
\begin{lemma}
...
\begin{proof}
  ...
  \]
  Since $\eta_{M/K}$ and $\eta_{L/K} \circ \eta_{M/L}$ agree at $0$, and both take value $0$, we know that the functions must agree everywhere. So done.
\end{proof}
...
\begin{eg}
  ...
  \]
  It is an exercise on the example sheet to show that this is indeed irreducible. So $K/\Q_p$ is a Galois extension of degree $p^{n - 1}(p - 1)$. Moreover, it is totally ramified by Question 6 on example sheet 2. Moreover,
  \[
    \pi = \zeta_{p^n} - 1
  \]
  ...
  \[
    \left(\frac{\Z}{\zeta_p^n}\right)^\times \to \Gal(L/\Q_p)
  \]
  ...
  \begin{align*}
    ...
    &= v_K(\zeta_{p^n}^{m - 1} - 1)
  \end{align*}
  since $\zeta_{p^n}$ is a unit. If $m = 1$, then this thing is inifinty. If it is not $1$, then $\zeta_{p^n}^{m - 1}$ is a primitive $p^{n - k}$th root of unity for the maximal $k$ such that $p^k \mid m- 1$. So by Q6 on example sheet 2, we have
  \[
  ...
  \]
  Now $m \equiv 1 \bmod p^k$ iff $\sigma_m(\zeta_{p^k}) = \zeta_{p^k}$. So
  \[
  ...
  \[
    \G_s(K/\Q_p) = 1.
  \]
  The function we integrate to get $\eta$ looks something like this:
  \begin{center}
    \begin{tikzpicture}
      \draw [->] (-1, 0) -- (7, 0);
      \draw [->] (0, -1) -- (0, 3);

 3 \draw (0, 2) -- (2, 2) node [pos=0.5, above] {$\frac{1}{p - 1}$} node [circ] {};
...
Local class field theory is the study of abelian extensions of local fields, ie. a Galois extension whose Galois group is abelian. It turns out that the best way of formulating this theory is to not only use finite extensions, but infinite extensions as well. So we need to begin with some infinite Galois theory.
...
\begin{defi}[Krull topology]\index{Krull topology}
  Let $M/K$ be a Galois extension. We say that $U \subseteq \Gal(L/K)$ is open if for every $\sigma \in U$, we can find a finite subextension $L/K$ of $M/K$ such that $\sigma \Gal(M/L) \subseteq U$. These sets form the open sets of a topology of $\Gal(M/K)$ called the \emph{Krull topology}. Then $\Gal(M/K)$ is a topological group with respect to the Krull topology.
\end{defi}
...
\begin{proof}
  We show that $\Gal(M/L)$ is closed? We can write
  \[
    L = \bigcup_{L' \subseteq L, L'/K\text{ finite}} L'.
  \]
  Then we have
  \[
    \Gal(M/L) = \bigcap_{L' \subseteq L, L'/K\text{ finite}} \Gal(M/L'),
  \]
  and each $\Gal(M/L')$ is open, hence closed. So the whole thing is closed.
\end{proof}

\subsection{Unramified extensions and Weil group}
We first define what it means for an infinite extension to be unramified or totally ramified.
\begin{defi}[Unramified extension]\index{unramified extension}
...
In general, an algebraic extension $M/K$ has a maximal unramified subextension
\[
  T = T_{M/K}/K
\]
which is Galois.
...
\begin{defi}[Arithmetic Frobenius]\index{arithmetic Frobenius}
  If $L/K$ is a finite unramified extension of local fields with $q = |k_K|$, then we had
  \[
    \begin{tikzcd}
      \Gal(L/K) \ar[r, "\sim"] & \Gal(k_L/k_K)
    \end{tikzcd},
  \]
  and $\Gal(k_L/k_K)$ is a cyclic group generated by $x \mapsto x^q$. So $\Gal(L/K)$ is cyclic with a canonical generator $\Frob_{L/K}$, which is the lift of $x \mapsto x^q$. This is called the \emph{(arithmetic) Frobenius} element of $L/K$.
\end{defi}
...
\]
So we get an element $\Frob_{M/K} \in \Gal(M/K)$. We claim that it is the unique lift of $x \mapsto x^{|k_K|}$.

Indeed if $K$ is a local field and $M/K$ is unramified, then the reduction map $\Gal(M/K) \to \Gal(k_M/k_K)$ commutes with the isomorphism $\Gal(M/K) \cong \varprojlim_{L/K} \Gal(L/K)$, ie. the following diagram commutes:
\[
  \begin{tikzcd}
    \Gal(M/K) \ar[r, "\text{reduction}"] \ar[d, "\sim"] & \Gal(k_M/k_K) \ar[d, "\sim"]\\
    \varprojlim_{L/K} \Gal(L/K) \ar[r, "\text{reduction}", "\sim"'] & \varprojlim_{L/K} \Gal(k_L/k_K)
  \end{tikzcd}
\]
The bottom map comes from the fact that finite unramified subextensions correspond to subextensions of the residue field. So these isomorphisms compose to tell us that the reduction map $\Gal(M/K) \to \Gal(k_M/k_K)$ is in fact an isomorphism. In particular, by tracing through the diagram, we see that the Frobenius $\Frob_{M/K}$ is the lift of $x \mapsto x^{|k_K|}$ on $k_M$.

Note that the Galois group $\Gal(M/K)$ need not be generated by the Frobenius in the case of an infinite extension. However, we can define
...
\begin{prop}
  Let $K$ be a local field, and $M/K$ Galois. Then $W(M/K)$ is dense in $\Gal(M/K)$. In other words, for any finite Galois subextension $L/K$ of $M/K$, the restriction map $W(M/K) \to \Gal(L/K)$ is surjective.
...
\begin{proof}
  We first prove density. Let $L/K$ be a subextension. We let $T = T_{M/K}$. Then $T_{L/K} = T \cap L$. Then we have a diagram
  \[
    \begin{tikzcd}
  ...
  \begin{align*}
    ...
    &\cong \Gal(L/K).
  \end{align*}
  The only non-trivial part in this chain is the assertion that $W(M/K) \Gal(M/L) = \Gal(M/K)$, ie. that $W(M/K)$ hits every coset of $\Gal(M/L)$, which follows from density.
\end{proof}
...
\begin{thm}[Local Artin reciprocity]
  ...
  \end{enumerate}
  Moreover, if $M/K$ is finite, then
  \[
    \Art_M(x)|_K^{\mathrm{K^{ab}}} = \Art_K(N_{M/K}(x))
  \]
  for all $x \in M^\times$ and $\Art_K$ induces an isomorphism
  \[
    K^\times/N_{M/K}(M^\times) \to \Gal(M\cap K^{\mathrm{ab}} / K).
  \]
\end{thm}
...
\begin{proof}
  By the previous theorem, we may wlog $L/K$ abelian by replacing with $L \cap K^{\mathrm{ab}}$. The $\Leftarrow$ direction is clear.
...
\begin{thm}
  Let $L/K$ and $M/K$ be finite abelian field extensions of a local field $K$. Then
  \[
...
\begin{thm}[Existence theorem]\index{existence theorem}
  For every open subgroup $H \subseteq K^\times$ of finite index, there is a unique finite abelian $L/K$ such that $H = N(L/K)$.
\end{thm}

Proofs of these are on the fourth example sheet.

In summary, what the Artin map gives us is a bijection
\[
  \begin{tikzcd}
    \{\text{open finite index subgroups of $K^\times$ }\} \ar[r, leftrightarrow] & \{\text{finite abelian extensions of $L/K$}\}\\
    H \ar[r, maps to] & (K^{\mathrm{ab}})^{\Art_K(H)}\\
    N(L/K) & L/K \ar[l, maps to]
  \end{tikzcd}.
\]
So this gives us a nice characterization of the finite abelian extensions of $K$ by the ``internal arithmetic'' of the field $K$.

Why is this called ``class field theory''? Usually, we call the field corresponding to the subgroup $H$ the \term{class field} of $H$. Historically, the first type of theorems like this are proved for number fields. The groups that appear on the left would be different, but in some cases, they are the class group of the number field.
...
There are many ways we can approach local class field theory. The approach we use, using Lubin-Tate theory, is the most accessible one. Details can be found in Iwasawa's \emph{Local Class Field Theory} and Yoshida's \emph{Local Class Field Theory via Lubin-Tate theory}. % check spelling

Another possible approach is via Galois cohomology. This is how it is done by Serre and Neukirch. This, however, relies on more advanced machinery.
...
\begin{proof}
  ...
  \[
    v_K(N(L/K)(x)) = n w(x) = f_{L/K}v_L(x).
  \]
  So we have a surjection
  \[
    \begin{tikzcd}
      \frac{K^\times}{N(L/K)} \ar[r, "v_K", two heads] & \frac{\Z}{f_{L/K}\Z}
    \end{tikzcd}.
  \]
...
Now we fix a uniformizer $\pi_K$. Then we have a topological group isomorphism $K^\times \cong \bra \pi_K\ket \times \mathcal{O}_K^\times$. To construct $K^{\mathrm{ab}}$, we need extensions with norm groups $\bra \pi_K^m\ket \times U_K^{(n)}$ for $m, n \in \Z_{\geq 0}$. It suffices to consider the $m$ and the $n$ separately, ie. $\bra \pi_K^m\ket \times \mathcal{O}_K^\times$ and $\bra \pi_K\ket \times U_K^{(n)}$, as intersection of groups correspond to taking compositor fields.
...
\begin{eg}
  ...
  \[
    L_n = \Q_p (\zeta_{p^n},
  \]
  where $\zeta_{p^n}$ is the primitive $p^n$th root of unity. Then this is a field with norm group $\bra p\ket \times(1 + p^n\Z_p)$.
  ...
  \begin{align*}
    ...
    &= \Z_p^\times.
  \end{align*}
  This is the Artin map of $\Q_p$ restricted to $\Z_p^\times$.

  Explicitly, if $m \in \Z_p^\times$, we write
  \[
    m = a_0 + a_1p + \cdots,
  \]
  ...
  \[
    \Art_{\Q_p}(m) = \sigma_m,
  \]
  where $\sigma_m$ is determined by
  \[
  \sigma_m(\zeta_{p^n}) = \zeta_{p^n}^{a_0 + a_1 p + \ldots + a_{n - 1}p^{n - 1}} = \lim_{k \to \infty} \zeta_{p^n}^{a_0 + a_1 p + \ldots + a_k p^k} \text{ ``$=$'' } \zeta_{p^n}^m,
  \]
  since if we add higher powers, we are just adding $p^n$th powers of $\zeta_{p^n}$, which is $1$. We now have a diagram
  \[
    \begin{tikzcd}
      \Q_p^\times \ar[d, "\cong"] \ar[r, "\Art_{\Q_p}"] & W(\Q_p^{\mathrm{ab}}/\Q_p) = W(\Q_p^{\mathrm{ur}} \Q_p(\zeta_{p^\infty} / \Q_p) \ar[d, "\text{restriction}", "\sim"']\\
      \bra p\ket \Z_p^\times \ar[r] & W(\Q_p^{\mathrm{ur}} / \Q_p) \times \Gal(\Q_p(\zeta_{p^\infty}) / \Q_p)
    \end{tikzcd}
  \]
...
\begin{thm}[Local Kronecker-Weber theorem]\index{Local Kronecker-Weber theorem}
  ...
  \begin{align*}
    ...
    \Q_p^{\mathrm{ur}} &= \bigcup_{n \in \Z_{\geq 1}, (n, p) = 1} \Q_p(\zeta_n).
  \end{align*}
\end{thm}
...
\begin{defi}[Higher ramification groups]\index{Higher ramification groups}
  ...
  \[
    G^s(M/K) = \{ \sigma \in \Gal(M/K): \sigma|_L \in G^s(L/K)\text{ for all finite Galois subextension $M/K$}\}.
  \]
\end{defi}
...
\[
  \begin{cases}
    \bra p\ket \times \Z_p^\times / \bra p^n \ket \times(1 + p^m \Z_p) & s = -1\\
    \bra p^n\ket \times \Z_p^\times / \bra p^n \ket (1 +p^m \Z_p) & -1 < s \leq 0\\
    \bra p^n\ket \times (1 + p^k \Z_p) / \bra p^n \ket \times (1 + p^m \Z_p) & k - 1 < s \leq k \leq m - 1\\
    1 7 s > m - 1
  \end{cases}
\]
under the Artin map.
...
\begin{thm}
  We have
  \[
    G^s(\Q_p^{\mathrm{ab}}/\Q_p) = \Art_{\Q_p}(1 + p^n \Z_p) = \Art_{\Q_p} (U^{(n)}),
  \]
  where $n$ is chosen such that $n - 1< s \leq n$, $n \in \Z_{\geq 0}$.
\end{thm}
