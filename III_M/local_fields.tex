...
Proving this theorem will require some preparation.

We will work with discrete valuation rings instead of discretely valued field (mostly up to personal taste), but the concept of a DVR is purely algebraic, and the algebraic structure of a DVR is equivalent to the ``topological/algebraic'' structure structure of a DVF. Also, when working with DVF's/DVR's, it is very common to use valuations rather than the absolute values.
...
\begin{lemma}
  Let $R$ be a ring, and let $x \in R$. Assume that $R$ is $x$-adically complete, ie. the natural map $R \to \varprojlim R/x^n R$ is an isomorphism, and that $R/xR$ is perfect of characteristic $p$. Then there is a unique map $[-]: R/xR \to R$ such that
  \[
    [a] \equiv a \mod x
  \]
  and
...
\begin{proof}
  Let $a \in R/xR$. Then there exists a unique $a^{p^{-n}} \in R/xR$ for all $n$ since $R/xR$ is perfect. Now we lift arbitrarily: take $\alpha_n \in R$ such that $\alpha_n \equiv a^{p^{-n}} \mod x$. Put $\beta_n = \alpha_n^{p^n}$. The claim is that
  \[
    \lim_{n \to \infty}\beta_n
  \]
  exists and is independent of the choices. We call this $[a]$.
  ...
  Since we are working in a discrete valuation ring, it suffices to show that $\beta_{n + 1} - \beta_n \to 0$ $x$-adically. Indeed, we have
  \[
  ...
  \]
  Using the fact that $R/xR$ has characteristic $p$, the binomial theorem give
  \[
    (\alpha_{n + 1}^p)^{p^n} \equiv \alpha_n^{p^n} \mod x^{n + 1}.
  \]
  So $\beta_{n + 1} - \beta_n \in x^{n + 1} R$. So $\lim_{n \to \infty} \beta_n$ exists.

  To see $[a] = a\mod x$, we just have to note that
  \[
    \lim_{n \to \infty} \alpha_n^{p^n} \equiv \lim (a^{p^{-n}})^{p^n} = \lim a = a \mod x.
  \]
  We now want to prove multiplicativity. If we pick another element $b \in R/xR$, with $\gamma_n \in R$ lifting $b^{p^{-n}}$ for all $n$, then $\alpha_n \gamma_n$ lifts $(ab)^{p^{-n}}$. So
  \[
    [ab] = \lim\alpha_n^{p^n} \beta_n^{p^n} = \lim \alpha_n^{p^n} \lim \gamma_n^{p^n} = [a][b].
  \]
...
\begin{defi}
  Let $R = \Z_p$. Then $[-]: \F_p \to \Z_p$ satisfies
  \[
    [x]^{p - 1} = [x^{p - 1}] = [1] = 1.
  \]
...
\begin{defi}[Teichm\"uller map]\index{Teichm\"uller map}\index{Teichm\"uller lift}\index{Teichm\"uller representative}
  The map $[-]: R/xR \to R$ is called the \emph{Teichm\:uller map/lift}. $[x]$ is called the \emph{Teichm\"uller lift/representative} of $x$.
\end{defi}

We can now prove our theorem.
\begin{proof}
  Let $K$ be a complete DVF. We want to prove that $\mathcal{O}_K \cong k_K[[T]]$. We know $\mathcal{O}_K$ has characteristic $p$. So $[-]: k_K \to \mathcal{O}_K$ is an injective ring homomorphism. We choose a uniformizer $\pi \in \mathcal{O}_K$, and define
  \[
  ...
  \]
  This is then a bijection by one of the basic properties of complete DVF's.
\end{proof}
...
\begin{eg}
  ...
  \]
  Here the union on the right is taken by treating $\Z[x_i \mid i \in I] \subseteq \Z_p [x_i^{p^{-1}} \mid i \in I] \subseteq \cdots$ in the natural way.
  ...
  \]
\end{eg}
We should think of $\F_p [x_i^{p^{-\infty}}\mid i \in I]$ as a ``universal'' perfect ring. Recall that polynomial rings are ``universal'' in the sense that for any ring, we can find some polynomial ring that surjects to it. This is the corresponding version when we require that the rings are perfect.
...
\begin{proof}[Proof sketch]
  ...
  But we know
  \[
    J + pA = \left\{\sum_ [a_n] p^n \mid a_0 \in I\right\}.
  \]
  So we have
  \[
...
\begin{proof}
  ...
  \]
  So $x$ is a unit. Now, looking at Teichm\"uller expansions and factoring out multiple fo $p$, any non-zero element $z$ can be written as $p^n u$ for a unique $n \geq \Z_{\geq 0}$ and $u \in A^\times$. Then we have
  \[
    v(z) =
    \begin{cases}
      n & z \not= 0\\
...
\begin{proof}
  ...
  \]
  Looking at valuations, one sees that $1, \pi, \pi, \cdots, \pi^{e - 1}$ are linearly independent onver $W(k)$. So we can form
  \[
...
\begin{proof}
  ...
  \]
  because the binomial coefficients $\binom{p^k}{i}$ are divisible by $p$ for $i \not= 0, p^k$. Note that we do have a negatie sign in front of $f(x)$ because $(-1)^{p^k}$ is $-1$ as long as $p$ is odd, and $1 = -1$ if $p = 2$.
...
\begin{prop}
  The map $f \mapsto (a_n(f))_{n = 0}^\infty$ defines an injective non-decreasing linear map $C(\Z_p, \Q_p) \to C_0$.
\end{prop}
...
\begin{proof}
  ...
  \]
  So the map is non-decreasing. Linearity follows from linearity of $\Delta$. To finish, we have to prove injectivity.
