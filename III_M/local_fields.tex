...
\begin{proof}
  Let $G = \Gal(M/K)$. We choose $\tau \in G$ such that
  \[
...
% m = i_{M/K}(\tau).
% \]
  We put $\sigma = \tau|_L$. We claim that
  \[
    i_{L/K}(\sigma) - 1 = \eta_{M/L}(i_{M/K}(\tau) - 1).
  \]
  We pick $\alpha \in L$ such that $\mathcal{O}_L = \mathcal{O}_M[\alpha]$. Then for any $g \in H$, we have
  \begin{align*}
    i_{M/K}(\tau g) &= v_M(\tau g(\alpha) - \alpha) \\
    &= v_M(\tau\gamma(\alpha) - g(\alpha) + g(\alpha) - \alpha) \\
    &\geq \min(v_M(\tau g(\alpha) - g(\alpha)), v_M(g(\alpha) - \alpha))\\
    &= \min(i_{M/K}(\tau), i_{M/K}(g)))\\
    &= \min(m, i_{M/K}(g)).
  \end{align*}
%
% Indeed, if $g \in G_{m - 1}(M/L) \subseteq H$, then by definition
% \[
% i_{M/K}(g) \geq m.
% \]
% Then
...
% by assumption on $g$.
%
% If $g \in H \setminus G_{m - 1}(M/L)$, then we have
% \[
% i_{M/K}(g) < m,\quad i_{M/K}(\tau g) = i_{M/K}(g),
% \]
% by the above calculations. Thus
  Now using our previous propsoition, we obtain
  \[
    i_{L/K}(\sigma) = e^{-1}_{M/K} \sum_{g \in H} \min(i_{M/K(\tau)}, i_{M/K}(g)).
  \]
  So
  \[
    \eta_{M/K}(i_{M/K(\tau)} - 1) = \left(e_{M/L}^{-1}\sum_{g \in H} \min(i_{M/K}(g) , i_{M/K(\tau)})\right) - 1 = i_{L/K}(\sigma) - 1.
  \]
  ...
  \begin{align*}
    ...
    \intertext{Since $\eta$ is strictly increasing, we have}
