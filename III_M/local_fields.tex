...
\begin{prop}
  Let $K$ be a local field, and $M/K$ Galois. Then $W(M/K)$ is dense in $\Gal(M/K)$.
...
\begin{proof}
  We first prove density. If we look at the definition of the inverse limit topology, density boils down to the property that for every finite Galois subextension $L/K$ of $M/K$, we have that $W(M/K)$ surjects to $\Gal(L/K)$ via restriction.

  We let $T = T_{M/K}$. Then $T_{L/K} = T \cap L$. Then we have a diagram
  \[
    \begin{tikzcd}
  ...
  \[
    \sigma \in W(M/L) \Leftrightarrow \sigma|_{T_{M/L}} \in \Forb_{T_{M/L}/L}^\Z \Leftrightarrow \sigma|_{T_{M/K}/K} \in \Frob_{T_{M/K}/K}^\Z \Leftrightarrow \sigma \in W(M/K).
  \]
  ...
  \[
    \frac{W(M/K)}{W(M/L)} = \frac{W(M/K)}{W(M/K) \cap \Gal(M/L)} \cong \frac{W(M/K)\Gal(M/L)}{\Gal(M/L)} = \frac{\Gal(M/K)}{\Gal(M/L)} \cong \Gal(L/K).
  \]
...
\begin{proof}
  ...
  \[
    \frac{k^\times}{N(L/K)} \cong \Gal(M/K) \cong k^\times N(M/K).
  \]
