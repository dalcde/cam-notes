...
\]
with $f \in L^2(U)$. We define the \term{formal adjoint} by
\[
...
Note make this argument for weak solutions, we have to contend with the fact that if $u \in H^1(U)$, then $\D^2 u$ doesn't necessarily exist even in a weak sense. Thus, we need to use \term{difference quotients}.
...
\begin{proof}
  ...
  \]
  Proceeding as in the proof of Garding's inequality, we deudce that
  \[
    \|\D u\|_{L^2(W)} \leq C (\|f\|_{L^2(U)} + \|u\|_{L^2(U)},
  \]
  and then we are done.
\end{proof}
This is an important result in the theory of PDEs. The proof is kind-of ugly, using difference equatinos, but once we have this result, it is very powerful.
...
\begin{defi}[Hyperbolic PDE]\index{hyperbolic operator}\index{hyperbolic PDE}
  ...
  This equation is \emph{hyperbolic} if the \term{principal symbol}
  \[
...
\]
Hyperbolicity then says the operator on the right is elliptic for each $t$ (or rather, the negative of the right hand side).
...
\begin{center}
  \begin{tikzpicture}
    ...
    \draw (0, 4) ellipse (1.5 and 0.3);

    \node [left] at (-1.5, 0) {$t = 0$};
    \node [left] at (-1.5, 4) {$t = T$};
...
\]
Let's look at the following \term{initial boundary value problem} (IVBP)\index{IBVP} for the wave equation: Suppose $u \in C^2(\bar{U}_T)$ satisfies
\begin{align*}
...
\end{align*}
To see why this is a sensible thing to do, let us do some \term{energy estimates}. We multiply by $u_t$ and integrate over $U_t$ to get
\begin{align*}
...
Estimates like this that control a solution without needing to construct it are known as \term{a priori estimates}. These are often crucial to establish the existence of solutions (c.f. Garding).
...
\begin{defi}[Weak solution]\index{weak solution!hyperbolic PDE}
