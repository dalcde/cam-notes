...
We are nearly ready to tackle an actual PDE. What we are going to look at now is the Cauchy--Kovalevskaya theorem for first-order systems.

We start by looking at a class of problems similar to the Cauchy problem for ODEs. We consider $\R^n$ with coordinates $(x^1, \ldots, x^n) = (x', x^n)$, where $x' = (x_1, \ldots, x^{n - 1}) \in \R^{n - 1}$. We consider a system of equations for an unknown $\mathbf{u}(x) \in \R^m$. We seek a solution to
\[
  \mathbf{u}_{x^n} = \sum_{j = 1}^{n - 1}bb(\mathbf{u}, x') \mathbf{u}_{x_j} + \mathbf{C}(\mathbf{u}, x')
\]
on $|x'|^2 + (x^n)^2 < r$, subject to $\mathbf{u} = 0$ when $x^n = 0$.

We assume the functions $B: \R^m \times \R^{n - 1} \to \Mat(m \times m)$ and $\mathcal{C}: \R^m \times \R^{n - 1} \to \R^m$ are given, real analytic functions. Note that both are independent of $x^n$.

Note that we assumed $B$ and $\mathcal{C}$ do not depend on $x^n$, but this is not a restriction, since we can always introduce a new variable $u^{m + 1} = x^n$, and enlarge the target space.
