...
\]
\begin{defi}[Non-characteristic surface]\index{non-characteristic surface}
...
These are spaces based on classical continuity and differentiability. Suppose $U \subseteq \R^n$ is an open set, we write $u \in C^k(U)$\index{$C^k(U)$} if $u: U \to \R$ is $k$-times differentiable and the partial derivatives $\D^\alpha u : U \to \R$ are continuous for $|\alpha| \leq k$. We write $u \in C^k(\bar{U})$\index{$C^k(\bar{U})$} if $u \in C^k(U)$ and $\D^\alpha u$ are bounded and uniformly continuous on $U$.
...
For $0 < \gamma \leq 1$, we say that $u \in C^{0, \gamma}(\bar{U})$ if $u \in C^0(\bar{U})$ and $u$ is \term{H\"older continuous} with index $\gamma$, i.e.\ there exists $C \geq 0$ such that
\[
...
\begin{thm}[Global approximation]
