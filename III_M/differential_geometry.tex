...
The key insight is that these notion of differentiability, derivatives etc. are \emph{local} properties. To know if a function is differentiable at a point $p$, we only need to know how the function behaves near $p$, and similarly such local information tells us how to compute derivatives. The reason we can do calculus on a sphere is because the sphere looks \emph{locally} like $\R^n$. Therefore we can make sense of calculus on a sphere.

Thus, what we want to do is to study calculus on things that look \emph{locally} like $\R^n$, and these are known as manifolds. Most of the time, our definitions from usual calculus on $\R^n$ transfers directly to manifolds. However, sometimes the global properties of our manifold will give us some new exciting things.

In fact, we've already seen such things when we did IA Vector Calculus. If we have a vector field $\R^3 \to \R^3$ whose curl vanishes everywhere, then we know it is the gradient of some function. However, this is no longer true if we consider such a vector field on $\R^3 \setminus \R$ instead, then this is no longer true, because the space is no longer connected! Here the global topology of the space gives rise to interesting phenomena we do not see in a local level.
...
\begin{eg}
  ...
  \end{align*}
  It is easy to show that gives a bijection to the open disk in $\R^3$. We similarly define the other $\varphi_i^{\pm}$. These then give us an atlas of $S^2$.
\end{eg}
...
\begin{proof}
  ...
  \[
    D(\varphi_1 \varphi_2^{-1})(\varphi(p)): \R^m \to \R^n
  \]
...
Now if we had such a partition of unity, we can pick an inner product on each $U_\alpha$, say $q_\alpha(\ph, \ph)$, and then we can define an inner product on the whole space by
\[
...
\end{defi}
This is a rather technical condition, rather than ``a subset that is also a manifold under the inherited smooth structure''. The two definitions are indeed equivalent, but picking this formulation makes it easier to proving things about it.
...
Sometimes, we like to think of a subobject not as a subset, but as the inclusion map $i: S \to M$ instead. However, when we are doing topology, there is this funny problem that a continuous bijection need not be a homeomorphism. So if we define submanifolds via inclusions maps, we get a weaker notion known as an \emph{immersed manifold}.
...
\]
into a manifold. There is then a natural map $\pi: TM \to M$ sending $v_p \in T_p$ to $p$ for each $p \in M$, and this is smooth. We can then define the smoothness of $v_p$ using the usual notion of smoothness of maps between manifolds.
...
\begin{enumerate}
  ...
  \item If $F \in C^\infty (M, N)$ and $X$ is a vector field on $N$, then we \emph{cannot} obtain a vector field on $N$ by $\D F(X)$, since $F$ might not be injective. If $F(p_1) = F(p_2)$, we need not have $\D F(X(p_1)) = \D F(X(p_2))$.
\end{enumerate}
...
\begin{defi}[$\Der(C^\infty(M))$]\index{$\Der(C^\infty(M))$}
  ...
  \]
  This is an $\R$-vector space, and in fact an $C^\infty(M)$ module.
\end{defi}
...
\begin{eg}
  ...
  \]
\end{eg}
We see that to find an integral curve, all we are doing is just solving ordinary differential equations. We know that all ODE's have smooth and unique solutions, and they have all the nice properties we can hope for. So we are going to get nice corresponding results for integral solutions. However, sometimes funny things happen.
...
\begin{eg}
  ...
  \]
  on $\R^2 \setminus \{0\}$. Then for any point $p = (x, y)$, if $y \not= 0$, then then $I_p = \R$, but if $y = 0$ and $x < 0$, then $I_p = (-\infty, -x)$. Similarly, if $y = 0, x > 0$, then $I_p = (-x, \infty)$.
  \begin{center}
    \begin{tikzpicture}
...
\begin{defi}[Complete vector field]\index{complete vector field}
  A vector field is complete if $I_p = \R$ for all $p \in M$.
\end{defi}
...
\begin{thm}\index{$\Theta_t(p)$}
  ...
  \[
    \Theta_t(p) = \gamma_p(t),
  \]
  where $\gamma_p$ is the maximal integral curve of $X$ through $p$ with $\gamma(0) = p$. Then $\Theta$ is a function smooth in $M$ and $t$, and
  \[
...
Thus, instead of talking about $\Vect^L(G)$, we talk about $T_e G$ instead, because it seems less scary. This isomorphism gives $T_eG$ the structure of a Lie algebra.

\begin{defi}[Lie algebra of Lie group]\index{Lie algebra of Lie group}
...
\begin{eg}
  ...
  \[
    X_\xi|_A = L_A(\xi) = \sum_{ij} (A \xi)_j^i \left.\frac{\partial}{\partial U_j^i}\right|_A = \sum A_k^i \xi_j^k \left.\frac{\partial}{\partial U_j^i} \right|_A
  \]
  So we have
  \[
  ...
  So we have
  \[
    [X_\xi, X_\eta] = \left[\sum_{i,j,k} U_k^i \xi_j^k \frac{\partial}{\partial U_j^i},\sum_{p, q, r} U_q^p \eta_r^p \frac{\partial}{\partial U_r^p}\right].
  \]
...
Tensor products is a very important concepts in Linear Algebra. It is something that is taught in no undergraduate courses and assumed knowledge in all graduate courses. For the benefit of the students, we will give a brief introduction to tensor products.
...
Our aim is to consider spaces $T_p M \otimes T_p M, \ldots, \Lambda^p T_p M$ etc as $p$ varies, ie. construct a ``tensor bundle'' for these tensor products, similar to how we constructed the tangent bundle. Thus, we need to come up with a general notion of vector bundle.
...
\[
  \{s_i \oplus 0, 0 \oplus \tilde{j}: i = 1, \cdots, r; j = 1, \cdots, \tilde{r}\}
\]
...
We are now going to restrict our focus to a special kind of tensors, known as \emph{differential forms}. Recall that in $\R^n$ (as a vector space), an alternating $n$-linear map tells us the signed volume of the parallelopiped spanned by $n$ vectors. In general, a differential $p$-form is an alternating $p$-linear map on the tangent space at each point, so it tells us the volume of an ``infinitesimal $p$-dimensional parallelopiped''.
...
\begin{eg}
  ...
  \begin{align*}
    ...
    &= \left(\frac{\partial a}{\partial x}\;\d x + \frac{\partial a}{\partial y}\;\d y\right)\wedge \d x + \left(\frac{\partial b}{\partial x}\;\d x + \frac{\partial b}{\partial y}\right)\wedge \d y\\
    &= \left(\frac{\partial b}{\partial y} - \frac{\partial a}{\partial x}\right)\;\d x \wedge \d y.
  \end{align*}
...
The actual proof will be divided into two parts. The first part is a purely algebraic result known as the \emph{snake lemma}, and the second part is a differential-geometric part that proves that what we have do satisfy the hypothesis of the snake lemma.
...
A map between cochain complexes
...
\begin{proof}[Proof of Mayer-Vietoris]
  ...
  Indeed, let $\{\varphi_U, \varphi_V\}$ be partitions of unity subordinate to $\{U, V\}$. Let $\omega \in \Omega^p(U \cap V)$. We set $\sigma_U \in \Omega^p(U)$ be
  \[
    \sigma_U =
    \begin{cases}
...
Informally, an orientation on a vector space $V$ is a choice of collection of ordered basis that we declare to be ``oriented''. If $(e_1, \cdots, e_n)$ is an oriented bases, then changing the sign of one of the $e_i$ changes orientation, while scaling by a positive multiple does not. Similarly, swapping two elements in the basis will induce a change in orientation.
...
\begin{defi}[Orientation of manifold]\index{orientation!manifold}\index{manifold!orientation}
  An \emph{orientation} of a manifold is defined to be an equivalence class of elements $\omega \in \Omega^n(M)$ that is nowhere vanishing, under the equivalence relation $ \omega \sim \omega'$ if there is some smooth $f: M \to \R_{> 0}$ such that $\omega = f \omega'$.
\end{defi}
...
\begin{thm}
  Given a parametrization of $M$, a parametrization $S_i$ of $M$, and an $\omega \in \Omega^n(M)$ with compact support, we have
  \[
...
We can try the naive definition
\[
...
This doesn't really solve our problem, though. The above lets us differentiate sections of the whole bundle $E$ along vector fields defined along an everywhere-defined vector field. However, what we want is to differentiate a path in $E$ along a vector field defined on that path only.
...
\begin{proof}[Proof of previous lemma]
  ...
  \]
  by the Leibnitz rule and the second property. But every term above is uniquely determined. So it follows that $\D_t V$ must be given by this formula.
...
In many cases, having a connection on a vector bundle allows us to differentiate a lot more things. Here we will note a few.
