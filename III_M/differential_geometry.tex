...
\begin{eg}
  ...
  \end{align*}
  It is easy to show that this gives a bijection to the open disk in $\R^3$. We similarly define the other $\varphi_i^{\pm}$. These then give us an atlas of $S^2$.
\end{eg}
...
\begin{defi}[Vector bundle]\index{vector bundle}
  ...
  \begin{enumerate}
    \item For each $p \in M$, $\pi^{-1}(p) = E_p$ is an $r$-dimensional vector space,
...
\begin{defi}[Length of curve]\index{length!curve}\index{curve!length}
  ...
  \[
    \ell(\gamma)= \int_I norm{\dot{\gamma}(t)} \;\d t.
  \]
\end{defi}
