...
\begin{lemma}
  Given $\xi \in T_e G$, we let $X_\xi(g) = \D L_g(\xi) \in T_g(G)$. Then the map $T_e G \to \Vect^L(G)$ by $X \mapsto X_\xi$ is an isomorphism of vector spaces.
\end{lemma}
...
We now want to state the Mayer-Vietoris theorem. This will allow us to compute de Rham cohomology groups by looking at the de Rham cohomology of open submanifolds. To do so, we need the language of chain complexes and exact sequences.
...
\begin{defi}[Induced connection on dual bundle]\index{induced connection!dual bundle}
  ...
  \[
    \d\bra s, \xi\ket = \bra \d_E s, \xi\ket + \bra s, \d_E \xi\ket,
  \]
...
\begin{lemma}[Parallel transport]
  Let $t_0 \in I$ and $\xi \in T_{\gamma(t_0)} M$. Then there exists a unique parallel vector field $V \in J(\gamma)$ such that $V(t_0) \xi$. We call $V$ the \term{parallel transport} of $\xi$ along $\gamma$.
\end{lemma}
...
\begin{proof}
  ...
  \]
  This is a linear ODE in $V$ with initial condition given by $V(t_0) = \xi$, which has a unique solution.
...
We want to capture the idea that $\R^n$ is flat, while $S^n$ is curved. How do we do this with connections?
\begin{defi}[Parallel vector field]\index{parallel vector field}
...
However, we cannot find a non-trivial parallel vector field on $S^2$. % insert picture
...
Naively, we would try to construct it as follows. Say $\dim M = 2$ with coordinates $x, y$. Put $p = (0, 0)$. Then we can transport $\xi$ along the line $\{y = 0\}$ to define $V(x, 0)$. Then fixing an $\alpha$, we parallel transport $V(\alpha, 0)$ along $\{x = \alpha\}$. So this determines $V(x, y)$.
...
Since flatness is a local property, it is clear that if a manifold is locally isometric to $\R^n$, then it is flat. What we want to prove is the converse --- if you are flat, then you are locally isometric to $\R^n$. But for completely, let's define what an isometry is.
...
\begin{proof}
  Since flatness is a local property, we know that if $M$ is locally isometric to $\R^n$, then it is flat.
\end{proof}
...
\begin{prop}
  ...
  \begin{enumerate}
    ...
    \item $[V_i, V_p] = 0$, ie. the $V_i$ form a frame that pairwise commutes.
  \end{enumerate}
...
\begin{proof}
  ...
  \[
    \D_\alpha\left(\frac{\partial}{\partial t_i}\right) = V_i
  \]
  whenever this is defined. Indeed, for $\mathbf{t} \in B$ and $f \in C^\infty(M, \R)$. Then we have
  \begin{align*}
    \d_\alpha \left(\left.\frac{\partial}{\partial t_i}\right|_\mathbf{t}\right)(f) &= \left.\frac{\partial}{\partial t_i}\right|_t f(\alpha(t_1, \cdots, t_n))\\
  ...
  \]
  and this is a basis for $T_p M$. So $\D \alpha|_0: T_0 \R^n \to T_p M$ is an isomorphism. By the inverse function theorem, this is a local diffeomorphism, and in this chart, the claim tells us hat
  \[
    V_i = \frac{\partial}{\partial x_i}.
  \]
...
\begin{proof}[Proof of theorem]
  Let $(M, g)$ be a flat manifold. We fix $p \in M$. We let $x_1, \cdots, x_n$ be coordinates centered at $p_1$, says defined for $|x_i| < 1$. We need to construct orthonormal vector fields. To do this, we pick an orthonormal basis at a point, and parallel transport it around.
  ...
  \[
    \nabla_{\frac{\partial}{\partial x_k}} E_i = 0\tag{$*$}
  \]
  ...
  \[
    \nabla_{\frac{\partial}{\partial x_j}} E_i = 0
  \]
  for all $i, j$.
  ...
  \[
    \nabla_{\frac{\partial}{\partial x_j}} E_i = 0\tag{$A$}
  \]
  for $j \leq k$ and $\{x_{k + 1} = \cdots = x_n = 0\}$. For $j = k + 1$, we know that $\nabla_{\frac{\partial}{\partial x_{k + 1}}} E_i = 0$ on $\{x_{k + 2} = \cdots = x_n = 0\}$ by $(*)$. So the only problem we have is for $j = k$ and $\{x_{k + 2} = \cdots = x_n = 0\}$.

  By flatness of the Levi-Civita connection, for $j \leq k$, we have
  \[
    \left[\nabla_{\frac{\partial}{\partial x_i}}, \nabla_{\frac{\partial}{\partial x_j}}\right] = \nabla_{[\frac{\partial}{\partial x_{k + 1}}, \frac{\partial}{\partial x_j}]} = 0.
  \]
  So we know
  \[
    \nabla_k \partial_j E_i = \nabla_j \nabla_k E_i = 0\tag{$B$}
  \]
  on $\{x_{k + 2} = \cdots = x_n = 0\}$. So combining $(A)$ and $(B)$, we know that $Y = 0$ on $\{x_{k + 2} = \cdots = x_n = 0\}$.
...
\begin{center}
  \begin{tikzpicture}
    \draw [->-=0.6] (0, 0) -- (2, 0) node [below] {$(s, 0 , \cdots, 0)$};
    \draw [->-=0.6] (2, 0) -- (2, 2) node [right] {$(s, t, \cdots, 0)$};
    \draw [->-=0.6] (2, 2) -- (0, 2) node [above] {$(0, t, \cdots, 0)$};
