...
\begin{defi}[Cohomology]\index{cohomology}
  ...
  \[
    H_p(V_{\Cdot}) = \frac{\ker \d_p}{\im \d_{p - 1}}.
  \]
\end{defi}
...
\begin{thm}[Snake lemma]\index{snake lemma}
  ...
  Then there are maps
  \[
    \delta: H_p(C_{\Cdot}) \to H_{p + 1}(A_{\Cdot})
  \]
  such that there is a long exact sequence
  \[
    \begin{tikzcd}
      \cdots \ar [r] & H_p(A_{\Cdot}) \ar[r, "i_*"] & H_p(B_{\Cdot}) \ar[r, "q_*"] & H_p(C_{\Cdot}) \ar[out=0, in=180, looseness=2, overlay, dll, "\delta"']\\
      & H_{p + 1}(A_{\Cdot}) \ar[r, "i_*"] & H_{p + 1}(B_{\Cdot}) \ar[r, "q_*"] & H_{p + 1}(C_{\Cdot}) \ar [r] & \cdots
    \end{tikzcd}.
  \]
\end{thm}
...
\begin{eg}
  ...
  \]
  It is easy to see that this is a comnnection, and also that it is compatible with the metric. So this is the Riemannian connection on $\R^n$.
\end{eg}
