...
\end{thm}
How can we prove this? As in all proofs of integrability, we are going to use the Riemann criterion.

First of all, since $g$ is continuous on a closed, bounded interval, it is uniformly continuous. So given $\varepsilon > 0$, we can find a $\delta$ such that $|g(x) - g(y)| < \varepsilon$ whenever $|x - y| < \delta$. Also, by the Riemann criterion, for any $\varepsilon'$, we can find a partition $P$ of $f$ such that
\[
  U(P, f) - L(P, f) = \sum_{j = 0}^{n - 1} (a_{j + 1} - a_j)\left(\sup_{[a_j, a_{j + 1}]} f - \inf_{[a_j, a_{j + 1}]} f\right) < \varepsilon
\]
The objective is to get a bound for the composite function. The thing we want to make small is
\[
  U(P, g\circ f) - L(P, g\circ f) = \sum_{j = 0}^{n - 1} (a_{j + 1} - a_j)\left(\sup_{[a_j, a_{j + 1}]} g\circ f - \inf_{[a_j, a_{j + 1}]} g\circ f\right).
\]
How can we make sure this difference is small? We want each term
\[
  (a_{j + 1} - a_j)\left(\sup\limits_{[a_j, a_{j + 1}]} g\circ f - \inf\limits_{[a_j, a_{j + 1}]} g\circ f\right)
\]
to be small.

Since $U(P, f) - L(P, f)$ is small, we know that each term
\[
  (a_{j + 1} - a_j)\left(\sup_{[a_j, a_{j + 1}]} f - \inf_{[a_j, a_{j + 1}]} f\right)
\]
is small. So ideally, we would want $\left(\sup\limits_{[a_j, a_{j + 1}]} f - \inf\limits_{[a_j, a_{j + 1}]} f\right)$ to be small. If so, the uniform continuity of $g$ implies that $\left(\sup\limits_{[a_j, a_{j + 1}]} g\circ f - \inf\limits_{[a_j, a_{j + 1}]} g \circ f\right)$ is also small. However, if $\left(\sup\limits_{[a_j, a_{j + 1}]} f - \inf\limits_{[a_j, a_{j + 1}]} f\right)$ is not small, this is possible only if $(a_{j + 1} - a_j)$ is small.

We will use these ideas to prove our result.
