...
\]
Note that $L_i$ and $\mathbf{L}^2$ are observables, but (i) implies we cannot simultaneously measure, eg. $L_1$ and $L_2$. The best that can be done is to measure $\mathbf{L}^2$ and $L_3$, say. This is possible by (ii). Finally, (iii) allows computation of the commutator of $L_i$ with any function of $\hat{x}_i$ and $\hat{p}_i$. First, we want to prove these commutation relations.
\begin{prop}\leavevmode
  \begin{enumerate}
...
\begin{proof}\leavevmode
  \begin{enumerate}
    \item We first do a direct approach, and look at specific indices instead of general $i$ and $j$. We have
      ...
      \]
      and all the double derivatives cancel. So done.
    ...
    \item This follows from (i) using the \emph{Leibnitz property}:
      \[
        [A, BC] = [A, B] C + B[A, C].
      \]
      This can be proved by directly expanding both sides. Then we have
      \begin{align*}
...
\end{align*}
Note these operators involve only $\theta$ and $\varphi$. Furthermore, we have
\[
...
For small $r$, we multiply by $r^2$ and discard the $rR$ and $rR^2$ terms to get
\[
...
\]
Degenerate states with different $\ell$ values reflects a hidden symmetry in the Coulomb potential.
...
However, there is an additional constraint on this. The \emph{Fermi-Dirac statistics} or \emph{Pauli exclusion principle} states that single particle states must be chosen to be distinct (including spin).

In other words, if we attempt to construct a multi-electron atom, we cannot put everything into the ground state. We are forced to put some electrons in higher energy states. We expect chemical reactivity to depend on occupancy of energy levels.
\begin{itemize}
...
This is a recognizable sketch of the periodic table. However, for $n = 3$ and above, this model does not hold well, since at these energy levels, electron-electron interactions become important.
\end{document}
