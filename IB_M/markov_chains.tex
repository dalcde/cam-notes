...
\begin{proof}(non-examinable)
  ...
  The next step is to couple the two chains together. The idea is to fix some state $s \in S$, and let $T$ be the earliest time at which $X_n = Y_n = s$. Because of recurrence, we can always find such at $T$. After this time $T$, they behave under the exact same distribution.
  ...
  \begin{align*}
    ...
    &= \P_{ij}(Y-n = k, T \leq n) + \P_{ij} (X_n = k, T > n)\\
...
\]
We now want to know look at $V_i$. If we stare at them hard enough, we see that $V_i(n) \leq k$ if and only if $T_k \leq n$. We can write an equivalent statement by letting $k$ be a real number. We denote $\lceil x\rceil$ as the least integer greater than $x$. Then we have
\[
...
In fact, there is an even easier check. We don't have to start with finding $\pi$, but just some $\lambda$ that is in detailed balance.
...
\begin{eg}[Random walk on a finite graph]
  ...
  This process is a rather ``physical'' process, and we would expect it to be reversible. So let's try to solve the detailed balance equation
  \[
    \lambda_i p_{i, j} = \lambda_j p_{j, i}.
  \]
  ...
  \]
  We now note that since each edge adds $1$ to the degrees of each vertex on the two ends, $\sum d_i$ is just twice the number of edges. So
  \[
    1 = 2c |E|.
  \]
  Hence we get
  ...
  \begin{itemize}
    ...
    \item Move two steps vertically, then one step horizontall.
  \end{itemize}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  At each epoch of time, our erratic night follows a legal move chosen uniformly from the set available. Hence we have a Markov chain derived from the chessboard. What is his invariant distribution. We can compute the number of possible moves from each position:
  \begin{center}
    \begin{tikzpicture}[scale=0.5]
      \foreach \x in {0,1,...,8} {
        \draw (0, \x) -- (8, \x);
