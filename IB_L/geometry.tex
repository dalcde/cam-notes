...
M\"obius transformations act on $\C_{\infty}$ and form a group $G$ by composition. For each matrix
\[
  A =
  \begin{pmatrix}
    a & b\\
...
\]
Now let's think about the sphere. On $S^2$, the rotations $\SO(3)$ act as isometries. In fact, the full isometry group of $S^2$ is $\Or(3)$. The proof is on the first example sheet.
...
\begin{proof}\leavevmode
  \begin{enumerate}
    ...
    \item We claim that $\SO(3)$ is generated by $r\left(\hat{\mathbf{y}}, \frac{\pi}{2}\right)$ and $r(\hat{\mathbf{z}}, \theta)$ for $0 \leq \theta < 2\pi$.
      ...
      To show this, we observe that $r(\hat{\mathbf{x}}, \phi) = r(\hat{\mathbf{y}}, \frac{\pi}{2}) r(\hat{\mathbf{z}}, \varphi) r(y, -\frac{\pi}{2})$. Note that we read the composition from right to left. You can convince yourself this is true by taking a physical sphere and try rotating. To prove it formally, we can just multiply the matrices out.

      Next, observe that for $\mathbf{v} \in S^2 \subseteq \R^3$, there are some angles $\varphi, \psi$ such that $g = r(\hat{\mathbf{z}}, \psi) r(\hat{\mathbf{x}}, \psi)$ maps $\mathbf{v}$ to $\hat{\mathbf{x}}$. We can do so by first picking $r(\mathbf{x}, \varphi)$ to rotate $\mathbf{v}$ into the $(x, y)$-plane. Then we rotate about the $z$-axis to send it to $\hat{\mathbf{x}}$. This is in fact just what we do in spherical polar coordinates!

      Then for any $\theta$, we have $r(\mathbf{v}, \theta) = g^{-1} r(x, \theta) g$, and our claim follows by composition.
...
We start with a (non-closed) path from $I$ to $-I$ in $\SU(2)$. Applying the map, we get a \emph{closed} loop from $I$ to $I$ in $\SO(3)$.

Hence, in $\SO(3)$, loops are behave slightly weirdly. If we go around the loop in $\SO(3)$, we didn't really get back to the same place. Instead, we have actually moved from $I$ to $-I$ in $\SU(2)$. It takes \emph{two} full loops to actually get back to $I$. In physics, this corresponds to the idea of spinors.
...
On the other hand, $\SO(3)$ is not simply connected. We have just constructed a loop in $\SO(3)$ from the path from $I$ to $-I$ in $\SU(2)$. We \emph{cannot} deform this loop until it just sits at a single point, since if we lift it back up to $\SU(2)$, it still has to move from $I$ to $-I$.
...
We've just been waffling about spaces and loops, and throwing around terms we haven't defined properly. These vague notions will be made precise in IID Algebraic Topology, and we will then (maybe) see that $\SU(2)$ is a \emph{universal cover} of $\SO(3)$.


\section{Triangulations and the Euler number}
We introduce the locally Euclidean torus, which we shall just call the ``torus''.
\begin{defi}[Torus]
  The \emph{torus} is the set $\R^2/\Z^2$ of equivalence classes of $(x, y) \in \R^2$ with the equivalence relation
  \[
...
Of course, there are many ways to represent a point in the torus. However, for any closed square $Q \subseteq \R^2$ with side length $1$, $T$ is obtained by identifying the sides
\begin{center} % should be a torus
  \begin{tikzpicture}
    \draw [mred, ->-=0.55] (0, 0) -- +(2, 0);
    \draw [mred, ->-=0.55] (0, 2) -- +(2, 0);

    \draw [mblue, ->-=0.55] (0, 0) -- +(0, 2); % different arrow
    \draw [mblue, ->-=0.55] (2, 0) -- +(0, 2);
...
We write $\mathring{Q}$ for the interior of $Q$. Then the natural map $\mathring{Q} \to T$ given by $\mathbf{v} \mapsto \mathbf{v} + \Z^2$ is a bijection onto some open set $U \subseteq T$. In fact, $U$ is just $T \setminus \{\text{two circles meeting in 1 point}\}$, where the two circles are the boundary of the square $Q$.

If $p \in \mathring{Q}$, then $f$ restricted to an open disk about $P$ is an isometry (onto the image, a subset of $\R^2$). Thus we say $d$ is a \emph{locally Euclidean metric}.
...
The next thing in the title of the chapter is \emph{triangulation}. To understand this, we need to define a triangle.
\begin{defi}[Topological triangle]
  A \emph{topological triangle} on $X = S^2$ or $T$ (or any metric space $X$) is the image of $R \subseteq X$ of a closed Euclidean triangle $\Delta \subseteq \R^2$ under some homeomorphism.
\end{defi}
...
The idea of triangulation is to cut the space $X$ into pieces that look like triangles.
...
\begin{eg}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw [mblue, ->-=0.55] (0, 0) -- +(0, 3); % different arrow
      \draw [mblue, ->-=0.55] (3, 0) -- +(0, 3);
      \foreach \x in {1,2} {
        \draw (\x, 0) -- +(0, 3);
        \draw (0, \x) -- +(3, 0);
      }
...
\end{eg}
Both of these examples are \emph{geodesic triangles}.
...
\end{defi}
In particular, we used spherical lines in $S^2$ and Euclidean triangles in $\mathring{Q}$.

It is a good idea for us to start proving things.
\begin{prop}
...
\begin{proof}
  Denote the ``faces'' of $\tau$ as $\Delta_1, \cdots, \Delta_F$, and denote $\tau_i = \alpha_i + \beta_i + \gamma_i$ to be the sum of the interior angles of the triangles (with $i = 1, \cdots, F$).
  ...
  \end{itemize}
\end{proof}
Note that in the definition of triangulation, we decomposed $X$ into topological triangles. We can also use decompositions by topological polygons, but they are slightly more complicated, since we have to worry about convexity. However, apart from this, everything works out well. In particular, the previous proposition also holds, and we have Euler's formula for $S^2$: $V - E + F = 2$ for any polygonal decomposition of $S^2$.

\section{Hyperbolic geometry}
The next type of geometry we will be studying is hyperbolic geometry.

It turns out there are three standard types of $2$-dimensional geometries. We have seen two so far --- Euclidean geometry (which is just the familiar one), spherical geometry (which we have just introduced). The third is hyperbolic geometry.

While spherical geometry is new to us, it is rather ``hands-on'' since we can view it via the embedding $S^2 \subseteq \R^3$. However, hyperbolic geometry will requires some preparation. This will include a new concept of a Riemannian metric. Informally, the idea of a Riemannian metric is an ``inner product varying from point to point''.
...
\begin{defi}[Derivative]
  ...
  \[
    \frac{\|f(a + h) - f(a) - \d f_a \cdot h\|}{\|h\|} \to 0
  \]
  as $h \to 0$ (with $h \in \R^n$). If $m = 1$, then $\d f_a$ is expressed as $\left(\frac{\partial f}{\partial x_a}(a), \cdots, \frac{\partial f}{\partial x_n}(a)\right)$, and the linear map is given by
  \[
...
\begin{eg}
  ...
  \]
  as $|w| \to 0$. We let $f'(z) = a + ib$ and $w = h_1 + i h_2$. Then we have
  \[
...
Now we can finally get to Riemannian metrics. We will work with Riemannian metrics on open subsets of $\R^2$. These can be further generalized in many ways, but we will not do that in this course.
...
\end{defi}
It should be clear what we are integrating is just the determinant of the metric. This is also known as the Gram determinant.
...
\begin{eg}
  ...
  \]
  This looks somewhat arbitrary, but we shall see this actually makes sense by identifying $\R^2$ with the sphere by the stereographic projection.

  Recall we studied stereographic projection $\pi: S^2 \setminus \{N\} \to \R^2$, sending $\pi(x, y, z) \to (u, v)$. For any point $P \not= N \in S$. We have $\pi(P) \in \R^2$, and thus an inner product $\bra \ph, \ph\ket_{\pi(P)}$ given by the metric.

  The tangent plane to $S^2$ at $P$ is (parallel to) $\{\mathbf{x} \in \R^3: \mathbf{x}\cdot \overrightarrow{OP} = 0\}$. We identify this as a copy of $\R^2$ (as a vector space) so that $0$ corresponds to $P$.

  We can then show that for every $\mathbf{x}_1, \mathbf{x}_2 \perp \overrightarrow{OP}$, the dot product in $\R^2$ (as the tangent plane with Euclidean metric) satisfies
  \[
    \mathbf{x}_1 \cdot \mathbf{x}_2 = \bra \d \pi_P(\mathbf{x}_1), \d \pi_P(\mathbf{x}_2)\ket_{\pi(P)}.
  \]
  This is left as an exercise for the reader on the third example sheet. It is helpful to notice % fill in
  \[
...
\begin{defi}[Isometry]
  Let $V, \tilde{V} \subseteq \R^2$ be open sets endowed with Riemannian metrics, denoted as $\bra \ph, \ph\ket_P$ and $\bra \ph, \ph\ket^\sim_Q$ for $P \in V, Q \in \tilde{Q}$ respectively.

  A diffeomorphism (ie. $C^\infty$ map with $C^\infty$ inverse) $\varphi: V \to \tilde{V}$ is an \emph{isometry} if for every $P \in V$, we get
  \[
    \bra \mathbf{x}, \mathbf{y}\ket_P = \bra \d \varphi_P(\mathbf{x}), d\varphi_P(\mathbf{y})\ket^\sim_{\varphi(P)}.
  \]
\end{defi}
Why do we have this weird-looking definition? A first guess might be to require
\[
  \bra \mathbf{x}, \mathbf{y}\ket_P = \bra \mathbf{x}, \mathbf{y}\ket_{\varphi(P)}.
\]
However, if we think carefully what these mean, this makes completely no sense. What $\mathbf{x}$ actually means is the direction of $\mathbf{x}$ from the point $P$, and we specify $\mathbf{x}$ by the coordinate structure we get from thinking of $V$ as a subspace of $\R^2$. There is no reason why the vector $\mathbf{x}$ should represent the ``same'' direction from a different point $\varphi(P)$ in a different space $\tilde{V}$. It turns out the right way to convert ``directions from $P$'' to ``directions from $\varphi(P)$'' is via acting by $\d \varphi_P$.

How are we sure this indeed is the right definition? We, at the very least, would expect this to preserve lengths and distances. Let's see this is indeed the case. If $\gamma: [0, 1] \to V$ is a $C^\infty$ curve, the composition $\tilde{\gamma} = \varphi \circ \gamma: [0, 1] \to \tilde{V}$ is a path in $\tilde{V}$. We let $P = \gamma(t)$, and hence $\varphi(P) = \tilde{\gamma}(t)$. Then
\[
  \bra \tilde{\gamma}'(t), \tilde{\gamma}'(t)\ket_{\tilde{\gamma}(t)}^\sim = \bra \d \varphi_P(\gamma'(t)), \d \varphi_P(\gamma'(t))\ket_{\varphi(P)}^\sim = \bra \gamma'(t), \gamma'(t)\ket_{\gamma(t) = P}.
\]
Integrating, we obtain
\[
  \length(\tilde{\gamma}) = \length(\gamma) = \int_0^1 \bra \gamma'(t), \gamma'(t)\ket _{\gamma(t)} \;\d t.
\]
That's enough preparation. We can start talking about hyperbolic plane.
\subsection{Two models for the hyperbolic plane}
We will provide two different models for the hyperbolic plane.

\begin{defi}[Poincar\'e disk model]
  The \emph{(Poincar\'e) disk model} for the hyperbolic plane is given by
  \[
    D \subseteq \C\cong \R^2,\quad D = \{\zeta \in \C: |\zeta| < 1\},
  \]
  the unit disk, and a Riemannian metric on this disk given by
  \[
...
\begin{defi}[Upper half-plane]
  ...
  \]
\end{defi}
We know $D$ bijects to $H$ via the M\"obius transformation $\varphi: \zeta \in D \mapsto i \frac{1 + \zeta}{1 - \zeta}$. This bijection is in fact a conformal equivalence, as defined in IB Complex Analysis/Complex Methods.
...
\]
We shall prove that this M\"obius map ``induces'' a unique Riemannian metric on $H$ so that $\varphi: \zeta \to z$ is an isometry. Then $H$ together with this Riemannian metric will be the upper half-plane model for the hyperbolic plane.

Instead of trying to convert the Riemannian metric on $D$ to $H$, we first try converting the Euclidean metric. The Euclidean metric on $\R^2 = \C$ is given by
\[
  \bra w_1, w_2\ket = \Re(w_1 \overline{w_2}) = \frac{1}{2}(w_1 \bar{w}_2 + \bar{w}_1 w_2).
\]
So if $\bra\ph, \ph\ket$ is Euclidean at $\zeta$, then at $z$ such that $\zeta = \frac{z - i}{z + i}$, we require (by definition of isometry)
\[
  \bra w_1, w_2\ket_z = \left\bra \frac{\d \zeta}{\d z} w_1, \frac{\d \zeta}{\d z} w_2\right\ket_{\mathrm{eucl}} = \left|\frac{\d \zeta}{\d z}\right|^2 \Re(w_1 \bar{w}_2).
\]
Hence, on $H$, we obtain the Riemannian metric
\[
  \left|\frac{\d \zeta}{\d z}\right| (\d x^2 + \d y^2).
\]
We can compute
\[
...
and hence
\[
  \frac{1}{1 - |\zeta|^2} = \frac{|z + i|^2}{|z + 1|^2 - |z - i|^2} = \frac{|z + i|^2}{4 \Im z}.
\]
...
We are next going to study the geometry of $H$, and will consider the projective special linear group
\[
  \PSL(2, \R) = \left\{z \mapsto \frac{az + b}{cz + d}: a, b, c, d \in \R, ad - bc = 1\right\}.
\]
Note that the coefficients have to be real, not complex. We will see these are isometries.
...
\begin{proof}
  ...
  \begin{enumerate}
    \item Translations $z \mapsto z + 1$ for $a \in \R$
    \item Dilations $z \mapsto az$ for $a > 0$
    \item The single map $z \mapsto -\frac{1}{z}$.
  \end{enumerate}
  So it suffices to show each of these preserves the metric $\frac{|\d z|^2}{y^2}$, where $z = x + iy$. The first two are straightforward to see. % fill in
...
Note that each $z \mapsto az + b$ with $a > 0, b \in \R$ is in $\PSL(2, \R)$. Also, we can use maps of this form to send any point to any other point. So $\PSL(2, \R)$ acts transitively on $H$.

Recall also that each M\"obius transformation preserves circles and lines in the complex plane. In particular, consider the line $L = i\R$, and let $g \in \PSL(2, \R)$. Then the image is either a circle centered at a point in $\R$, or a straight line perpendicular to $\R$.
...
\begin{proof}
  It suffices to show that for each hyperbolic line $\ell$, there is some $g \in \PSL(2, \R)$ such that $g(\ell)= L^+$. This is clear when $\ell$ is a vertical half-line.
...
\begin{defi}[Hyperbolic distance]
  For points $z_1, z_2 \in H$, the \emph{hyperbolic distance} $\rho(z_1, z_2)$ is the length of the segment $[z_1, z_2] \subseteq \ell$ for the hyperbolic line through $z_1, z_2$ (parametrized monotonically).
\end{defi}
Thus $\PSL(2, \R)$ preserves hyperbolic distances.
...
\begin{cor}[Triangle inequality]
  ...
  \]
  if and only if $z_2$ lies between $z_1$ and $z_2$.
\end{cor}
  ...
  \begin{enumerate}
    ...
    \item First, we need to check this indeed maps $D$ to itself. To do this, we first make sure it sends $\{|z| = 1\}$ to itself. If $|z| = 1$, then
      ...
      \[
        |g(z)| = 1.
      \]
      Finally, it is easy to check $g(a) = 0$. So $G$ indeed fixes $D$. We can then show it is an isometry by plugging it into the formula.
  \end{enumerate}
\end{proof}
It is an exercise to show all $g \in G$ is of the form
\[
...
\begin{proof}
  ...
  \]
\end{proof}
Again, we exploited the idea of performing the calculation in an easy case, and then using isometries to move everything else to the easy case. In general, when there is a ``distinguished'' point in the hyperbolic plane, it is often convenient to use the disk model, move it to $0$ by an isometry.
...
How does reflection in hyperbolic lines work?
...
\begin{proof}
  ...
  \[
    \rho(P, Q) = \rho(g(P), Q).
  \]
  Now note that $g(\ell')$ is also a line meeting $L^+$ perpendicularly at $Q$. So we must have $g(\ell') = \ell'$. Then in particular $g(P) \in \ell'$. So we must have $g(P) = P$ or $g(P) = P'$, where $P'$ is the image under reflection in $L^+$.
  ...
  We now let $A \in H^+$, where $H^+ = \{z \in H: \Re z > 0\}$.
  \begin{center}
    \begin{tikzpicture}
      \draw (-3, 0) -- (3, 0);
...
\begin{thm}[Gauss-Bonnet theorem for hyperbolic triangles]
  ...
  \]
\end{thm}
Recall we already had a Gauss-Bonnet theorem for spherical triangles. We will later formulate a Gauss-Bonnet theorem for general surfaces.

\begin{proof}
  First do the case where $\gamma = 0$, so $C$ is ``at infinity''. We use the $H$ model, and wlog $C = \infty$ (apply $\PSL(2, \R)$) if necessary. Then $AC$ and $BC$ are in the same vertical half-lines. So $AB$ is the arc of a semi-circle. So $AB$ is an arc of a semicircle.
  \begin{center}
    \begin{tikzpicture}
      \draw (-3, 0) -- (3, 0);
  ...
  In general, we use $H$ again, and we can arrange $AC$ in a vertical half-line. Also, we can move $AB$ to $x^2 + y^2 = 1$, noting that these transformation keeps $AC$ vertical.
  \begin{center}
    \begin{tikzpicture}
      \draw (-3, 0) -- (3, 0);
...
Recall that in the Euclidean plane, we have the parallel axiom: given a line $\ell$ and $P \not\in \ell$, there exists a unique line $\ell'$ containing $P$ with $\ell \cap \ell' = \emptyset$.

This fails in both $S^2$ and the hyperbolic plane --- but for very different reasons! We will come back to this at the very end.
...
\[
  S^+ = S \cap \{z > 0\}.
\]
We let $\pi$ be the stereographic projection from (0, 0, -1) $\pi: S^+ \to D \subseteq \C = \R^2$ by
\[
...
We shall move on from studying specific classic geometries to general, modern differential differential geometry.
\begin{defi}[(Parametrized) smooth embedded surface]
  ...
  \begin{enumerate}
    \item $\boldsymbol\sigma$ is a homeomorphism (ie. a bijection with continuous inverse)
    \item $\boldsymbol\sigma$ is $C^\infty$ (smooth) on $V$ (ie. continuous partial derivatives of all orders).
    \item For all $Q = \boldsymbol\sigma(P)$, the partial derivatives $\frac{\partial \boldsymbol\sigma}{\partial u}(P)$ and $\frac{\partial \boldsymbol\sigma}{\partial v}(P)$ are linearly independent.
  \end{enumerate}
\end{defi}
Recall that if we write
\[
  \boldsymbol\sigma(u, v) = (x(u, v), y(u, v), z(u, v)),
\]
then
\[
  \boldsymbol\sigma_u(P) = \frac{\partial \boldsymbol\sigma}{\partial u}(P) =
  \begin{pmatrix}
  ...
  \end{pmatrix}
  (P)
  = d\boldsymbol\sigma_P (\mathbf{e}_1),
\]
where $\mathbf{e}_1, \mathbf{e}_2$ are the standard basis of $\R^2$. Similarly, we have
\[
  \boldsymbol\sigma_v(P) = \d \boldsymbol\sigma_P(\mathbf{e}_2).
\]
...
\begin{defi}[Tangent space]
  The subspace of $\R^3$ spanned by $\boldsymbol\sigma_u, \boldsymbol\sigma_v$ is the \emph{tangent space} $T_QS$ to $S$ at $Q = \boldsymbol\sigma(P)$.
\end{defi}

\begin{defi}[Smooth parametrisation]
  The function $\boldsymbol\sigma$ is a \emph{smooth parametrisation} of $U \subseteq S$.
\end{defi}

\begin{prop}
  Let $\boldsymbol\sigma: V \to U$ and $\tilde{\boldsymbol\sigma}: \tilde{V} \to U$ be two $C^\infty$ parametrisations of a surface. Then the homeomorphism
  \[
    \varphi = \boldsymbol\sigma^{-1} \circ \tilde{\boldsymbol\sigma}: \tilde{V} \to V
  \]
...
\begin{proof}
  ...
  Writing $\pi: \tilde{\boldsymbol\sigma} \to N'$ for the projection $\pi(x, y, z) = (x, y)$ we can put these things in a commutative diagram:
  \[
    \begin{tikzcd}
      & \boldsymbol\sigma(N) \ar[d, "\pi"]\\
      N \ar[r, "F"'] \ar[ru, "\boldsymbol\sigma"] & N'
    \end{tikzcd}.
  \]
  We now let $\tilde{N} = \tilde{\boldsymbol\sigma}^{-1}(\boldsymbol\sigma(N))$ and $\tilde{F} = \pi \circ \tilde{\boldsymbol\sigma}$, which is yet again smooth. Then we have the following larger commutative diagram.
  \[
    \begin{tikzcd}
      & \boldsymbol\sigma(N) \ar[d, "\pi"]\\
      N \ar[r, "F"'] \ar[ru, "\boldsymbol\sigma"] & N' & \tilde{N} \ar[l, "\tilde{F}"] \ar[lu, "\tilde{\boldsymbol\sigma}"']
    \end{tikzcd}.
  \]
  Then we have
  \[
    \varphi = \boldsymbol\sigma^{-1} \circ \tilde{\boldsymbol\sigma} = \boldsymbol\sigma^{-1} \circ \pi^{-1} \circ \pi \circ \tilde{\boldsymbol\sigma} = F^{-1} \tilde{F},
  \]
...
\begin{proof}
  We know
  \[
    \tilde{\boldsymbol\sigma} (\tilde{u}, \tilde{v}) = \boldsymbol\sigma(\varphi_1(\tilde{u}, \tilde{v}), \varphi_2(\tilde{u}, \tilde{v})).
  \]
  We can then use the partial derivatives to compute the partial derivatives as
  \begin{align*}
    \tilde{\boldsymbol\sigma}_{\tilde{u}} &= \varphi_{1, \tilde{u}} \boldsymbol\sigma_u + \varphi_{2, \tilde{u}} \boldsymbol\sigma_v\\
    \tilde{\boldsymbol\sigma}_{\tilde{v}} &= \varphi_{1, \tilde{v}} \boldsymbol\sigma_u + \varphi_{2, \tilde{v}} \boldsymbol\sigma_v
  \end{align*}
  ...
  \]
  This is invertible since $\varphi$ is a diffeomorphism. So $(\boldsymbol\sigma_{\tilde{u}}, \boldsymbol\sigma_{\tilde{v}})$ and $(\boldsymbol\sigma_{u}, \boldsymbol\sigma_v)$ are different basis of the same two-dimensional vector space. So done.
\end{proof}

Note that we can compute
\[
  \tilde{\boldsymbol\sigma}_{\tilde{u}} \times \tilde{\boldsymbol\sigma}_{\tilde{v}} = \det(J(\varphi)) \boldsymbol\sigma_u \times \boldsymbol\sigma_v.
\]
\begin{defi}[Unit normal]
  The \emph{unit normal} to $S$ at $Q \in S$ is
  \[
    N = N_Q = \frac{\boldsymbol\sigma_u \times \boldsymbol\sigma_v}{\|\boldsymbol\sigma_u \times \boldsymbol\sigma_v\|},
  \]
...
\[
  \bra \mathbf{a}, \mathbf{b} \ket_P = \bra \d \boldsymbol\sigma_P (\mathbf{a}), \d \boldsymbol\sigma_P(\mathbf{b})\ket_{\R^3}.
\]
...
where
\begin{align*}
  E &= \bra \boldsymbol\sigma_u, \boldsymbol\sigma_u\ket = \bra \mathbf{e}_1, \mathbf{e}_1\ket_P\\
  F &= \bra \boldsymbol\sigma_u, \boldsymbol\sigma_v\ket = \bra \mathbf{e}_1, \mathbf{e}_2\ket_P\\
  G &= \bra \boldsymbol\sigma_v, \boldsymbol\sigma_v\ket = \bra \mathbf{e}_2, \mathbf{e}_2\ket_P.
\end{align*}
Note that, say, $\boldsymbol\sigma_u$ is defined to be $\boldsymbol\sigma_u = \d \boldsymbol\sigma_P (\mathbf{e}_1)$. Similarly, $\boldsymbol\sigma_v = \d \boldsymbol\sigma_P (\mathbf{e}_2)$.

Thus, this induces a Riemannian metric of $V$. This is also called the first fundamental form corresponding to $\boldsymbol\sigma$. This is what we do in practical examples.

We will assume the following property, which we are not bothered to prove.
\begin{prop}
  If we have two parametrizations related by $\tilde{\boldsymbol\sigma} = \boldsymbol\sigma \circ \varphi: \tilde{V} \to U$, then $\varphi: \tilde{V} \to V$ is an isometry of Riemannian metrics (on $V$ and $\tilde{V}$).
\end{prop}
...
\begin{defi}[Length and energy of curve]
  ...
  \]
\end{defi}
We can think of the energy as something like the kinetic energy, except that we are missing the factor of $\frac{1}{2}m$, because they are annoying.
...
\[
  \gamma = \boldsymbol\sigma^{-1} \circ \Gamma: [a, b] \to V.
\]
This curve has two components, say $\gamma = (\gamma_1, \gamma_2)$. Then we have
\[
  \Gamma'(t) = (\d \boldsymbol\sigma)_{\gamma(t)}(\dot{\gamma}_1(t) \mathbf{e}_1 + \dot{\gamma}_2(t) \mathbf{e}_2) = \dot{\gamma}_1 \boldsymbol\sigma_u + \dot{\gamma}_2 \boldsymbol\sigma_v,
\]
and thus
\[
...
\begin{proof}
  ...
  \[
    \frac{\d}{\d t} (2 (E\dot{u} + F\dot{v})) = E_u \dot{u}^2 + 2F\dot{u}\dot{v} + G\dot{v}^2,
  \]
  which is exactly the geodesic ODE. Similarly, the second equation gives the other geodesic ODE. So done.
\end{proof}

We want to be able to do this more generally for embedded surfaces $S \subseteq \R^3$.
...
Alternatively, $\Gamma$ is a geodesic if and only if it is a stationary point of the energy
\[
  \int_a^b \|\Gamma'(t)\|^2 \;\d t.
\]
This alternative formulation shows that being a geodesic does not depend on the parametrization $\sigma$.
...
\begin{proof}
  Recall the Cauchy-Schwartz inequality for continuous functions $f, g \in C[0,1]$. Then
  \[
...
Now note that in the geodesic equations, if we expand out the derivative, we find that the coefficients of $\ddot{\gamma}_1$ and $\ddot{\gamma}_2$ are given by the matrix
\[
  \begin{pmatrix}
    E & F\\
    F & G
  \end{pmatrix}.
\]
Thus, the geodesic equations are equivalent to
\[
  \begin{pmatrix}
    \ddot{\gamma}_1\\ \ddot{\gamma}_2
  \end{pmatrix} =
  F(\gamma_1, \gamma_2, \dot{\gamma}_1, \dot{\gamma_2)}
\]
for some function $F$. From the general theory of ODE's in IB Analysis II, subject to some sensible conditions, given any $P = (u_0, v_0) \in V$ and $\mathbf{a} = (p_0, q_0) \in \R^2$, there is a \emph{unique} geodesic curve $\gamma(t)$ defined for $|t| < \varepsilon$ with $\gamma(0) = P$ and $\dot{\gamma}(0) = \mathbf{a}$. In other words, we can choose a point, and a direction, and then there is a geodesic going that way.
...
We can make this (sligthly) more precise, and provide a quick sketch of how we can do this formally. We let $\psi: U \to V$ be some chart with $P \in U \subseteq S$. We wlog $\psi(P) = 0 \in V \subseteq \R^2$. We denote by $\theta$ the polar angle (coordinate), defined on $V \setminus \{0\}$.
\begin{center}
  \begin{tikzpicture}
    \draw [->] (-0.5, 0) -- (3, 0);
    \draw [->] (0, -0.5) -- (0, 3);
...
\[
  \begin{tikzcd}
    W \ar[r, "\sigma"] & V_0 \ar[r, "\psi^{-1}"] & U_0 \subseteq S
  \end{tikzcd}
\]
is a valid parametrization. Thus $\sigma^{-1} \circ \psi$ is a valid chart.

The image $(r, \theta)$ of this chart are the \emph{geodesic polar coordinates}. We have the folloiwng lemma:
...
\begin{lemma}[Gauss' lemma]
  ...
  \]
\end{lemma}
This is why we like geodesic polar coordates. Using these, we can put the Riemannian metric into a very simple form.

Of course, this is just a sketch of what really happens, and there are many holes to fill in. For more detials, go to IID Differential Geometry.
...
\]
given by the same formula is a homeomorphism onto the image. The proof of this is left as an exercise for the reader. % complete?
...
Gaussian curvature is something we can define for a surface. But for the moment, we shall consider it for a curve.

\begin{defi}[Curvature of curve]
  We let $\eta: [0, \ell] \to R^2$ be a curve parametrized by unit speed, ie. $\|\eta'\| = 1$. The \emph{curvature} $\kappa$ at the point $\eta(s)$ is determined by
  \[
    \eta'' = \kappa \mathbf{n},
  \]
...
\]
where $\kappa$ is the curvature at $\gamma(t)$. By Taylor's theorem, we have
\begin{multline*}
...
\begin{proof}
  ...
  \]
  Then $\mathbf{e}$ and $\mathbf{f}$ are unit and orthogonal. We also let $\mathbf{N}$ be a third unit vector orthogonal to $\mathbf{e}$ and $\mathbf{f}$ so that they form a basis of $\R^3$.
  ...
  But we have
  \[
    \alpha = \mathbf{e}_u \cdot \mathbf{f} = \sigma_{uu}\cdot \frac{\sigma_v}{\sqrt{G}} = \left((\sigma_u \cdot \sigma_v) - \frac{1}{2} (\sigma_u \cdot \sigma_u)_v\right) \frac{1}{\sqrt{G}} = 0.
  \]
  So $\alpha$ vanishes. Also, we have
  \[
  ...
  \begin{align*}
    ...
    &= -(\sqrt{G})_{uu}.
  \end{align*}
  Thus, using the notation of the previous proposition, we have
  \begin{align*}
    \mathbf{N}_u \times \mathbf{N}_v &= (a \sigma_u + b \sigma_v)(c \sigma_u + d \sigma_v) \\
  ...
  \end{align*}
  Writing $\mathbf{N}\cdot \mathbf{e} = 0$, we know
  \[
    N_u \cdot \mathbf{e} + \mathbf{N}\cdot \mathbf{e}_u = 0.
  \]
  So
  \[
    \mathbf{N}_u \cdot \mathbf{e} = -\lambda_1,
  \]
  and similarly for the other terms. So we get
  \[
    K\sqrt{G} = \lambda_1 \mu_2 - \lambda_2 \mu_1 = -(\sqrt{G})_{uu}.
  \]
  as required. Phew.
\end{proof}
...
Note that embedded surfaces are quite general surfaces, but do not include, say, the hyperbolic plane. We can generalize our notions by considering surfaces ``without embedding in $\R^3$''. These are known as abstract surfaces.
...
Finally, we notice that we really need the notion of abstract surface for the hyperbolic plane, since it cannot be realized as an embeded surface in $\R^3$. The proof is not obvious at all, and is a theorem of Hilbert.
...
\begin{eg}\leavevmode
  \begin{enumerate}
   ...
   \item For the sphere $S$, we use the spherical coordinates, fixing the radius to be $1$. So we specify each point by
     ...
     \]
     Note that $\rho$ is not really the radius in spherical coordinates, but just one of the angle coordinates. We then have a the metric
     \[
...
It is a fact that this is equivalent to the axioms that the angles of a triangle sum to $\pi$. Thus, the Gauss-Bonnet theorem tells us the parallel postulate is captured by the fact that the curvature of the Euclidean plane is zero everywhere.
\end{document}
