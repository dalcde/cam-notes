...
\begin{enumerate}[resume]
  ...
  \item If we set for $\gamma: [a, b] \to \C$ the \emph{length}
    \[
      \mathrm{length}(\gamma) = \int_a^b |\gamma'(t)|\;\d t,
    \]
    then
    \[
      \left|\int_\gamma f(z)\;\d z\right| \leq \mathrm{length} (\gamma) \sup_{t \in \im \gamma}|f(t)|.
    \]
\end{enumerate}
...
\begin{eg}
  ...
  \begin{center}
    \begin{tikzpicture}
      \draw [->] (-3, 0) -- (3, 0);
      \draw [->] (0, -1) -- (0, 3);
      \draw [->-=0.7] (-2, 0) -- (2, 0) node [pos=0.7, below] {$\gamma_1$};
      \draw [->-=0.3, ->-=0.75] (2, 0) arc (0:180:2) node [pos=0.3, anchor = south west] {$\gamma_2$};
      \node [below] at (2, 0) {$R$};
      \node [below] at (-2, 0) {$-R$};
...
\begin{proof}
  Pick our favorite $a_0 \in U$. For $w \in U$, we choose a path $\gamma_w: [0, 1] \to U$ such that $\gamma_w(0) = a_0$ and $\gamma_w(1) = w$. This exists because $U$ is path connected. Using some fancy topological arguments, we can indeed pick this to be piecewise linear, and hence piecewise smooth. We thus defin
  \[
  ...
  \begin{align*}
    ...
    &\leq \frac{1}{|h|} \mathrm{length}(\delta_h) \sup_{\delta_h} |f(z) - f(w)|\\
