...
\subsection{Zeroes and singularities}
...
\begin{eg}
  ...
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  Since we don't want the contour itself to pass through singularities, we make the square pass throguh $\pm\left(N + \frac{1}{2}\right)$. Then the residue theorem says
  \[
...
\begin{eg}
  ...
  \]
  The point is that to define $\log z$, we have to cut the plane to avoid multi-valuedness. In this case, we might choose to cut it along $i \R\leq 0$, giving a branch of $\log$, for which $\arg(z) \in \left(-\frac{\pi}{2}, \frac{3\pi}{2}\right)$. We need to avoid running thorugh zero. So we might look at the following contour:
  \begin{center}
    \begin{tikzpicture}
      \draw [->] (-3, 0) -- (3, 0);
      \draw [->] (0, -3) -- (0, 3);
...
\begin{eg}
  ...
  \]
  Viewing $\sqrt{z} = e^{\frac{1}{2}\log z}$, on the two pieces of the contour along $\R_{\geq 0}$, $\log z$ differs by $2 \pi i$. So $\sqrt{z}$ changes sign. This cancels with the sign change arising from going in the wrong direction. Therefore the residue theorem sasys
  \[
...
\begin{proof}
  We let $q \in \C \setminus U$, and let $\phi(z) = z - q$. So $\phi: U \to \C$ is non-vanishing. It is also clearly holomorphic and non-constant. By an exercise (possbily on the example sheet), there is a holomorphic function $g: U \to \C$ such that $\phi(z) = e^{g(z)}$ for all $z$. In particular, our function $\phi(z) = z - q: U \to \C^*$ can be written as $\phi(z) = h(z)^2$, for some function $h: u \to \C^*$ (by letting $h(z) = e^{\frac{1}{2}g(z)}$).
