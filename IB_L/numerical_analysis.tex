...
\]
\begin{thm}
  If $\{p_n\}_{k = 0}^n$ are orthogonal polynomials with respect to $\bra \ph, \ph \ket$, then
  \[
...
Suppose we can freely choose $\{a_i\}_{i = 0}^N$ and $\{x_i\}_{i = 0}^N$. Then since we now have $2n + 2$ free parameters, we might expect to find an approximation that is exact for $f \in P_{2N + 1}[x]$. This is not always possible, but there are cases when we can. The most famous example is Gaussian quadrature.
...
\begin{proof}
  ...
  \]
  since $q^2$ is always non-negative and has finitely many zeroes. However,
  \[
    \sum_{k = 1}^\nu b_k q^2(c_n) = 0.
  \]
...
What should we do now? Recall that we wanted to pick the nodes to be the zeroes of the orthogonal polynomial. However, to do this, we must make sure the roots indeed lie in $[a, b]$. This is what we will prove now --- given any inner product, the roots of the orthogonal polynomials must lie in $[a, b]$.
\end{document}
