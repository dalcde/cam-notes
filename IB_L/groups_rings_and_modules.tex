...
It turns out there are no interesting integral domains.
\begin{lemma}
...
\begin{enumerate}
  ...
  \item Let $f$ be irreducible, and suppose $(f) \subseteq J \subseteq \F[X]$. Then $J = (g)$ for some $g$. Since $(f) = (g)$, we must have $f = gh$ for some $h$. But $f$ is irreducible. So either $g$ or $h$ is a unit. If $g$ is a unit, then $(g) = \F[X]$. If $h$ is a unit, then $(f) = (g)$. So $(f)$ is a maximal ideal.
\end{enumerate}
\end{document}
