...
It turns out finite abelian groups are very easy to classify. We can just write down a list of all finite abelian groups. We write down the classification theorem, and then prove it in the last part of the course, where we hit this with a huge hammer.
...
We now move on to something completely different --- rings. In a ring, we are allowed to add, subtract, multiply but not divide. In this course, we are only going to consider rings in which multiplication is commutative, since these rings behave like ``number systems''.
...
\begin{proof}
  ...
  \]
  This is obviously an ideal, and you will check this manually in example sheet 2. % check
...
Here is an important definition, and we will spend some time thinking about this definition.
...
\begin{proof}
  ...
  The remaining implications are left as an exercise for the reader. % complete
\end{proof}
