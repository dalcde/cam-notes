...
Consider the \term{unit disk}\index{$D$}
\[
...
We begin by studying the simplest case of a Ginzburg--Landau vortex. Our field is a complex-valued function $\Phi: V \to \C$, with potential given by
\[
  V(\Phi) = \frac{1}{2} \int \R^2 \left((\nabla \Phi, \nabla \Phi) + \frac{\lambda}{4} (1 - (\Phi, \Phi))^2 \right)\;\d x^1 \;\d x^2.
\]
where $\lambda > 0$.

Recall that the inner product is given by
\[
  V(a, b) = \frac{\bar{a} b + a \bar{b}}{2},
\]
and is invariant under phase rotation, i.e.\ $(e^{i\chi} a, e^{i\chi b}) = (a, b)$ for $\chi \in \R$. So in particular, $V(e^{i\chi}\Phi) = V(\Phi)$. Thus, our theory has a global $\U(1)$ symmetry.

We will restrict to fields satisfying $|\Phi| \in [0, 1]$. To justify this, we first note that the Euler--Lagrange equations says if we have a static solution, then we must have
\[
...
We can now define vortices.

\begin{defi}[Gintzburg--Landau vortex]\index{Gintzburg--Landau vortex}
...
\begin{center}
  ...
  \end{tikzpicture}
\end{center}
This is known as a \emph{$3$-dimensional hedgehog}.

For general $N$, it might be more instructive to look at how the current looks like. Recall that the current is defined by $j = (i\Phi, \nabla \Phi)$. We can write this more explicitly as
\begin{align*}
  (i\Phi, \nabla \Phi) &= (if_N e^{iN\theta}, (\nabla f_N) e^{iN\theta} + i f_N N \nabla \theta e^{iN\theta})\\
  &= (if_N, \nabla f_N + i f_N N \nabla \theta).
\end{align*}
We see that $if_N$ and $\nabla f_N$ are orthogonal, while $i f_N$ and $i f_N N \nabla \theta$ are parallel. So the final result is
\[
  j = (i\Phi, \nabla \Phi) = f_N^2 N \nabla \theta.
\]
...
As $|x| \to \infty$, we have $f_N \to 1$. So the winding number is given by As before, we can compute the winding number of this system to be
\[
...
It turns out this model has a problem --- the energy is infinite! If we look at $V(f_N e^{iN \theta})$, then there are many different terms, all of which are positive. We will focus on the $\frac{\partial}{\partial \theta}$ term. Then we have
\begin{align*}
...
\[
  \D \Phi = \D_A \Phi = \d \Phi - i A \Phi = \sum_{j = 1}^2 (\nabla_A)_j \Phi \;\d x^j = \sum_{j = 1}^2 (\partial_j \Phi - i A \phi) \;\d x^j.
\]
...
\begin{align*}
  \D \Phi &\mapsto (\d - i (A + \d \chi)) (\Phi e^{i \chi}) \\
...
\end{align*}
The last two equations are similar to Maxwell's equation, saying the curl of the magnetic filed is given by the current. The first equation is like the previous vortex equation in the ungauged case, but since we have the covariant derivative, this is a coupled equation.
...
One application of this is for modeling superconductors. We can think of such a fluid as being a combination of ``normal'' matter and superconducting matter, and $|\Phi|$ measures how much superconducting matter we've got. When we apply a weak magnetic field, then it is not able to penetrate the superconductor. As we increase the magnetic field, it begins to penetrate, and produces some vortices in the matter. In the core of these vortices, we have $|\Phi| = 0$, and then we are left completely regular matter. However, far away from the vortices, we have $|\Phi| \approx 1$.

Let's try to look at the solutions. For $\lambda > 0$, there exists vortex solutions of the form
\begin{align*}
  \Phi &= f_N(r) e^{iN\theta}\\
...
\begin{align*}
  N &= \frac{1}{\pi} \int_{\R^2} j^0(\Phi)\;\d x \\
  ...
  &= N
\end{align*}
As we change $N$, we think of the structure as winding around more. What we see next time, though, is that we can think of this as superpositions of multiple vortices.

We can also compute the magnetic field. Note that we have
\[
  B\;\d x^1 \wedge \d x^2 = \d A.
\]
This allows us to conveniently calculate
\[
  B \;\d x^1 \wedge \;\d x^2 = \d A = N \alpha_N'(r) \;\d r \wedge \d \theta = \frac{1}{r} N \alpha_N' \d x^1 \wedge \d x^2.
\]
...
\[
  (\d \Phi_\theta = i A_\theta \Phi + o(r^{-1}).
\]
Then if we integrate around a circular contour, since only the angular part contributes, we obtain
\[
  \oint_{|x| = R} (i \Phi, \d \Phi) = \oint_{|x| = R} (i \Phi, i A \Phi)\;\d \theta = \oint_{|x| = R} A + o(1).
\]
...
\]
With the boundary condition that $f_N$ and $\alpha_N$ vanish at $r = 0$, we expect
\[
  - f_N'' - \frac{1}{r} f_N' + \frac{N^2}{r^2} f_N = 0.
\]
to give a good approximation to $f_N$, since we have a $\frac{1}{r^2}$ on the left hand side. This equation is homogeneous, and then the solutions are just
\[
  f_N = r^{\pm N}.
\]
So for regularity, we want the one that $\to 0$. So we take $f_N \sim r^N$.

This gives us a bit of idea of how to think about vortices, namely as zeroes of analytic functions.

\subsection{Bogomolny/self-dual vortices and Taubes' theorem}
We can think of this solution as a collection of $N$ vortices all superposed at the origin. Is it possible to have separated vortices all over the plane? Naively, we would expect that the vortices exert forces on each other, and so we don't get a static solution. However, it turns out in the $\lambda = 1$ case, we do get static, separated solutions.

So we restrict to $\lambda = 1$. We shall come up with a complete description of all solutions. Recall that when we did the sine-Gordon kinks, we wanted
\[
  \theta'' = \sin \theta,
\]
...
We shall get a similar but more interesting description for the $\lambda = 1$ vortices.

This time, the moduli space will be $\C^N$, given by $N$ complex parameters describing the equilibrium. % quotient by S^n
\begin{thm}[Taubes' theorem] % or taubes' ?
  For $\lambda = 1$, the space of (gauge equivalence classes of) solutions of the Euler--Lagrangian equations $\delta V_1 = 0$ is $\mathcal{M} \cong \C^N$, where $N$ is the winding number. To be precise, given $N \in \N$ and an unordered set of points $\{Z_1, \cdots, Z_N\}$, there exists a solution $A(x; Z_1, \cdots, Z_N)$ and $\Phi(x; Z_1, \cdots, Z_n)$ which solves the Euler-Lagrange equations $\delta V = 0$, and also
  \[
  ...
  Furthermore,
  \[
    V_1(A(\ph, Z_1, \cdots, Z_N), \Phi(\ph; Z_1, \cdots, Z_n)) = \pi N\index{$*$}
  \]
  and
  \[
  ...
  \]
\end{thm}
Let's try to interpret some of these statements. The equation $(*)$ says the energy is just a function of the number of particles, and does not depend on where they are. This means there is no force between the vortices. In situations like this, people say the Bogomolny bound is saturated.

The final statement tells us topology is what is driving the vortices, as we have already seen.

What is the mathematical structure that allows this to happen? It is in the Bogomolny equations, which are \emph{first order} equations. The Euler--Lagrange equations themselves are second order, but the Bogomolny equations are just first-order. This is a mathematical signature that suggests these interesting structures are present.

We'll discuss three crucial ingredients in this, but we will not do the full proof, because it is too much analysis. The proof can be found in a book by \emph{Jaffe and Taubes}
...
\[
  \Phi = e^{\omega} f
\]
where $f$ is holomorphic. Since $e^{\omega}$ is never zero, we can apply all our knowledge of holomorphic functions to $f$, and deduce something about $\Phi$.

Thus, we know that $\Phi$ has isolated zeroes, where $\Phi \sim (z - Z_j)^{n_j}$ for some power $n_j$.
...
\end{align*}
We are now use the kink asymptotic boundary conditions to obtain $\theta(+\infty) = 2\pi$ and $\theta(-\infty) = 0$. So the boundary terms gives $8$. Thus, subject to these boundary conditions, we can write the sine-Gordon energy as
\[
...
\begin{proof}
  ...
  \]
  We now dissect the terms one by one. We first use the definition of $B$ and integration by parts to obtain
  \begin{align*}
...
\]
Suppose we can indeed do so. Then we have
\[
  A_1 = \frac{1}{2}(\partial_2 u + \partial_1 \Theta),\quad A_2 = - \frac{1}{2} (\partial_1 u - \partial_2 \theta).
\]
...
Physically, we can imagine these results that if $d \geq 3$, then there is a tendency for the field to collapse on itself. On the other hand, in $d = 1$, the collapsing and expansion forces perfectly balance and we can obtain stationary solitons.

What happens when we have a gauge theory? We scale our field again as
\[
  \Phi_\lambda (x) = \Phi(\lambda x).
\]
How about the gauge field? We don't scale it in the same way, because we want to have
\[
  \D \Phi_\lambda(x) = \lambda (\D \Phi)(\lambda x).
\]
So we want to set
\[
  A_\lambda(x) = \lambda A(\lambda x).
\]
Geometrically, if we define $\chi_\lambda: \R^d \to \R^d$ by $x \mapsto \lambda x$, then we have
\[
  \chi_\lambda^* (A_j \;\d x^j) = \lambda A_j (\lambda x) \;\d x^j.
\]
So this is the right way to scale it.

Then we find that
\[
  B_\lambda(x) = \lambda^2 B(\lambda x).
\]
We can now see what happens in Derrick's argument. Since we already used the letter $\lambda$, we will denote the scaling parameter by $\mu$. We have
\begin{align*}
...
\]
Thus, the solitons can exist because they are stabilized by the electromagnetic field.

Note that in the case $\lambda = 1$, this equation is just the integral form of one of the Bogomolny equation!

There are different ways to get around Derrick's argument if we want to have interesting solitons. One way is to introduce gauge theory. Another way is to modify the $\Delta \phi$ term to include higher derivatives, which is what the Skyrme model does.

Derrick's discussion suggests the existence of vortices if $\lambda > 0$. However, we do not expect a large family of such vortices --- the fact that multi-vortices exist is the statement that there are no forces between vortices.

By doing numerical simulations, we find that when $\lambda < 1$, the vortices attract. For $\lambda > 1$, the vortices repel.

\begin{eg}
  When $\lambda > 1$, the symmetric vortices with $N > 1$ are unstable, as they want to break up into multiple single vortices.
\end{eg}

\subsubsection*{Scattering of vortices}
We focus on the $\lambda = 1$ case. Then we have static multi-vortices. For example, in $N = 2$, the solutions are parametrized by pairs of points in $\C$, up to
\[
  (Z_1, Z_2) \sim (Z_2, Z_1).
\]
We said the moduli space is $\C^2$, and this is indeed true. However, $Z_1$ and $Z_2$ are no longer good coordinates for this space. Instead, good coordinates on the space $\mathcal{M} = \mathcal{M}_@$ is given by some functions symmetric in $Z_1$ and $Z_2$. One valid choice is
\[
  Q = Z_1 + Z_2,\quad P = Z_1 Z_2.
\]
Let's now consider what happens if we allow these vortices to move.

The initial data of this is the initial velocities of the particles. Suppose we fix the center of mass, so that $Q(t) = 0$. Then we obtain
\[
  P = - Z_1^2.
\]
Now if we can find a solution where $P$ changes sign when it goes through zero, then this corresponds to $Z_1 \mapsto i Z_1$, $Z_2 = - Z_1$.

This is the \emph{$90^\circ$ scattering phenomenon}, and holds for other $\lambda$ as well.

Note that at the point when they collide, we lose track of which vortex is which.

The process of obtaining an effective dynamics for these vortices involve the effective Lagrangian, just as we did for kinks. However, it is rather more complicated.
