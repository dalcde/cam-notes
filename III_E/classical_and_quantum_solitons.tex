...
In the Skyrme model, the field will be valued in $\SU(2)$. It is convenient to introduce coordinates for our Skyrme field\index{Skyrme field}. As usual, we let $\boldsymbol\tau$ be the Pauli matrices. Then we can write the Skyrme field as
\[
...
\]
This is a \emph{non-linear} constraint, and is known as a \term{$\sigma$-model}.
...
\]
This is known as the \term{right current}. From practical, computational purposes, it is convenient to note that
\[
...
On the other hand, we know that $\SU(2)$ is isomorphic to $S^3$. Geometrically, we think of these as ``different'' $S^3$. We should think of $\SU(3)$ as the ``unit sphere'', and write it as $S^3_1$. However, we can think of the compactification of $\R^3$ as a sphere with ``infinite radius'', so we denote it as $S^3_\infty$. Of course, topologically, they are the same.
...
\]
The factor of $2\pi^2$ comes from the volume of the three sphere, and there is also a factor of $6$ coming from how we anti-symmetrize the term.

If we were to derive this, then we would have to pull back a normalized volume form from $S_1^3$ and then integrate over all space. In this formula, we chose to use the $\SO(4)$-invariant volume form, but in general, we can pull back any volume form.
...
\]
This is manifestly in $\SU(2)$, because $\cos^2 f + \sin^2 f = 1$. We need some boundary conditions. We need $U \to 1$ at $\infty$. On the other hand, we will see that we need $U \to -\mathbf{1}$ at the origin to get baryon number $1$. So $f \to \pi$ as $r \to 0$, and $f \to 0$ as $r \to \infty$. So it looks roughly like this: % concave upwards
\begin{center}
  \begin{tikzpicture}
...
In our $B > 1$ spherically symmetric hedgehogs, we wrapped around the sphere many times, and it turns out that was not a good idea.
...
But we don't have to! Pick any function $R: S^2 \to S^2$. Then we can construct the map $\Sigma_f R$ that sends $S_r^2$ to $S_{f(r)}^2$ via the map $R$. For simplicity, we will use the same $R$ for all $r$. If we did this, then we obtain a non-trivial map $\Sigma_f R: S^3 \to S^3$.

Since $R$ itself is a map from a sphere to a sphere (but one dimension lower), $R$ also has a degree. It turns out this degree is the same as the degree of the induced map $\Sigma_f R$! So to produce higher Baryon number hedgehogs, we simply have to find maps $R: S^2 \to S^2$ of high degree.
...
There is quite a lot of freedom in this construction, since we are free to pick $f(r)$, as well as the radial function $R_k(z)$. The geometric degree $k$ of $R_k(z)$ we care about is the same as the algebraic degree\index{degree!algebraic}\index{degree!rational function} of $R_k(z)$. Precisely, if we write
\[
  R_k(z) = \frac{p_k(z)}{q_k(z)},
\]
where $p_k$ and $q_k$ are coprime, then the algebraic degree of $R$ is the maximum of the degrees of $p_k$ and $q_k$ as polynomials. Since there are finitely many coefficients for $p_k$ and $q_k$, this is a finite-dimensional problem, which is much easier than solving for arbitrary functions. We will talk about this degree later.
...
\end{align*}
We can actually do the simulations and obtain
\begin{center}
  \includegraphics[width=\textwidth]{images/B1-8-Skyrmions.pdf}

  {By R.\ A.\ Battye and P.\ M.\ Sutcliffe}
\end{center}
...
\]
This is known as the \term{Wronskian}.
...
\begin{eg}
  ...
  Generically, we expect there to be $6$ roots. It turns out the Wronksian has a zero at $\infty$ as well. To see this more rigorously, we can rotate the Riemann sphere a bit by a M\"obius map, and then see there are $6$ finite roots. Looking back at our previous picture, this is indeed the case.
\end{eg}
...
\end{center}
This is obtained by putting 8 many $B = 4$ Skyrmions side by side, and then cutting off a corner.

This strategy tends to work quite will, actually. With this idea, we can in fact find Skyrmion solutions with Baryon number infinity! We can form an infinite cubic crystal out of $B = 4$ subunits. For $m = 0$, the energy per baryon is $\approx 1.038 \times (12 \pi^2 B)$. This is a very close to the lower bound!

We can also do other interesting things. In the picture below, on the left, we have a usual $B = 7$ Skyrmion. On the right, we have deformed the Skyrmion into what looks like a $B = 4$ Skyrmion and a $B = 3$ Skyrmion put together. This is a cluster structure, and it turns out this deformation doesn't cost us a lot of energy. This can be used as a model of the lithium-7 nucleus.
\begin{center}
...
We now try to understand what happens when we put different $B = 1$ hedgehogs next to each other. To understand this, we look at the profile function $f$, for $m = 0$. For large $r$, this has the asymptotic form
\[
  f(r) \sim \frac{C}{r^2}.
\]
...
But these two Skyrmions attract! So two Skyrmions in this ``attractive channel'' can merge to form the $B = 2$ torus, which is the true minimal energy solution.
...
As a field theory, the configuration space for a baryon number $B$ Skyrme field is $\Maps_B(\R^3 \to \SU(2))$, with appropriate boundary conditions. These are all topologically the same as $\Maps_0(\R^3 \to \SU(2))$, because if we fix a single element $f \in \Maps_B(\R^3 \to \SU(2))$, then multiplication by $f$ gives us a homeomorphisms between the two spaces. Since we imposed the vacuum boundary conditions, this is also the same as $\Maps_0(S^3 \to S^3)$.
...
\]
The translation part is trivial. The Skyrmion just gets the ability to move and round, and thus gains momentum. So we are going to ignore it. The remaining group acting is $\SO(3) \times \SO(3)$. This is a bit subtle. We have $\pi_1(\SO(3)) = \Z_2$, so we would expect $\pi_1(\SO(3) \times \SO(3)) = (\Z_2)^2$. However, in the full theory, we only have a $\pi_1$ of $\Z_2$. So we need to identify a loop in the first $\SO(3)$ with a loop in the second $\SO(3)$.

Our wavefunction is thus a function on some cover of $\SO(3) \times \SO(3)$. While $\SO(3) \times \SO(3)$ is the symmetry group of the full theory, for a particular classical Skyrmion solution, it has a smaller symmetry group. For example, the $B = 3$ solution has a tetrahedral symmetry. Then we require our wavefunction to be invariant under this group up to a sign.

For a single $\SO(3)$, we have a rigid-body wavefunction $\bket{J\; L_3\; J_3}$, where $J$ is the spin, $L_3$ is the third component of spin relative to body axis, and $J_3$ is the third component of spin relative to space axis. Since we have two copies of $\SO(3)$, our wavefunction can be represented by
\[
...
\begin{eg}
  ...
  Since we have an odd number of Skyrmions, then for any axis $\hat{\mathbf{n}}$, we must have
  \[
