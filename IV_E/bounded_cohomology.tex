...
\begin{proof}
  ...
  \]
  Now let $m \colon \ell^\infty(\Gamma) \to \R$ be a left-invariant mean. We notice that the map
  \[
...
\begin{ex}
  Show that if $h\colon \Z \to \Homeo^+(S^1)$ and $\varphi = h(1)$, then under the isomorphism
  \[
    H^2_b(\Z, \Z) \to \R/\Z,
  \]
...
\begin{proof}[Proof sketch]
  First one uses compactness and Zorn's lemma to show the existence of a minimal, non-empty, closed, invariant subset.
...
From this, and a bit more work, we can deduce the following corollary.
\begin{cor}
