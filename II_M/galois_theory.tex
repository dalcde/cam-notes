...
\begin{defi}[Splitting field]
  ...
  \]
  for all $a \in K$ and $\alpha_j \in L$, ie. we can factorize it into a product of linear terms. Alternatively, this says that $L$ contains all roots of $f$.
...
\begin{proof}\leavevmode
  \begin{enumerate}
    ...
    \item Assume $L$ and $L'$ are both splitting fields of $f$ over $K$. We want to find a $K$-isomorphism from $L$ to $L'$.
      ...
      Now we know that there is an isomorphism $F[t]/\bra g\ket \to F(\alpha)$ by $\bar t \mapsto \alpha$. The isomorphism $\psi: F \to F'$ extends to a isomorphism $\mu: F[t] \to F'[t]$. Then since the coefficients of $f$ are in $K$, we have  $f = \mu(f) = \mu(g)\mu(h)$.

      So $\mu(g) | f$ in $F'[t]$. Since $g$ is irreducible in $F[t]$, $\mu(g)$ is irreducible in $F'[t]$. So by the previous proposition, there is some $\alpha' \in \Root_{\mu(g)}(L') \subseteq \Root_f(L')$ and isomorphism $F'[t]/\bra \mu(g)\ket \to F'(\alpha')$.
