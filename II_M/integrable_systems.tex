...
Now we want to look informally at Lie algebras. Suppose we have a curve $\mathbf{x}_1(\varepsilon)$ in our parameter space passing through $0$ at time $0$. Then we can obtain a curve
\[
  A(\varepsilon) = g(\mathbf{x}_1(t))
\]
in our Lie group $G$. We then set $a = A'(0)$. Then we can write
\[
  A(\varepsilon) = I + \varepsilon a + o(\varepsilon).
\]
Now suppose we have a second curve $B(x)$, which we expand similarly as
\[
  B(\varepsilon) = I + \varepsilon b + o (\varepsilon).
\]
Let $\mathfrak{g}$ be the set of all possible leading order terms from such curve.
...
\]
So if $a, b \in \mathfrak{g}$, then so is $a + b \in \mathfrak{g}$. Also, for $\lambda \in \R$, we can construct a curve
\[
  t \mapsto A(\lambda t).
\]
Then we have
...
So we get that $\mathfrak{g}$ has the structure of a vector space! This is already a little interesting. Groups are complicated. They have this werid structure and they are not necessarily commutative. However, we get a nice, easy vector space structure form the group structure.

We can now define a curve $C(t)$ for $t > 0$ by
\[
...
\end{align*}
In is an exercise to show that this is actually true, because we have to keep track of the second order terms we didn't write out to make sure it does cancel.

So if $a, b \in \mathfrak{g}$, then
\[
  [a, b]_L = ab - ba \in \mathfrak{g}.
\]
Vector spaces with this extra structure is called a \emph{Lie algebra}. The idea is that the Lie algebra consists of elements of the group infinitesimally close to the identity. While the product of two elements $a, b$ infinitesimally close to the identity need not remain infinitesimally close to the identity, the commutator $ab - ba$ does (at least, if we think of these as matrices; subtracting group elements don't make sense in general).
...
\begin{eg}
  ...
  We now want to check that $\mathfrak{g}$ is closed under the commutator
  \[
    [a, b]_L = ab - ba.
  \]
  We check that
  ...
  \]
  So this is indeed antisymmetric. So it is reasonable to assume that
  \[
    \so(n) = \{a \in \Mat_n(\R): a + a^T = 0\}.
  \]
  Note that we actually have to check that every antisymmetric matrix comes from some curve. It is an exercise to check that the curve
  \[
    A(t) = \exp(at).
  \]
  works.
...
Note that it is standard that if we have a group whose name is in capital letters (eg. $\SO(n)$), then the corresopnding Lie algebra is the same thing in lower case, fraktur letters (eg. $\so(n)$).
...
Ultimately, we will be interested in coordinate transformations born of the action of some Lie group. In other words, we think of new coordinates
\[
  \tilde{\mathbf{x}} = g(\mathbf{x}),
\]
where $\mathfrak{g} \in G$ for some Lie group $G$. For example, if $G$ is the group of rotations, then we can get new coordinates by rotating.
...
\]
So we call $V$ the generator of the flow. This flow map is an example of a \emph{one-parameter group of transformations}.
\begin{defi}[One-parameter group of transformations]\index{one-parameter group of transformations}
  A smooth map $g^\varepsilon: \R^n \to \R^n$ is called a \emph{one-parameter group of transformations} if
  \[
  ...
  \]
  Also, every vector field $\mathbf{V}: \R^n \to \R^n$ generates a one-parameter group of transformations via solutions of
  \[
    \frac{\d}{\d \varepsilon} \tilde{\mathbf{x}} = \mathbf{V}(\tilde{\mathbf{x}}),\quad \tilde{\mathbf{x}}(0) = \mathbf{x}.
  \]
\end{defi}
We are now going to introduce some standard but odd-looking notation.
\begin{notation}
  ...
  \]
  Conversely, any linear differential operator gives us a vector field like that. We will now refer to such differential operators as vector fields. So we think of the $\frac{\partial}{\partial x_i}$ as a basis for our vector field.
\end{notation}
...
\]
Now if we think of the vector field as a differential operator, then we simply have
\[
...
\begin{eg}
  ...
  \[
    (\tilde{x}(\varepsilon), \tilde{y}(\varepsilon)) = (xe^{\varepsilon}, y + \varepsilon) \equiv g^\varepsilon (x, y).
  \]
\end{eg}

\begin{eg}
  Consider the action of $\SO(2) \cong S^1$ on $\R^2$ via
  \[
...
\end{eg}
In general, for any $g(\mathbf{t}) \in G$ (in a neighbourhood of $e \in G$) via a sequence of one-parameter group of transformations
\[
...
Suppose $u = u(\mathbf{x})$ is a solution to a differential equation
\[
  \Delta [\mathbf{x}, u, \partial u/\partial \mathbf{x}, \cdots] = 0.
\]
Let $g^\varepsilon$ be a $1$-parameter group of transformations generated by a vector field $V$, and consider the new coordinates
\[
  (\tilde{\mathbf{x}}, \tilde{u}) = g^\varepsilon(\mathbf{x}, u).
\]
We call $g^\varepsilon$ a \term{Lie point symmetry} of $\Delta$ if
\[
  \Delta[\tilde{\mathbf{x}}, \tilde{\mathbf{u}}, \partial\tilde{u}/\tilde{\mathbf{x}}, \cdots] = 0
\]
whenever $\Delta [\mathbf{x}, u, \partial u/\partial \mathbf{x}, \cdots] = 0$. In other words, it takes solutions to solutions.
...
\end{eg}
Obviously Lie point symmetries give us new solutions from old ones. They are also important in problems of reduction. We first do an example, before we consider the general theory.
\begin{eg}
  Consider the ODE
  \[
  ...
  \]
  We introduce new coordinates $(s, t)$ defined by
  \[
    s = \frac{u}{x}, t = \log |x|,
  \]
  which satisfies $V(s) = 0, V(t) = 1$. We call these ``invariant coordinates''. Note that we can write this as
  \[
    x = e^t, \quad u = se^t.
  \]
  ...
  \[
    V = \frac{\partial}{\partial t}.
  \]
  These coordinates are handy, because in these coordinates, the symmetry looks very simple. Since we are told that $\frac{\partial}{\partial t} = 0$, we know that the differential equation would have no explicit $t$-dependence.
  ...
  Since there are no explicit $t$'s on the right hand side, we can actually integrate this thing up. We can write this as
  \[
  ...
  \]
  So the knowledge of the Lie point symmetry allowed us to integrate up our ODE via quadratures.
\end{eg}
...
\]
ie. we have reduced the order of the ODE by $1$. Now carry on.

Now this is nice, but we still need to find the Lie point symmetries. So far, we have just written them down. We want a systematic way of finding them.

Consider the $0$th order ODE
\[
  \Delta[x, u] = 0.
\]
Then we know $g^\varepsilon: (x, u) \mapsto (\tilde{x}, \tilde{u})$ is a Lie point symmetry if
\[
  \Delta[\tilde{x}, \tilde{u}] = \Delta[g^\varepsilon(x, u)] = 0
\]
when $\Delta[x, u] = 0$.

Now if $V$ is a generator of $g^\varepsilon$, then an equivalent statement (see example sheet 3) is
\[
  V(\Delta) = 0
\]
whenever $\Delta = 0$. This essentially says that the flow doesn't change $\Delta$ iff the derivative of $\Delta$ along $V$ is constantly zero, which makes sense. Here we are thinking of $V$ as a differential operator.

We call this constraint an \term{on-shell}, because we only impose it whenever $\Delta = 0$ is satisfied, instead of at all points.

This equivalent statement is very easy! This is just an algebraic equation for the coefficients of $V$, and it is very easy to solve!

However, these aren't really ODE's. They are just equations. So how do we generalize this to $N \geq 1$ order ODE's? Consider a general vector field
\[
  V(x, u) = \xi(x, u) \frac{\partial}{\partial x} + \eta(x, u) \frac{\partial}{\partial u}.
\]
This only knows what to do to $x$ and $u$. But know how $x$ and $u$ change, we should also know how $u_x, u_{xx}$ etc. change. Indeed this is true, and this is known as the prolongation of the vector field.
...
\]
If we call $(x, u)$ coordinates for the \term{base space}, then we call the extended system $(x, u, u_x)$ coordinates for the \term{first jet space}. So what we've seen is that a one-parameter group of transformation of the base space induces a one-parameter group of transformation of the first jet space. This is known as the \term{prolongation}, written
\[
  \pr^{(1)}g^\varepsilon: (x, u, u_x) \mapsto (\tilde{x}, \tilde{u}, \tilde{u}_{\tilde{x}} = (e^\varepsilon x, e^{-\varepsilon} u, e^{-2\varepsilon} u_x).
\]
It is a bit strange to call $u_x$ a coordinate. If we don't like to do that, we can just replace $u_x$ with $p_1$. If we have the $n$th derivative, we just replace the $n$th derivative with $p_n$.
...
\]
as an algebraic equation on the $n$th jet space. So we have dressed up our problem into an algebraic problem, just like the case of $\Delta[x, u] = 0$ we discussed at the beginning. Then $g^\varepsilon: (x, u) \mapsto (\tilde{x}, \tilde{u})$ is a Lie point symmetry if
\[
...
\]
when $\Delta = 0$. This is just a one-parameter group of transformations on a huge coordinate system. Thinking of all $x, u, \cdots, u^{(n)}$ as just independent coordinates, we can rewrite it in terms of vector fields. This just says
\[
  \pr^{(n)}V (\Delta) = 0.
\]
...
If we tried to compute the prolongation of the action of the Lie \emph{group}, then it would be horrendous. However, what we actually need to compute is the prolongation of the vector field, which is about the Lie \emph{algebra}. This makes it much nicer.
...
\]
where we have to find out what $\eta_k$ is. Before we try to find out a formula for this, we that the $\eta_k$ must satisfy
\begin{align*}
...
\end{align*}
We now see what our problem is. When $x, u$ change, we want to know he corresponding change of $u^{(k)}$ up to first order.
...
Now if someone just gives us a PDE, how can we figure out if it is integrable? We start with something we've learnt in IA Differential Equations (not really).
...
We are interested in ODE's of the form
\[
  \frac{\d^2 w}{\d z^2} + p(z) \frac{\d w}{ \d z} + q(z) w = 0,
\]
and we want to know if the solutions have singularities.

\emph{Any} singularity of a solution $w = w(z)$ is inherited from the functions $p(z), q(z)$. The locations of the singularities will \emph{not} depend on initial conditions $w(z_0), w'(z_0)$.

This is certainly \emph{not} the case for non-linear ODE's. For example, the equation
\[
  \frac{\d w}{\d z} + w^2 = 0
\]
gives us
...
\begin{eg}
  ...
  \[
    w(z) = \frac{1}{z - z_0}.
  \]
  Since this movable singularity is a pole, this as the \emph{Painleve\'e property}.
\end{eg}
...
The remaining six gave way to solutions that were genuinely new functions, called the six \term{Painlev\'e transcendents}. % insert Painlev\'e equations

Fun fact: Painlev\'e served as the prime minister of France for two years.

This is all good, but what has this got to do with integrability of PDE's?

There is a Ablowitz-Ramani-Segur conjecture:
\begin{conjecture}[Ablowitz-Ramani-Segur conjecture]\index{Ablowitz-Ramani-Segur conjecture}
  Every ODE reduction of an integrable PDE has the Painlev\'e property.
\end{conjecture}
What do we mean by ODE reduction? If we have a Lie point symmetry of a PDE, then we can introduce coordinates that are invariant and form subsequence ODE's in these coordinates.

\begin{eg}
  In the wave equation, we can try a solution of the form $u(x, t) = f(x - ct)$, and then the wave equation gives us an ODE in terms of $f$.
\end{eg}

This is a conjecture, because we have a handwavy ``proof'' of it, but not a formal proof.
...
\begin{eg}
  ...
  \]
  for some function $w$. So we have
  \[
    u_t = - \frac{1}{5} - t w'(z),\quad u_x = w'(z),\quad u_{xx} = w''(z),\quad u_{xxx} = w'''(z).
  \]
  So KdV becomes
  \[
    0 = u_t + u_{xxx} - 6 uu_x = - \frac{1}{6} + w'''(t) - 6w w'(t).
  \]
  We would have had some problems if the $t$'s didn't get away, because we wouldn't have an ODE in $w$. But since we constructed these coordinates out of Lie-point symmetries, we are guaranteed that the things go away.
...
\end{enumerate}
We can then see if our PDE is \emph{not} integrable.

Unfortunately, there is no real test for the converse.
\printindex
\end{document}
