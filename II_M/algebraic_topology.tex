...
\begin{eg}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw [->] (1.8, 3) -- +(0, -2.3) node [pos=0.5, right] {$p$};
    \end{tikzpicture}
  \end{center}
...
\begin{lemma}
  ...
  \begin{enumerate}
    \item The action of $\pi_1(X, x_0)$ on $p^{-1}(x_0)$ is transitive if (and only if) $\tilde{X}$ is path connected.
...
\begin{proof}\leavevmode
  \begin{enumerate}
    \item Choose a path $\tilde{\gamma}: \tilde{x}_0 \to \tilde{x}_0'$. This projects to a loop at $x_0$ in $X$.
    \item $\gamma$ stabilizes $\tilde{x}_0$ iff $\gamma$ lifts to a loop.
  \end{enumerate}
\end{proof}
\end{document}
