...
\begin{defi}[Presentation of a group]
  ...
  \]
\end{defi}
This is just the usual notation we have for groups, eg. $D_{2n} = \bra r, s \mid r^n, s^2, srsr\ket$.
...
\begin{eg}
  ...
  \begin{center}
    \begin{tikzpicture}
      \foreach \x/\r in {2/0, -2/180} {
        \begin{scope}[shift={(\x, 0)}, rotate=\r]
  ...
  So we get
  \[
    \pi_1 F_g = \bra a_1, b_1, \cdots, a_g, b_g\mid a_1 b_1 a_1^{-1} b_1^{-1} \cdots a_g b_g a_g^{-1}b_g^{-1}\ket.
  \]
...
\begin{proof}(of simplicial approximation theorem)
  ...
  By the Lebesgue number lemma, there exists some $\delta$, the \emph{Lebesgue number} of the cover, such that for each $x \in |K|$, $B_\delta(x)$ is contained in \emph{some} element of the cover. By the previous lemma, there is an $r$ such that $\mu(K^{(r)}) < \delta$. % insert Lebesgue number lemma somewhere.
...
\begin{eg}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \draw [->-=0.58] (1, 1.732) -- (2, 0) node [pos=0.5, right] {$\sigma_2$};
...
\begin{defi}[Boundary and cycles]
  The \emph{$k$-cycles} of $K$ are $\ker d_k$. The \emph{$k$-boundaries} are $\im d_{k + 1}$.
\end{defi}
...
Now if $c \in \im d_2$, then $c = db$ for some 2-chain $b$, then $c$ is the boundary of some two-dimensional thing. This is why we call this a 1-boundary. For example, suppose we have our cycle as above, but is itself a boundary of a 2-chain.
\begin{center}
  \begin{tikzpicture}
...
\begin{eg}
  ...
  \]
  After doing some row operations with our matrix, we see that the image of $d_1$ is a two-dimensional subspace generated by two of the edges. Hence we have
  \[
    H_0(K) = \Z.
  \]
...
\[
  ...
  \end{tikzcd}
\]
The intuition behind this definition is as follows: suppose $C_{\Cdot} = C_{\Cdot} (K)$ and $D_{\Cdot} = C_{\Cdot}(L)$ for $K, L$ simplicial complexes, and $f_{\Cdot}$ and $g_{\Cdot}$ are ``induced'' by simplicial maps $f, g: K \to L$. Suppose $H: |K| \times I \to |L|$ is a homotopy from $f$ to $g$. We also suppose that $H$ actually comes from a simplicial map $K \times I \to L$.

There is a slight problem here, since we don't exactly know how we can make $K \times I$ a simplicial complex. We will not go into the details of this construction, but leave it for the last question on example sheet 3.
...
\]
We think of this as an $(n + 1)$-chain. What is its boundary? We've got the vertical sides plus the top and bottom. What are these? The bottom should just be $f(\sigma)$, and the top is just $g(\sigma)$, since $H$ is an homotopy from $f$ to $g$. How about the sides? They are what we get when we pull the boundary $\partial \sigma$ up with the homotopy, ie. $H(\partial \sigma \times I) = h_{n - 1} \circ d_n(\sigma)$. Now note that $f(\sigma)$ and $g(\sigma)$ have opposite orientations, so we get the result
\[
...
\end{proof}
It is an exercise to check that being chain homotopic is an equivalence relation of chain maps. Also, if $a_0: A_0 \to C_0$ is a chain map and $f_{\Cdot} \simeq g_{\Cdot}$, then $f_{\Cdot} \circ a_{\Cdot} \simeq g_{\Cdot} \circ a_{\Cdot}$.
...
\begin{defi}[Chain homotopy equivalence]
  ...
  \[
    f_{\Cdot} \circ g_{\Cdot} \simeq \id_{D_{\Cdot}},\quad g_{\Cdot} \simeq \id_{C_{\Cdot}}.
  \]
\end{defi}
We should think of this in exactly the same way as we think of homotopy equivalences of spaces. The chain complexes themselves are not necessarily the same, but the induced homology groups will be.
 ...
 % insert obvious things about functoriality.
\begin{lemma}
...
\begin{cor}
  Let $K$ be the standard $(n - 1)$ simplex (ie. the proper faces of $L$ from above). Then for $n \geq 2$, we have
  \[
    H_k(K) =
    \begin{cases}
      \Z & k = 0\\
...
Algebraically, we can think of an exact sequence as chain complexes with trivial homology groups. Alternatively, we see the homology groups as measuring the failure of a sequence to be exact. However, this is often not a very helpful to think about it.
...
The Mayer-Vietoris theorem is a powerful theorem we will use, but the result by itself is often not useful, since we don't know what the map $\partial_*$ is. To understand this map, we need to know how to prove the previous theorem.
\begin{thm}
  If we have a short exact sequence of complexes
  \[
    \begin{tikzcd}
...
\begin{proof}
  The proof of this is in general not hard. It just involves a lot of checking of the details, such as making sure the homomorphisms are well-defined, are actually homomorphisms, are exact at all the places etc. The only important and non-trivial part is just the construction of the map $\partial_*$. Other than this, we will not spend our time trying to pin down all the details.

  First we look at the following commutative diagram:
  \[
    \begin{tikzcd}
      0 \ar[r] & A_n \ar[r, "i_n"] \ar[d, "d_n"] & B_n \ar[r, "j_n"] \ar[d, "d_n"] & C_n \ar[r] \ar[d, "d_n"] & 0\\
      0 \ar[r] & A_{n - 1} \ar[r, "i_{n - 1}"] & B_{n - 1} \ar[r, "i_{n - 1}"] & C_{n - 1} \ar[r] & 0
    \end{tikzcd}
  \]
  ...
  \end{enumerate}
  Let's just do the easiest ones. We check $d_{n - 1}(z) = 0$. To do so, we need to add an additional layer.
  \[
    \begin{tikzcd}
      0 \ar[r] & A_n \ar[r, "i_n"] \ar[d, "d_n"] & B_n \ar[r, "j_n"] \ar[d, "d_n"] & C_n \ar[r] \ar[d, "d_n"] & 0\\
      0 \ar[r] & A_{n - 1} \ar[r, "i_{n - 1}"] \ar[d, "d_{n - 1}"] & B_n \ar[r, "j_{n - 1}"] \ar[d, "d_{n - 1}"] & C_n \ar[r] \ar[d, "d_{n - 1}"] & 0\\
      0 \ar[r] & A_{n - 2} \ar[r, "i_{n - 2}"] & B_{n - 2} \ar[r, "i_{n - 2}"] & C_{n - 2} \ar[r] & 0
    \end{tikzcd}
  \]
...
\begin{defi}[Contiguous]
  ...
  \end{center}
\end{defi}
Why do we want this definition? The significance is the following result:
...
\begin{proof}
  ...
  \[
    ...
    \end{cases}
  \]
  So only $n = 0$ is (a little) interesting, but it is easy to check that $a_*$ is an isomorphism in this case.
...
We now have everything we need to move from simplical maps to continuous mpas. Putting everything we have proved so far together, we obtain the following result:
...
\begin{cor}
  If $f: |K| to |L|$ is a homeomorphism, then $F_*: H_n(K) \to H_n(L)$ is an isomoprhism for all $n$.
\end{cor}
...
\begin{lemma}
  Let $L$ be a simplicial complex (with $|L| \subseteq \R^n$), there is an $\varepsilon = \varepsilon(L) > 0$ such that if $f, g: |K| \to |L|$ satisfying $\|f(x) - g(x)\| < \varepsilon$, then $f_* = g_*: H_n(K) \to H_n(L)$ for all $n$.
\end{lemma}
...
\begin{proof}
  Use the previous lemma, and the compactness of $|K| \times I$. Details are left for the reader.
\end{proof}
...
At first, we motivated the study of algebraic topology by saying we wanted to show that $\R^n$ and $\R^m$ are not homeomorphic. However, historically, this is not why people studied homology groups, since there are other ways to prove this is true.
...
\]
This triangulation works nicely with the antipodal, since this maps a vertex to a vertex. To understand the fundamental group better, we need the following lemma:

\begin{lemma}
  In the triangulation of $S^n$ given by vertices $V_k = \{\pm\mathbf{e}_0, \pm \mathbf{e}_1, \cdots, \pm\mathbf{e}_n\}$. Then the element
  \[
...
Why do we want to care about the Lefschetz number? The important thing is that we will use this to prove a generalization of Brouwer's fixed point theorem that allows us to talk about things that are not balls.
...
\[
  \chi(X) = \sum_{i \geq 0} (-1)^i \text{ number of $i$ simplexes}.
\]
...
\begin{eg}
  If $X$ is \emph{any} contractible polyhedron (eg. a disk), then $L(f) = 1$ for any $f: X \to X$, which is obvious once we contract the space to a point. So $X$ has a fixed point.
\end{eg}
