...
\begin{proof}
  ...
  \[
    ...
    \end{tikzcd}
  \]
  We want to check that $d_{n - 1}(z) = 0$. We want to use the commutativity of the diagram. In particular, we know
  \[
...
\begin{prop}
  For $a: K' \to K$ a simplicial approximation to the identity, the induced map $a_*: H_n(K') \to H_n(K)$ is an isomorphism for all $n$.
\end{prop}
...
\begin{proof}
  ...
  \[
    ...
    \end{cases}
  \]
  So only $n = 0$ is (a little) interesting, but it is easy to check that $a_*$ is an isomorphism.
