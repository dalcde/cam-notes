...
\begin{lemma}
  ...
  \begin{enumerate}
    \item For each $x_1 \in E_1$, the function $x_2 \mapsto f(x_1, x_2)$ is $\mathcal{E}_2$-measurable.
...
\begin{proof}
  ...
  \]
  The proof of the case of $p = \infty$ is similar. Now assume $p \in (1, \infty)$. We assume that $\|f\|_p, \|g\|< \infty$, or else the inequality is trivial. Similarly, we assume $\|f + g\|_p \geq 0$.
...
The main goal is to show that if $f \in L^1, \hat{f} \in L^1$, then the Fourier inversion formula holds:
\[
...
\begin{proof}
  ...
  \begin{align*}
    e(y) &\leq \int 2^{p - 1}(|h(x - y)|^p + |h(x)|^p)\;\d x\\
    &= 2^p \|h\|_p^p.
  \end{align*}
...
\begin{proof}[Proof of Fourier inversion theorem]
  ...
  \]
  By the first part, we know that $\|f_t - f\| \to 0$ as $t \to 0$. So we can find a sequence $(t_n)$ with $t_n > 0$, $t_n \to 0$ so that $f_{t_n} \to f$ a.e. Combining these, we know that
  \[
    f(x) = \int \hat{f} e^{-i(u, x)}\;\d u \text{ a.e.}
  \]
\end{proof}

\subsubsection*{Fourier transform and \texorpdfstring{$L^2$}{L^2}}
...
\begin{proof}
  ...
  Since $f_t, \hat{f}_t \in L^2$, we know that the Plancherel identity holds, ie.
  \[
...
\begin{proof}
  ...
  \]
  By the Plancherel identity, we know $F_0$ preserves the $L^2$ norm, ie.
  \[
    \|F_0([f])\|_2 = \|[f]\|_2.
  \]
  Also, we know that $\mathcal{L}^1 \cap \mathcal{L}^2$ is dense in $\mathcal{L}^2$, since it contains all Gaussian convolutions. So we know $F_0$ extends uniquely to an isometry $F: \mathcal{L}^2 \to \mathcal{L}^2$.
  ...
  \]
  is sent to itself by the map $F$. Also if $f \in V$, then $F^4[f] = [f]$ (note that applying it twice does not suffice, because we actually have $F^2[f](x) = [f](-x)$). So $V$ is contained in the image $F$, and also $V$ is dense in $\mathcal{L}^2$, again because it contains all Gaussian convolutions. So we know that $F$ is surjective.
\end{proof}
