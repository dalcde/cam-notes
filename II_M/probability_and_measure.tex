...
\]
\subsection{New measure spaces from old}
We are now going to look at different ways of producing new measure spaces from old ones.
...
\begin{defi}[Restriction of measure space]\index{restriction of measure space}\index{measure space!restriction}
  ...
  \]
  and $\mu_A$ is the restriction of $\mu$ to $\mathcal{E}_A$, ie.
  \[
    \mu_A(B) = \mu(B)
  \]
...
\]
In the particular case of Lebesgue integration, if $A$ is an interval with left and right end points $a, b$ (ie. it can be open, closed, half open or half closed), then we write
\[
  \int_A f\;\d \mu = \int_a^b f(x) \;\d x.
\]
There is another construction we would be interested in.
\begin{defi}[Pushforward/image of measrue]\index{pushforward of measure}\index{image measure}\index{measure!pushforward}\index{measure!image}
...
\begin{defi}[Density]\index{density!random variable}\index{random variable!density}
  ...
  \[
    \E[g(X)] = \int_\Omega g(x) f_X(x) \;\d x.
  \]
\end{defi}

We are now going to prove the change of variables formula. As explained earlier, the Lebesgue integral and Riemann integral agree when both are defined, we know that the fundamental theorem of calculus holds for the Lebesgue integral.
...
\begin{proof}
  ...
  \]
  We will want to use the monotone class theorem to show that this includes all bounded functions. Then we know that
  \begin{enumerate}
...
Now we ask the question --- when can we switch integration and differentiation?
...
\begin{thm}
  ...
  \begin{enumerate}
    ...
    \item There exists an integrable function $g$ such that
      \[
        \left|\frac{\d}{\d t}f(t, x)\right| \leq g(x)
      \]
      for all $x \in E$ and $t \in U$.
  \end{enumerate}
  Then the map
  \[
    x \mapsto \frac{\partial f}{\partial x}(t, x)
  \]
  is integrable for all $t$, then the function
  \[
...
\begin{proof}
  ...
  \]
  Ssince $f$ is differentiable, we know that $g_n(x) \to 0$ as $n \to \infty$. Moreover, we know that
  \[
    |g_n(x)| \leq 2g(x)
  \]
...
\begin{defi}[Product $\sigma$-algebra]\index{product $\sigma$-algebra}\index{$\sigma$-algebra!proudct}
  Let $(E_1, \mathcal{E}_1, \mu_1)$ and $(E_2, \mathcal{E}_2, \mu_2)$ be finite measure spaes. We let
  \[
...
\begin{lemma}
  ...
  \begin{enumerate}
    \item For each $x_1 \in E_1$, the function $x_2 \mapsto f(x_1, x_2)$ is $\mathcal{E}_2$-measuralbe.
    \item If $f$ is bounded or non-negative measurable, tthen
      \[
...
\begin{proof}
  We are again going to use the monotone class theorem. Let $V$ be the set of all $\mathcal{E}$-measuralbe functoins such that (i) holds. We have
  \begin{enumerate}
  ...
  If $f$ is $\mathcal{E}$-meausrable, then $(f \vee (-n))\wedge n$ is a bounded, measurable function. So $f \vee (-n) \vee n \in V$ for all $n$. So
  \[
    f = \lim_{n \to \infty} (f \vee ((-n))\wedge n \in V.
  \]
  So we are done.

  Alternatively, this follows from the fact that the map $\iota_2: E_2 \to E$ given by $\iota_2(x_2) = (x_1, x_2)$ is measurable, and that the composition of measurable functions is measurable.

  For the second part, we again let $V$ be the set of all measurable functions $f$ such that $x_2 \mapsto \int_{E_1} f(x_1, x_2) \mu_1(\d x_1)$ is $\mathcal{E}_2$-measurable.
  \begin{enumerate}
    \item It is clear that $\mathbf{1}_E, \mathbf{1}_A \in V$ for all $A \in \mathcal{A}$.
    \item $V$ is a vector space by linearity of the integral.
    \item Suppose $(f_n)$ is a non-negative sequence in $V$ and $f_n \nearrow f$, then
      \[
        \left(x_2 \mapsto \int_{E_1} f_n(x_1, x_2)\; \mu_1(\d x_1)\right) \nearrow \left(x_2 \mapsto \int_{E_1} f(x_1, x-2)\;\mu(\d x_1)\right)
      \]
  ...
  Now if $f$ is a general non-negative measruable function, then $f \wedge n$ is bounded and measurable, hence $f\wedge n \in V$. Therefore $f \in V$ by the monotone convergence theorem.
\end{proof}
...
\begin{proof}\leavevmode
  \begin{enumerate}
    \item Let $V$ be the set of all measurable such that $(*)$ holds. Then $V$ is a vector space since integration is lienar.
      \begin{enumerate}
...
Recall that when we constructed the Lebesgue measure, the Caratheodory extension theorem yields a measure on the ``Lebesgue $\sigma$-algebra'' $\mathcal{M}$, which was strictly bigger than the Borel $\sigma$-algebra. It was shown in the first example sheet that $\mathcal{M}$ is complete, ie. if we have $A \subseteq B \subseteq \R$ with $B \in \mathcal{M}$, $\mu(B) = 0$, then $A \in \mathcal{M}$. We can also take the Lebesgue measure on $\R^d$ to be defined on $\mathcal{M} \otimes \cdots \otimes \mathcal{M}$. However, it happens that $\mathcal{M} \otimes \mathcal{M}$ together with the Lebesgue measure on $\R^2$ is no longer complete (proof is left as an exercise for the reader).

\subsection{Product measure and independent random variables}
Recall that random variables $X_1, \cdots, X_n$ are independent iff the $\sigma$-algebras $\sigma(X_1), \cdots, \sigma(X_n)$ are independent. We will show that random variables are independent iff their laws are given by the product measure.
...
We are going to prove four very important inequalities in measure theory. These are very useful for bounding the integral of a function. These are
\begin{enumerate}
...
\end{enumerate}
Amongst many things, these will be useful for understanding the $L^p$ spaces, which are the right classes of functions to look at when we want to talk about the Fourier transforms.
...
\begin{defi}[$L^p$ spaces]\index{$L^p$ space}\index{$\|f\|_p$}
  ...
  \]
\end{defi}
It is not clear that this is a norm, because the triangle inequality is not obvious from the definition. This is going to come as a consequence of the Minkowski inequality. Moreover, it is not even clear this is closed under addition! (the problem of non-negativity can be easily solved by quotienting, as we will soon see)
...
\begin{defi}[Convex function]\index{convex function}
  Let $I \subseteq \R$ be an interval. Then $c: I \to \R$ is convex if for any $t \in [0, 1]$ and $x, y \in I$, then
  \[
...
\begin{proof}
  Consider $x < m < y$ with $x, y, m \in I$. Take
  \[
    t = \frac{y - m}{y - x}.
  \]
  ...
  So we have
  \[
    \frac{1}{m - x}(c(m) - c(x)) \leq \frac{1}{y - m} (c(y) - c(m)).
  \]
  ...
  \[
    \frac{1}{m - x}(c(m) - c(x)) \leq a \leq \frac{1}{y - m} (c(y) - c(m)).
  \]
...
\begin{proof}[Proof of Jensen's inequality]
  ...
  \[
    c(X) \geq aX + b.
  \]
  We want to take the expectation of the LHS, but why can we do that? Recall that
  \[
  ...
  \]
  since $X$ is integrable. So $\E[c(X)]$ makes sense. Note that this does not rule out the possibility that it is infinite. Thus, we have
  \[
...
\begin{proof}
  We assume that $\|f\|_p > 0$ and $\|f\|_p < \infty$. Otherwise, there is nothing to prove. By scaling, we may assume that $\|f\|_p = 1$. We set
  \[
  ...
  \end{align*}
  The key realization now is taht $\frac{1}{q} + \frac{1}{p} = 1$ means that $q(p - 1) = p$. So this becomes
  \[
...
\begin{proof}
  ...
  \[
    \|f + g\|_1 = \int |f + g| \;eq \int (|f| + |g|) = \int |f| + \int |g| = \|f\|_1 + \|g\|_1.
  \]
  The proof of the case of $p = \infty$ is similar. Now assume $p \in (1, \infty)$. We assume that $\|f\|_p, \|g\|< \infty$, or else the inequality is trivial. Similarlly, we assume $\|f + g\|_p \geq 0$.

  We now check that under these assumption, we must have $\|f + g\| < \infty$ as well. Indeed, we have
  \[
...
Given these intequalities, we can go and prove some properties of $L^p$ spaces.

\subsection{Properties of \texorpdfstring{$L^p$}{L^p}}
...
One important property of $L^p$ is that it is complete, ie. every Cauchy sequence converges.
...
\begin{proof}
  ...
  Suppose that $(f_n)$ is a sequence in $L^p$ with $\|f_n - f_m\| \to 0$ as $n, m \to \infty$. Take a subsequence $(f_{n_k})$ of $(f_n)$ with
  \[
  ...
  \[
    \norm{\sum_{k = 1}^\infty |f_{n_{k + 1}} - f_{n_k}|}_p \leq \sum_{k = 1}^M \|f_{n_{k + 1}} - f_{n_k}\|_p \leq 1.
  \]
  In particular,
  \[
    \sum_{k = 1}^\infty |f_{n_{k + 1}} - f_{n_k}| < \infty \text{ a.e.}
  \]
  So $f_{n_k}(x)$ converges a.e. $x$, since the real line is complete. So we set
  \[
    f(x) =
    \begin{cases}
...
Now for $f, g \in L^2$, we can define the inner product
\[
  \bra f, g\ket = \int fg\;\d \mu.
\]
This defines an inner product on $\mathcal{L}^2$ that induces the $\|\ph\|_2$ norm, ie.
\[
  \|f\|_2^2 = \bra f, f\ket.
\]
...
\begin{defi}[Conditional expectation]\index{conditional expectation}
  Suppose we have a probability space $(\Omega, \mathcal{F}, \P)$, and $(G_n)$ be a collection of pairwise disjoint events with $\bigcup_n G_n = \Omega$. We let
  \[
    \mathcal{G} = \sigma(G_n: n \in \N).
  \]
  The \emph{conditional expectation} of $X$ given $\mathcal{G}$ is the random variable
  \[
    Y = \sum_{n = 1}^\infty = \sum_{n = 1}^\infty \E[X \mid G_n] \mathbf{1}_{G_n},
  \]
  where
  \[
...
If $X \in L^s(\P)$, then $Y \in L^2(\P)$, and it is clear that $Y$ is $\mathcal{G}$-measurable. We claim that this is in fact the projection of $X$ onto the subspace $L^2(\mathcal{G}, \P)$ of $\mathcal{G}$-measurable $L^2$ random variables in the ambient space $L^2(\P)$.
...
\end{prop}
In some sense, this tells us $Y$ is our best prediction of $X$ given the information encoded in $\mathcal{G}$.
...
\section{Convergence in \texorpdfstring{$L^1(\P)$}{L^1(P)} and uniform integrability}
What we are looking at here is the following question --- suppose $(X_n), X$ are random variables and $X_n \to X$ in probability. Under what extra assumptions is it true that $X_n$ also converges to $X$ in $L_1$, ie. $\E[X_n - X] \to 0$ as $X \to \infty$.

We have to be very careful with this, because it is not always true. If we take $(\Omega, \mathcal{F}, \P) = ((0, 1), \mathcal{B}((0, 1)), \text{Lebesgue})$, and $X_n = n\mathbf{1}_{(0, 1/n)}$. Then $X_n \to 0$ in probability, and in fact $X_n \to 0$ almost surely. However,
\[
...
\begin{defi}[Uniformly integrable]\index{uniformly integrable}\index{UI}
  ...
  \[
    I_\mathcal{X}(\delta) = \sup\{\E[|X|\mathbf{1}_A]: X \in \mathcal{X}, A \subseteq \mathcal{F} \text{ with }\mathcal{P}[A] < \delta\}.
  \]
...
\begin{eg}
  Let $\mathcal{X} = \{X\}$, where $X \in L^1(\P)$. Then $\mathcal{X}$ is uniformly integrable. To see this, we need to show that for any $\varepsilon > 0$, there exists some $\delta > 0$ such that $\P[A] < \delta$ implies $\E[|X|\mathbf{1}_A] < \varepsilon$.

  Suppose not. Then there exists $\varepsilon > 0$ and a sequence $(A_n)$ in $\mathcal{F}$ where $\E[|X|\mathbf{1}_{A_n}] \geq \varepsilon$ for all $n$, and $\P[A_n] \leq 2^{-n}$. We use the first Bore-Cantelli lemma. Since $\sum \P[A_n] < \infty$, we know $\P[A_n\text{ i.o.}] = 0$. We have
  \[
    \varepsilon \leq \E[X \mathbf{1}_{\bigcup_{m \geq n} A_n}] \to \E[|X| \mathbf{1}_{A_m\text{ i.o.}}]
  \]
  by dominated convergence with dominating function $|X|$. But the last thing is just $0$ since the probability that $A_n \text{ i.o.}$ is zero. This is a contradiction.
\end{eg}

\begin{eg}
  Let $\mathcal{X}$ be an $L^p$-bounded family for some $p > 1$. We claim that $\mathcal{X}$ is uniformly integrable.
...
Looking at the example from earlier, we had $X_n = n \mathbf{1}_{(0, 1/n)}$, which is $L^1$ bounded, but not uniformly integrable. So $L^1$ boundedness is not enough.
...
\begin{thm}[Bounded convegence theorem]
...
\begin{proof}
  ...
  \end{align*}
  We know the second term vanishes, while the first term $\to 0$ as $n \to \infty$. So $|X| \leq C$ a.s.
...
The main application is that when $\{X_n\}$ is a type of stochastic process known as a \term{martingale}. This can be seen in III Advanced Probability and III Stochastic Calculus.

\section{Fourier transform}
From now on, we will write $L^p$ for \emph{complex valued} Borel functions on $\R^d$ with
\[
  \|f\|_p = \left(\int_{\R^d} |f|^p\right)^{1/p} < \infty.
\]
\begin{defi}[Fourier transform]\index{Fourier transform}
  The \emph{Fourier transform} $\hat{f}$ of $f \in L^1(\R^d)$ is given by
  \[
    \hat{f}(u) = \int_{\R^d}f(x) e^{i (u, x)}\;\d x,
  \]
  where $u \in \R^d$ and $(u, x)$ denotes the inner product, ie.
  \[
    (u, x) = u_1 x_1 + \cdots + u_d x_d.
  \]
\end{defi}

The main goal is to show that if $f \in L^1, \hat{f} \in L^1$, then the Fourier inversion holds:
\[
  f(x) = \frac{1}{(2\pi)^d} \int \hat{f}(u) e^{-i(u, x)} \;\d u\text{ a.e.}
\]
We will also prove the Plancherel identity: if $f \in L^2(\R^d)$, then
\[
...
More general, we can define the Fourier transform of a measure:

\begin{defi}[Fourier transform of measure]\index{Fourier transform!of measure}
  The Fourier transform of a \emph{finite measure} $\mu$ on $\R^d$ is the function
  \[
...
\begin{defi}[Characteristic function]\index{characteristic function}\index{random variable!characteristic function}
  Let $X$ be a random variable. Then the \emph{characteristic function} of $X$ is the Fourier transform of its law, ie.
  \[
...
\begin{defi}[Convolution of random variables]\index{convolution!random variable}\index{random variable!convolution}
  ...
  \begin{align*}
    \mu * \varsigma (A) &= \P[X + Y \in \mathcal{A}] \\
    &= \int \int \mathbf{1}_{A}(x + y)\;\mu(\d x)\;\nu(\d y)
  \end{align*}
\end{defi}
Let's suppose that $\mu$ has a density function $f$ with respect to the Lebesgue measure. Then we have
\begin{align*}
  \mu * \nu(A) &= \int \int \mathbf{1}_A(x + y) f(x) \;\d x\;\nu(\d y)\\
  &= \int \int \mathbf{1}_A(x) f(x - y) \;\d x\;\nu(\d y)\\
...
\begin{defi}[Convolution of function with measure]\index{convolution!function with measure}
  ...
  \]
\end{defi}
We check that it is indeed in $L^p$.
\[
  \int \left(\int |f(x - y)| \nu(\d y)\right)^p \;\d x.
\]
...
We now want to work towards proving the Fourier inversion formula. Our strategy is as follows:
\begin{enumerate}
  \item Show that the Fourier inversion formula holds for a Gaussian distribution by direct computations.
  \item Show that the formula holds for Gaussian convolutions, ie. the convolution of an arbitrary function with a Gaussian.
...
We start off by working in the case $d = 1$ and $Z \sim N(0, 1)$. We want to compute the Fourier transform of the law of this guy, ie. the Fourier transform of its characteristic function. We will use a nice trick.
...
\]
So in fact the convolution is pointwise bounded. We see that the bound gets worse as $t \to \infty$, and we will see that this is because as $t \to \infty$, the convolution $f * g_t$ becomes a better and better approximation of $f$, and we did not assume that $f$ is bounded.

Similarly, we can compute that
\[
  \|\widehat{f * g_t}\| = \|\hat{f} \hat{g}_t\|_1 \leq (2\pi)^{d/2} t^{-d/2}\|f\|_1,
\]
and
\[
  \|\widehat{f * g_t}\|_\infty \leq \|\hat{f}\|.
\]
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \left(\frac{1}{2\pi}\right)^d \int \int f(x - y) \hat{g}_t(u) e^{-i(u, y)}\;\d u\;\d y\\
    ...
    &= \left(\frac{2}{2\pi}\right)^d \int \widehat{f * g_t}(u) e^{-i(u, x)}\;\d u
  \end{align*}
  So done.
\end{proof}
\printindex
