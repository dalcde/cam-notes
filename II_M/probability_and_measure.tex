...
The next problem is integrating under the differentiation sign. We want to know when we can say
\[
...
%
  ...
  \begin{enumerate}
    \item It is clear that $\mathbf{1}_E, \mathbf{1}_A \in V$ for all $A \in \mathcal{A}$ (where $\mathcal{A}$ is as i n the definition of the product $\sigma$-algebra).
...
\begin{defi}[$L^p$ spaces]\index{$L^p$ space}\index{$\|f\|_p$}
  ...
  \]
\end{defi}
However, it is not clear that this is a norm. First of all, $\|f\|_p$ does not imply that $f = 0$. It only means that $f = 0$ a.e. But this is easy to solve. We simply quotient out the vector space by functions that differ on a set of measure zero. The more serious problem is that we don't know how to prove the triangle inequality.
...
\subsection{Convergence in \texorpdfstring{$L^1(\P)$}{L^1(P)} and uniform integrability}
...
Of course, not only do we want to know if the Fourier transform exists. We also want to know if it lies in some nice space, eg. $L^2$.
...
\end{thm}
Note that in general, there is no guarantee that $F$ sends a function to its Fourier transform. We know that only if it is a well-behaved function (ie. in $L^1 \cap L^2$). However, the formal property of it being a bijection from $\mathcal{L}^2$ to itself will be convenient for many things.
...
We are now going to study a new topic --- ergodic theory. Due to time constraints, we will only briefly touch on the topic. The general setup of ergodic theory is as follows:
\begin{enumerate}
  \item We have a measure space $(E, \mathcal{E}, \mu)$ and a measurable map $\Theta: E \to E$ that is measure preserving, ie. $\mu(A) = \mu(\Theta^{-1}(A))$ for all $A \in \mathcal{E}$.
\end{enumerate}
...
\]
So $\frac{S_n(f)}{n}$ will the average of the first $n$ things. This will be related to the law of large numbers.
...
\begin{proof}
  ...
  \]
  Since $\mathcal{T}_n$ is a $\sigma$-algebra, we and $\Theta^{-n}(A) \in \mathcal{J}_N$ for all $A \in \mathcal{A}$ and $\sigma(\mathcal{A}) = \mathcal{E}$, we know $\Theta^{-n}(A) \in \mathcal{T}_N$ for all $A \in \mathcal{E}$.

  So if $A \in \mathcal{E}_\Theta$, ie. $A = \mathcal{O}^{-1}(A)$, then $A \in \mathcal{T}_N$ for all $n$. So $A \in \mathcal{T}$.

  From the Kolmogorov 0-1 law, we know either $\mu[A] = 1$ or $\mu[A] = 0$. So done.
\end{proof}

\subsection{Ergodic theorems}
To prove the Ergodic theorems, we need the following magical lemma:
\begin{lemma}
  Let $f$ be integrable, and
  \[
    S^* = \sup_{n \geq 0} S_n(f) \geq 0,
  \]
  since $S_0(f) = 0$ by convention. Then
  \[
