...
The general settings is that we have a measure space $(E, \mathcal{E}, \mu)$ and a measurable map $\Theta: E \to E$ that is measure preserving, ie. $\mu(A) = \mu(\Theta^{-1}(A))$ for all $A \in \mathcal{E}$.
...
\begin{proof}
  ...
  So if $A \in \mathcal{E}_\Theta$, ie. $A = \Theta^{-1}(A)$, then $A \in \mathcal{T}_N$ for all $n$. So $A \in \mathcal{T}$.
...
\begin{proof}[Proof of von Neumann ergodic theorem]
  ...
  Then we have
  \begin{align*}
    \norm{f - g}_p^p &= \int \liminf_n \norm{\frac{S_n(f - g)}{n}}^p \;\d \mu\\
    &\leq \liminf \int \norm{\frac{S_n(f - g)}{n}}^p \;\d\mu\\
    &\leq \|f - g\|_p^p.
  \end{align*}
...
\begin{proof}
  ...
  \]
  So we know that $(S_n/n)^4 \to 0$ a.s., ie. $S_n/n \to 0$ a.s.
\end{proof}
