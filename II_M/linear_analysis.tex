...
\begin{thm}
  ...
  The spectrum is given by $\sigma(T) = \sigma_p(T) \cup 0$. Moreover, for every $\lambda_i \in \sigma_p (T)$, the eigenspace has finite dimensions.
\end{thm}
...
\begin{proof}
  Suppose not. There there are infinitely many independent $\mathbf{x}_1, \mathbf{x}_2, \mathbf{x}_3, \cdots$ such that $T \mathbf{x}_i = \lambda_i \mathbf{x}_i$ with $|\lambda_i| \geq 1$.

  Define $X_n = \spn\{\mathbf{x}_1, \cdots, \mathbf{x}_n\}$. Since the $\mathbf{x}_i$'s are linearly independent, there exists $\mathbf{y}_n \in X_n \cap X_{n - 1}^\perp$ with $\|\mathbf{y}_n\|_H = 1$. Note that this is the only place where we really use the fact that $H$ is a Hilbert space --- we need to produce $X_{n - 1}^\perp$.
