...
\begin{thm}[Osgood]
  Let $f_n: [0, 1] \to \R$ be a sequence of continnuous functions such that for all $x\in [0, 1]$
  \[
    \sup_n |f_n(x)| < \infty
  \]
  Then there are some $a, b$ wiht $0 \leq a < b \leq 1$ such that
  \[
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item Let $\mathbf{w}\in B(1) \subseteq W$. We want to show that there exists $\mathbf{v}\in V$ with $\|\mathbf{v}\|_V < 1$ such that $T\mathbf{v} = \mathbf{w}$.
      ...
      By our estimate, we know that $\sum_{i = 1}^n \mathbf{v}_i$ is a Cauchy sequence.  By the completenss of $\mathbf{V}$,  there is some $\mathbf{v}$ such that
      \[
