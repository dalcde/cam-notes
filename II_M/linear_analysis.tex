...
\begin{proof}
  ...
  \begin{align*}
    \|(T - \lambda I)\mathbf{x}_n \| &\leq \|(T - \lambda_n I) \mathbf{x}_n\| + \|(\lambda - \lambda_n) I \mathbf{x}_n\|\\
  ...
  \[
    \|R(\lambda_n)\|_{\mathcal{B}(X)} |\mu - \lambda_n| = \|R(\lambda_n)\|_{\mathcal{B}(X)} \|(T - \lambda_n I) - (T - \mu I)\|_{B(X)} \leq 1.
  \]
...
\begin{proof}
  ...
  To show that $\im (I - T)$ is closed, it suffices to show it is bounded \emph{below}, i.e.\ there is some $C > 0$ such that
  \[
    \|\mathbf{x}\|_H \leq C \|(I - T) \mathbf{x}\|_H
  \]
  for all $\mathbf{x} \in H$. If this were the case, then if $(I - T) \mathbf{x}_n \to \mathbf{y}$ in $H$, then
  \[
  ...
  But $\mathbf{x}_n \to \mathbf{y}$ also implies $T \mathbf{x}_n \to T \mathbf{y}$. So this means we must have $T\mathbf{y} = \mathbf{y}$. But this is a contradiction, since $(T - I)$ has trivial kernel.
%
