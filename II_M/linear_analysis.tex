...
\begin{proof}
  ...
  \]
  It suffices to prove this for $\lambda$ large. In that case, we have
  \[
    \|(\lambda I - T)^{-1}\|_{\mathcal{B}(X)} \leq \|\lambda^{-1}(I - \lambda^{-1}T)^{-1}\|_{\mathcal{B}(X)} \to 0
  \]
  as $|\lambda| \to \infty$. So it is bounded.

  By ``Liouville's theorem'', $R(\lambda)$ is constant, which is clearly a contradiction since $R(\lambda) \not= R(\mu)$ for $\lambda \not= \mu$.
\end{proof}
Of course, to do this properly, we need a version of Liouville's theorem for Banach-space valued functions as opposed to complex-valued functions. We will do that next time.
\end{document}
