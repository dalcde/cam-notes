...
\begin{cor}[Hahn-Banach theorem 2.0]
  Let $W \subseteq V$ be real normed vector spaces. Given $f \in W^*$, there exists a $\tilde{f} \in V^*$ such that $\tilde{f}|_W = f$ and $\|\tilde{f}\|_{V^*} = \|\tilde{f}\|_{W^*}$.
\end{cor}
...
\begin{proof}
  ...
  \]
  So indeed we have $\|\tilde{f}\|_{V^*} = \|\tilde{f}\|_{W^*}$.
\end{proof}
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item This is the step where we use the Baire category theorem.
      ...
      \]
      We have written $W$ as a coutnable union of closed sets. Since $W$ is a Banach space, by the Baire category thereom, there is some $n \geq 1$ such that $\overline{T(n B_V(1))}$ has non-empty interior. Hence there is some $\delta > 0$, $\mathbf{w}_0 \in W$ such that
      \[
      ...
      \[
        \overline{T(B_V(1))} \supseteq B_W(-\mathbf{w}_0, \delta).
      \]
      Then by linearity, intuitively, since the image contains the balls $\overline{B_W(\mathbf{w}_0, \delta)}$ and $\overline{B_W(-\mathbf{w}_0, \delta)}$, it must contian everything in between. In particular, it must contain $\overline{B_W(\delta)}$.
...
\begin{eg}
  ...
  \[
    T: D(T) \to C([0, 1]),
  \]
  where the domain $D(T)$ is defined as the vector space $C^1([0, 1])$, the vector space of continuous functions $f: [0, 1] \to \R$ such that $f$ is continuously differentiable. We write it as $D(T)$ since we want to use the norm $\|\cdot \|_{D(T)} = \|\cdot \|_{C([0, 1])}$, as opposed to the usual norm on $C^1([0, 1])$. We define $T$ to be the derivative: $T(f) = f'$.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item To show $f$ is continuous, it suffices to check that $\{x: f(x) \> \alpha\}$ and $\{x: f(x) < \alpha\}$ for all $\alpha \in \R$, as this shows that the pre-images of all open intervals in $\R$ are open. We can in fact show that
      \[
        \{x: f(x) < \alpha\} = \bigcup_{q < \alpha} U_q,
      \]
      which is open, and similarly we have
      \[
        \{x: f(x) > \alpha\} = \bigcup_{q > \alpha} X \setminus C_q.
      \]
      So done.
  \end{itemize}
\end{proof}
